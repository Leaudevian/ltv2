...
Citizen.CreateThread(function()
	...
	while true do
		...
		if mdt ~= true then
			local Passengers = GetVehicleNumberOfPassengers(GetVehiclePedIsIn(PlayerPedId()))
			SendNUIMessage({
				text = (Passengers),
				meta = 'passengers'
			})
		...
		if mdt == false and IsPedSittingInAnyVehicle(PlayerPedId()) == false then
			SendNUIMessage({
				meta = 'close'
			})
			first = false
			mdt = true
		elseif mdt ~= false and first ~= true and IsPedSittingInAnyVehicle(PlayerPedId()) == true and GetVehiclePedIsIn(PlayerPedId(), true) == Bus and IsWorking_Bus == true then
			Wait(1000)
				SendNUIMessage({
					meta = 'open'
				})
		...
		if mdt == false then
			local Speed1 = GetEntitySpeed(GetVehiclePedIsIn(PlayerPedId(), false)) * 2.2369
			local meta = 'speed'
		...
		if IsControlJustPressed(1, 214) or IsDisabledControlJustPressed(1, 214) then
			if IsWorking_Bus == true then
				if IsPedSittingInAnyVehicle(PlayerPedId()) and GetVehiclePedIsIn(PlayerPedId(), true) == Bus then
