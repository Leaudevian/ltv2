	...
	while true do
		...
		if NetworkIsPlayerActive(player) then
            ...
            elseif IsEntityDead(playerPed) and InLaststand and not isDead then
                ...
                local killerName = killerId ~= -1 and GetPlayerName(killerId) .. " " .. "("..GetPlayerServerId(killerId)..")" or "Lui-même ou un PNJ"
                local weaponLabel = QBCore.Shared.Weapons[killerWeapon] ~= nil and QBCore.Shared.Weapons[killerWeapon]["label"] or "Inconnue"
                local weaponName = QBCore.Shared.Weapons[killerWeapon] ~= nil and QBCore.Shared.Weapons[killerWeapon]["name"] or "Arme inconnue"
                TriggerServerEvent("qb-log:server:CreateLog", "Mort", GetPlayerName(player) .. " ("..GetPlayerServerId(player)..") est mort", "red", "**".. killerName .. "** a tué ".. GetPlayerName(player) .." avec un **".. weaponLabel .. "** (" .. weaponName .. ")")
	...
	while true do
            ...
            if isDead then
                if not isInHospitalBed then 
                    if deathTime > 0 then
                        ...
                        DrawTxt(0.865, 1.44, 1.0, 1.0, 0.6, "~w~ Maintenir ~r~[E] ("..hold.." sec.)~w~ pour réapparaîte ~r~("..Config.BillCost.."$)~w~", 255, 255, 255, 255)
                    end
                end
		...
		EnableControlAction(0, 249, true)
                ...
                if LaststandTime > Laststand.MinimumRevive then
                    DrawTxt(0.94, 1.44, 1.0, 1.0, 0.6, "Vous êtes inconscient : ~r~" .. math.ceil(LaststandTime) .. "~w~ secondes", 255, 255, 255, 255)
                else
                    DrawTxt(0.845, 1.44, 1.0, 1.0, 0.6, "Vous êtes inconscient : ~r~" .. math.ceil(LaststandTime) .. "~w~ secondes, vous pouvez être pris en charge", 255, 255, 255, 255)
                end
