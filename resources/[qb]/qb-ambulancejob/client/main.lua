...
BodyParts = {
    ['HEAD'] = { label = 'tête', causeLimp = false, isDamaged = false, severity = 0 },
    ['NECK'] = { label = 'cou', causeLimp = false, isDamaged = false, severity = 0 },
    ['SPINE'] = { label = 'colonne vertébrale', causeLimp = true, isDamaged = false, severity = 0 },
    ['UPPER_BODY'] = { label = 'haut du corp', causeLimp = false, isDamaged = false, severity = 0 },
    ['LOWER_BODY'] = { label = 'bas du corp', causeLimp = true, isDamaged = false, severity = 0 },
    ...
    ['LFINGER'] = { label = 'doigts gauche', causeLimp = false, isDamaged = false, severity = 0 },
    ...
    ['RHAND'] = { label = 'main droite', causeLimp = false, isDamaged = false, severity = 0 },
    ['RFINGER'] = { label = 'doigts droit', causeLimp = false, isDamaged = false, severity = 0 },
    ['RLEG'] = { label = 'jambe droite', causeLimp = true, isDamaged = false, severity = 0 },
        ...
        if QBCore ~= nil then
            ...
            if #(pos - vector3(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z)) < 1.5 then
                if doctorCount >= Config.MinimalDoctors then
                    QBCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "~g~E~w~ - Appeler un medecin")
                else
                    QBCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "~g~E~w~ - Enregistrement")
                end
                if IsControlJustReleased(0, 38) then
                    if doctorCount >= Config.MinimalDoctors then
                        ...
                        QBCore.Functions.Progressbar("hospital_checkin", "Enregistrement ...", 2000, false, true, {
                            ...
                            if bedId ~= nil then 
                                ...
                                QBCore.Functions.Notify("Les lits sont occupés ...", "error")
                            end
                        end, function() -- Cancel
                            TriggerEvent('animations:client:EmoteCommandStart', {"c"})
                            QBCore.Functions.Notify("Echec de l'enregistrement!", "error")
                        end)
                    end
                end
            elseif #(pos - vector3(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z)) < 4.5 then
                if doctorCount >= Config.MinimalDoctors then
                    QBCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "Appeler")
                else
                    QBCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "Enregistrement")
                end
            end
            ...
            if closestBed ~= nil and not isInHospitalBed then
                if #(pos - vector3(Config.Locations["beds"][closestBed].coords.x, Config.Locations["beds"][closestBed].coords.y, Config.Locations["beds"][closestBed].coords.z)) < 1.5 then
                    QBCore.Functions.DrawText3D(Config.Locations["beds"][closestBed].coords.x, Config.Locations["beds"][closestBed].coords.y, Config.Locations["beds"][closestBed].coords.z - 0.3, "~g~E~w~ - S'allonger")
                    if IsControlJustReleased(0, 38) then
    ...
    while true do
        ...
        if QBCore ~= nil then
            if isInHospitalBed and canLeaveBed then
                local pos = GetEntityCoords(PlayerPedId())
                QBCore.Functions.DrawText3D(pos.x, pos.y, pos.z, "~g~E~w~ - Sortir du lit")
                if IsControlJustReleased(0, 38) then
                    LeaveBed()
                end
            else
...
AddEventHandler('hospital:client:HealInjuries', function(type)
    ...
    TriggerServerEvent("hospital:server:RestoreWeaponDamage")
    QBCore.Functions.Notify("Vos blessures ont été guéries!")
end)
...
AddEventHandler('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    Citizen.CreateThread(function ()
        Citizen.Wait(5)
        if isRevive then
            QBCore.Functions.Notify("Vous êtes pris en charge ...")
...
AddEventHandler('hospital:client:SendBillEmail', function(amount)
    SetTimeout(math.random(2500, 4000), function()
        ...
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = "Pillbox",
            subject = "Facturation Hôpital",
            message = "Chère " .. gender .. " " .. charinfo.lastname .. ",<br /><br />Vous avez reçu un email avec la facture de la dernière visite à l'hôpital.<br />Les coûts finaux sont de : <strong>$"..amount.."</strong><br /><br />Nous vous souhaitons une récupération rapide!",
            button = {}
        })
    end)
end)
        ...
        if #injured > 0 then
            ...
            if #injured <= Config.AlertShowInfo then
                for k, v in pairs(injured) do
                    limbDamageMsg = limbDamageMsg .. "Votre " .. v.label .. " se sent "..Config.WoundStates[v.severity]
                    if k < #injured then
                        limbDamageMsg = limbDamageMsg .. " | "
                    end
                end
            else
                limbDamageMsg = "Vous avez des douleurs sur de nombreux endroits ..."
            end
...
function DoBleedAlert()
    if not isDead and tonumber(isBleeding) > 0 then
        QBCore.Functions.Notify("Vous êtes "..Config.BleedingStates[tonumber(isBleeding)].label, "error", 5000)
    end
end
