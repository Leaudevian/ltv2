...
AddEventHandler('hospital:server:SendDoctorAlert', function()
	...
	for k, v in pairs(QBCore.Functions.GetPlayers()) do
		...
		if Player ~= nil then 
			if (Player.PlayerData.job.name =="ambulance" and Player.PlayerData.job.onduty) then
				TriggerClientEvent("hospital:client:SendAlert", v, "Un médecin est nécessaire à l\'Hospital Pillbox")
			end
		end
	end
end)
...
AddEventHandler('hospital:server:MakeDeadCall', function(blipSettings, gender, street1, street2)
	local src = source
	local genderstr = "Homme"

	if gender == 1 then genderstr = "Femme" end

	if street2 ~= nil then
		TriggerClientEvent("112:client:SendAlert", -1, "Un(e) ".. genderstr .." est blessé(e) à " ..street1 .. " "..street2, blipSettings)
		TriggerClientEvent('qb-policealerts:client:AddPoliceAlert', -1, {
            timeOut = 5000,
            alertTitle = "Personne blessé(e)",
            details = {
                [1] = {
		...
		TriggerClientEvent("112:client:SendAlert", -1, "Un(e) ".. genderstr .." est blessé(e) à "..street1, blipSettings)
		TriggerClientEvent('qb-policealerts:client:AddPoliceAlert', -1, {
            timeOut = 5000,
            alertTitle = "Personne blessé(e)",
            details = {
...
QBCore.Commands.Add("status", "Vérifiez la santé d'un joueur", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
	if Player.PlayerData.job.name == "ambulance" then
		TriggerClientEvent("hospital:client:CheckStatus", source)
	else
		TriggerClientEvent('QBCore:Notify', source, "Vous n\'êtes pas un EMS", "error")
	end
end)

QBCore.Commands.Add("heal", "Guérir un joueur", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source) 
	if Player.PlayerData.job.name == "ambulance" then
...
QBCore.Commands.Add("revivep", "Faire revivre un joueur", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
	if Player.PlayerData.job.name == "ambulance" then
...
QBCore.Commands.Add("revive", "Faire revivre un joueur ou vous-même (Admin Only)", {{name="id", help="Player ID (may be empty)"}}, false, function(source, args)
	if args[1] ~= nil then
...
QBCore.Commands.Add("setpain", "Définir vôtre niveau de blessure ou celui d'un joueur (Admin Only)", {{name="id", help="Player ID (may be empty)"}}, false, function(source, args)
	if args[1] ~= nil then
		local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
		if Player ~= nil then
...
QBCore.Commands.Add("kill", "Tuez un joueur ou vous-même (Admin Only)", {{name="id", help="Player ID (may be empty)"}}, false, function(source, args)
	if args[1] ~= nil then
		local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
		if Player ~= nil then
...
AddEventHandler('hospital:server:CanHelp', function(helperId, canHelp)
	...
	if canHelp then
		...
		TriggerClientEvent('QBCore:Notify', helperId, "Vous ne pouvez pas aider cette personne..", "error")
	end
end)
