            ...
            if InApartment then
                ...
                if CurrentDoorBell ~= 0 then
                    if entrancedist < 1.2 then
                        QBCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.exit.x, Apartments.Locations[ClosestHouse].coords.enter.y - POIOffsets.exit.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.exit.z + 0.1, '~g~G~w~ - Ouvrir la porte')
                ...
                if entrancedist < 3 then
                    QBCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.exit.x, Apartments.Locations[ClosestHouse].coords.enter.y - POIOffsets.exit.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.exit.z, '~g~E~w~ - Quitter l\'appartement')
                ...
                if stashdist < 1.2 then
                    QBCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.stash.x, Apartments.Locations[ClosestHouse].coords.enter.y - POIOffsets.stash.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.stash.z, '~g~E~w~ - Cacher')
                    if IsControlJustPressed(0, 38) then -- E
                ...
                elseif stashdist < 3 then
                    QBCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.stash.x, Apartments.Locations[ClosestHouse].coords.enter.y - POIOffsets.stash.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.stash.z, 'Cacher')
                end

                --Outfits
                if outfitsdist < 1.2 then
                    QBCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.clothes.x, Apartments.Locations[ClosestHouse].coords.enter.y - POIOffsets.clothes.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.clothes.z, '~g~E~w~ - Tenues')
                    if IsControlJustPressed(0, 38) then -- E
                        TriggerEvent('qb-clothing:client:openOutfitMenu')
                    end
                elseif outfitsdist < 3 then
                    QBCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.clothes.x, Apartments.Locations[ClosestHouse].coords.enter.y - POIOffsets.clothes.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.clothes.z, 'Tenues')
                end

                --Logout
                if logoutdist < 1.5 then
                    QBCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.logout.x, Apartments.Locations[ClosestHouse].coords.enter.y + POIOffsets.logout.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.logout.z, '~g~E~w~ - Se déconnecter')
                    if IsControlJustPressed(0, 38) then -- E
                        TriggerServerEvent('qb-houses:server:LogoutLocation')
                    end
                elseif logoutdist < 3 then
                    QBCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x - POIOffsets.logout.x, Apartments.Locations[ClosestHouse].coords.enter.y + POIOffsets.logout.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.logout.z, 'Se déconnecter')
                end
                ...
                if doorbelldist < 1.2 then
                    QBCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.doorbell.x, Apartments.Locations[ClosestHouse].coords.doorbell.y, Apartments.Locations[ClosestHouse].coords.doorbell.z, '~g~G~w~ - Sonner')
                    if IsControlJustPressed(0, 47) then -- G
                        MenuOwners()
                ...
                if IsOwned then
                   if entrance < 1.2 then
                        QBCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x, Apartments.Locations[ClosestHouse].coords.enter.y, Apartments.Locations[ClosestHouse].coords.enter.z, '~g~E~w~ - Enter dans l\'appartement')
                        if IsControlJustPressed(0, 38) then -- E
                ...
                elseif not IsOwned then
                    if entrance < 1.2 then
                        QBCore.Functions.DrawText3D(Apartments.Locations[ClosestHouse].coords.enter.x, Apartments.Locations[ClosestHouse].coords.enter.y, Apartments.Locations[ClosestHouse].coords.enter.z, '~g~E~w~ - Changer d\'apparement')
                        if IsControlJustPressed(0, 38) then -- E
...
AddEventHandler('apartments:client:SpawnInApartment', function(apartmentId, apartment)
    ...
    if rangDoorbell ~= nil then
        ...
        if doorbelldist > 5 then
            QBCore.Functions.Notify("Vous êtes trop loin de la sonnette")
            return
        end
    end
...
AddEventHandler('apartments:client:RingDoor', function(player, house)
    ...
    QBCore.Functions.Notify("Quelqu\'un sonne à la porte!")
end)
...
function MenuOwners()
    ...
    MenuTitle = "Owners"
    ClearMenu()
    Menu.addButton("Sonner à la porte", "OwnerList", nil)
...
function OwnerList()
    QBCore.Functions.TriggerCallback('apartments:GetAvailableApartments', function(apartments)
        ped = PlayerPedId();
        MenuTitle = "Sonner à la porte : "
        ClearMenu()

        if apartments == nil then
            QBCore.Functions.Notify("Il n\'y a personne..", "error", 3500)
            closeMenuFull()
        else
            for k, v in pairs(apartments) do
                Menu.addButton(v, "Sonner", k) 
            end
        end
        Menu.addButton("Retour", "MenuOwners",nil)
    end, ClosestHouse)
end
...
function OutfitsLijst()
    QBCore.Functions.TriggerCallback('apartments:GetOutfits', function(outfits)
        ...
        if outfits == nil then
            QBCore.Functions.Notify("Vous n\'avez pas sauvegardé de tenues...", "error", 3500)
            closeMenuFull()
        else
...
function optionMenu(outfitData)
    ...
    Menu.addButton("Supprimer la tenue", "removeOutfit", outfitData) 
