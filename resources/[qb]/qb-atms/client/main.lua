    ...
    if cards ~= nil and cards[1] ~= nil then
        ...
        for k, v in pairs(Config.ATMModels) do
            ...
            if atm then 
                ...
                QBCore.Functions.Progressbar("accessing_atm", "Accès au guichet automatique", 1500, false, true, {
        ...
        QBCore.Functions.Notify("Vous n'avez de carte de crédit sur vous, rapprochez vous de votre banque.", "error")
    end
end)
...
RegisterNUICallback("removeCard", function(data, cb)
    QBCore.Functions.TriggerCallback('qb-debitcard:server:deleteCard', function(hasDeleted)
        if hasDeleted then
            ...
            QBCore.Functions.Notify('La carte a supprimée.', 'success')
        else
            QBCore.Functions.Notify('Échec de la suppression de la carte.', 'error')
        end
    end, data)
end)
