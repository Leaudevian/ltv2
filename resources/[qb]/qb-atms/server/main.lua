...
Citizen.CreateThread(function()
    while true do
        Wait(3600000)
        dailyWithdraws = {}
        TriggerClientEvent('QBCore:Notify', -1, "Limite de retrait quotidienne réinitialisée", "success")
    end
end)
        ...
        if dailyWith < 5000 then
            ...
            if xCH ~= nil then
                ...
                if bankCount > 0 then
                    ...
                    TriggerClientEvent('QBCore:Notify', src, "Retrait de $" .. data.amount .. ' de votre carte de crédit. Limite quotidienne : ' .. dailyWithdraws[cardHolder], "success")
                else
                    TriggerClientEvent('QBCore:Notify', src, "Vous ne pouvez pas entrer en moins dans l\'ATM.", "error")
                end
                ...
                if bankCount > 0  then
                    ...
                    exports.oxmysql:insert('UPDATE players SET money=@money WHERE citizenid=@citizenid', {['@money'] = xCH.money, ['@citizenid'] = cardHolder})
                    dailyWithdraws[cardHolder] = dailyWithdraws[cardHolder] + tonumber(data.amount)
                    TriggerClientEvent('QBCore:Notify', src, "Retrait de $" .. data.amount .. ' de votre carte de crédit. Limite quotidienne : ' .. dailyWithdraws[cardHolder], "success")
                else
                    TriggerClientEvent('QBCore:Notify', src, "Vous ne pouvez pas entrer en moins dans l\'ATM.", "error")
            ...
            TriggerClientEvent('QBCore:Notify', src, "Vous avez dépassé la limite de retrait quotidienne.", "error")
        end
    end
end)
...
RegisterServerEvent('qb-atms:server:qb-target:atm')
AddEventHandler('qb-atms:server:qb-target:atm', function(data)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    local visas = xPlayer.Functions.GetItemsByName('visa')
    local masters = xPlayer.Functions.GetItemsByName('mastercard')
