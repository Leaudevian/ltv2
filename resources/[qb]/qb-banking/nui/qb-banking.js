...
function populateBanking(data)
{
    ...
    if(data.cardInformation !== undefined) {
        if (data.cardInformation.cardLocked == true) {
            ...
            $("#debitCardStatus").html('<div class="card-header">Carte bloquée</div><div class="card-body">Votre carte est actuellement bloquée.</div><div class="card-footer"><button class="btn btn-primary btn-block" id="unLockCard">Débloqué ma carte</button></div>');
        } else {
            $("#debitCardStatus").removeClass('bg-danger');
        $("#debitCardStatus").addClass('bg-success');
        $("#debitCardStatus").html('<div class="card-header">Carte débloquée</div><div class="card-body">Votre carte est actuellement active.</div><div class="card-footer"><button class="btn btn-primary btn-block" id="lockCard">Bloqué ma carte</button></div>');
        }
...
$(function() {
    ...
    $("#updateNewPin").click(function() {
        ...
        if(newPin !== null && newPin !== undefined && newPin.replace(/[^0-9]/g,"").length === 4) {
            ...
            $("#newPinReqMsg").html('Vous devez entrer un code PIN de 4 chiffres.')
        }
    ...
    $("#initiateWithdraw").click(function() {
        ...
        if(amount !== undefined && amount > 0) {
            ...
            $("#withdrawErrorMsg").html('Une erreur s\'est produite lors du traitement de votre retrait, soit le montant n\'a pas été entré, soit un montant négatif.');
        }
    });
    ...
    $("#initiateWithdrawATM").click(function() {
        ...
        if (currentLimit + parseInt(amount) <= Config.ATMTransLimit) {
            if(amount !== undefined && amount > 0) {
                ...
                $("#withdrawATMError").css({"display":"none"});
                $("#withdrawATMErrorMsg").html('Vous avez retiré avec succès ' + amount + '$ de votre compte.');
                currentLimit = currentLimit + parseInt(amount);
            } else {
                // Error doing withdraw
                $("#withdrawATMErrorMsg").removeClass('alert-success').addClass('alert-danger');
                $("#withdrawATMError").css({"display":"block"});
                $("#withdrawATMErrorMsg").html('Une erreur s\'est produite lors du traitement de votre retrait, soit le montant n\'a pas été entré, soit un montant négatif.');
            }
        } else {
            $("#withdrawATMErrorMsg").removeClass('alert-success').addClass('alert-danger');
            $("#withdrawATMError").css({"display":"block"});
            $("#withdrawATMErrorMsg").html('Désolé, vous avez atteint la limite journalière de retrait de ' + Config.ATMTransLimit + '$ pour la aujourd\'hui, veuillez utiliser une succursale si vous avez besoin d\'argent plus tôt.');
        }
    });
    ...
    $("#initiateDeposit").click(function() {
        ...
        if(amount !== undefined && amount > 0) {
            ...
            $("#depositErrorMsg").html('Une erreur s\'est produite lors du traitement de votre dépôt, soit le montant n\'a pas été entré, soit un montant négatif.');
        }
    });
    ...
    $("#processCard").click(function() {
        ...
        if(pinValue !== null && pinValue !== undefined && pinValue.replace(/[^0-9]/g,"").length === 4) {
            ...
            $("#pinCreatorErrorMsg").html('Il y a eu une erreur lors de la création de votre code PIN, vous devez entrer un code PIN de 4 chiffres.');
        }
    ...
    $("#initiateTransfer").click(function() {
        ...
        if(amount !== undefined && amount !== null && amount > 0 && sortcode !== undefined && sortcode !== null && sortcode > 0 && account !== undefined && account !== null && account > 0) {
            ...
            $("#transferError").css({"display":"block"});
            $("#transferErrorMsg").html('Il y avait une erreur avec les informations que vous avez entrées, assurez-vous que le numéro de compte et le montant sont correctement remplis.');
        }
    ...
    $("[data-action=ATMwithdraw]").click(function() {
        ...
        if (currentLimit + parseInt(amount) <= Config.ATMTransLimit) {
            ...
            $("#successMessageATM").html('Désolé, vous avez atteint la limite journalière de retrait de ' + Config.ATMTransLimit + '$ pour la aujourd\'hui, veuillez utiliser une succursale si vous avez besoin d\'argent plus tôt.');
        }
    });
        ...
        if(number == "ENTER") {
            ...
            if (enteredPin !== null && enteredPin !== undefined || enteredPin.replace(/[^0-9]/g,"").length !== 4) {
                    if(clientPin == enteredPin) {
                        ...
                        $("#pinErrorMsg").html('Vous avez entré un code PIN incorrect.');
                        $("#pinErrorDiv").css({"display":"block"});
                    }
            } else {
                $("#pinErrorMsg").html('Vous devez entrer un code PIN de 4 chiffres.');
                $("#pinErrorDiv").css({"display":"block"});
            }
