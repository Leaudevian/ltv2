...
AddEventHandler('qb-banking:createNewCard', function()
    ...
    TriggerEvent('qb-log:server:CreateLog', 'banking', 'Banque', "Créé nouvelle carte **[" .. xPlayer.PlayerData.citizenid .. "]**", src)
end)
...
AddEventHandler('qb-banking:createBankCard', function(pin)
    ...
    TriggerClientEvent('QBCore:Notify', src, 'Vous avez commandé avec succès une carte de crédit.', 'success')
    
    TriggerEvent('qb-log:server:CreateLog', 'banking', 'Banque', 'Commandé avec succès une carte de crédit', src)
end)
...
AddEventHandler('qb-banking:doQuickDeposit', function(amount)
    ...
    if tonumber(amount) <= currentCash then
        ...
        if bank then
            TriggerClientEvent('qb-banking:openBankScreen', src)
            TriggerClientEvent('qb-banking:successAlert', src, 'Vous avez fait un dépôt '..amount..'$ d\'argent liquide avec succès.')
            TriggerEvent('qb-log:server:CreateLog', 'banking', 'Banque', 'Fait un dépôt '..amount..'$ d\'argent liquide avec succès.', src)
        end
    end
end)
...
AddEventHandler('qb-banking:doQuickWithdraw', function(amount, branch)
    ...
    if tonumber(amount) <= currentCash then
        ...
        if cash then 
            TriggerClientEvent('qb-banking:openBankScreen', src)
            TriggerClientEvent('qb-banking:successAlert', src, 'Vous avez fait un retrait de '..amount..'$ avec succès.')
            TriggerEvent('qb-log:server:CreateLog', 'banking', 'Banque', 'Fait un retrait de '..amount..'$ avec succès.', src)
        end
    end
end)
...
AddEventHandler('qb-banking:updatePin', function(pin)
    if pin ~= nil then 
        ...
        TriggerClientEvent('qb-banking:openBankScreen', src)
        TriggerClientEvent('qb-banking:successAlert', src, 'Vous avez mis à jour votre code PIN de carte de crédit avec succès.')
    end
end)
...
AddEventHandler('qb-banking:savingsDeposit', function(amount)
    ...
    if tonumber(amount) <= currentBank then
        ...
        local savings = savingsAccounts[xPlayer.PlayerData.citizenid].AddMoney(tonumber(amount), 'Transfert de votre compte courant vers votre compte épargne')
        while bank == nil do Wait(0) end
        while savings == nil do Wait(0) end
        TriggerClientEvent('qb-banking:openBankScreen', src)
        TriggerClientEvent('qb-banking:successAlert', src, 'Vous avez fait un dépôt sur votre compte épargne de '..tostring(amount)..'$ avec succès.')
        TriggerEvent('qb-log:server:CreateLog', 'banking', 'Banque', 'Fait un dépôt sur son compte épargne '..tostring(amount)..'$ avec succès..', src)
    end
end)
...
AddEventHandler('qb-banking:savingsWithdraw', function(amount)
    ...
    if tonumber(amount) <= currentSavings then
        local savings = savingsAccounts[xPlayer.PlayerData.citizenid].RemoveMoney(tonumber(amount), 'Transfert de votre compte épargne vers votre compte courant')
        local bank = xPlayer.Functions.AddMoney('bank', tonumber(amount), 'banking-quick-withdraw')
        while bank == nil do Wait(0) end
        while savings == nil do Wait(0) end
        TriggerClientEvent('qb-banking:openBankScreen', src)
        TriggerClientEvent('qb-banking:successAlert', src, 'Vous avez fait un retrait de votre compte épargne '..tostring(amount)..'$ avec succès.')
        TriggerEvent('qb-log:server:CreateLog', 'banking', 'Banque', 'A fait un retrait de son compte épargne '..tostring(amount)..'$ avec succès.', src)
    end
end)
...
AddEventHandler('qb-banking:createSavingsAccount', function()
    ...
    TriggerClientEvent('qb-banking:openBankScreen', src)
    TriggerClientEvent('qb-banking:successAlert', src, 'Vous avez ouvert avec succès un compte d\'épargne.')
    TriggerEvent('qb-log:server:CreateLog', 'banking', 'Banque', "Ouverture d'un nouveau compte épargne", src)
end)


QBCore.Commands.Add('givecash', 'Donner de l\'argent au joueur.', {{name = 'id', help = 'Id du joueur'}, {name = 'amount', help = 'Montant'}}, true, function(source, args)
    local src = source
				...
				if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(id))) < distance then
					if xPlayer.Functions.RemoveMoney('cash', amount) then
						if xReciv.Functions.AddMoney('cash', amount) then
							TriggerClientEvent('QBCore:Notify', src, "Vous avez donné à l\'ID " .. tostring(id) .. ' ' .. tostring(amount) .. '$ avec succès.', "success")
							TriggerClientEvent('QBCore:Notify', id, "Vous avez reçus " .. tostring(amount) .. '$ de l\'ID ' .. tostring(src) .. ' avec succès.', "success")
							TriggerClientEvent("payanimation", src)
						else
							TriggerClientEvent('QBCore:Notify', src, "Impossible de donner à l'ID donné.", "error")
						end
					else
						TriggerClientEvent('QBCore:Notify', src, "Vous n\'avez pas ce montant.", "error")
					end
				else
					TriggerClientEvent('QBCore:Notify', src, "Vous êtes trop loin de lui.", "error")
				end
			else
				TriggerClientEvent('QBCore:Notify', src, "Tu es mort LOLILOL.", "error")
			end
		else
			TriggerClientEvent('QBCore:Notify', src, "Mauvais ID.", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', src, "Utilisation / Givecash [ID] [AMOUNT]", "error")
	end
end)
