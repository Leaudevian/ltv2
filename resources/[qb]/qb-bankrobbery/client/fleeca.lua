                                    ...
                                    if lockerDist < 0.5 then
                                        ...
                                        if IsControlJustPressed(0, 38) then
                                            if CurrentCops >= Config.MinimumFleecaPolice then
                                                ...
                                                QBCore.Functions.Notify('Minimum de '..Config.MinimumFleecaPolice..' policiers nécessaires', "error") 
                                            end
                                        end
                                    end
                                end
    ...
    if closestBank ~= nil then
        QBCore.Functions.TriggerCallback('qb-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                        ...
                        if CurrentCops >= Config.MinimumFleecaPolice then
                            if not Config.SmallBanks[closestBank]["isOpened"] then 
                                QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                                    if result then 
                                        TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                                        QBCore.Functions.Progressbar("hack_gate", "Connexion du périphérique de piratage ..", math.random(5000, 10000), false, true, {
                                        ...
                                        QBCore.Functions.Notify("Il vous manque quelque chose ..", "error")
                                    end
                                end, "trojan_usb")
                            else
                                QBCore.Functions.Notify("On dirait que la banque est déjà ouverte ..", "error")
                            end
                        else
                            QBCore.Functions.Notify('Minimum de '..Config.MinimumFleecaPolice..' policiers nécessaires', "error")
                        end
                ...
                QBCore.Functions.Notify("La sécurité est actif, l\'ouverture de la porte n'est pas possible...", "error", 5500)
            end
        end)
    end
end)
    ...
    if bankId == "paleto" then
        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
            if hasItem then
                ...
                IsDrilling = true
                QBCore.Functions.Progressbar("open_locker_drill", "Briser le coffre-fort ..", math.random(18000, 30000), false, true, {
                    disableMovement = true,
                    disableCarMovement = true,
                    disableMouse = false,
                    disableCombat = true,
                    ...
                    QBCore.Functions.Notify("Succès!", "success")
                    IsDrilling = false
                end, function() -- Cancel
                    StopAnimTask(PlayerPedId(), "anim@heists@fleeca_bank@drilling", "drill_straight_idle", 1.0)
                    TriggerServerEvent('qb-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                ...
                QBCore.Functions.Notify("On dirait que le coffre est trop résistant ..", "error")
                TriggerServerEvent('qb-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            end
        end, "drill")
    elseif bankId == "pacific" then
        ...
        IsDrilling = true
        QBCore.Functions.Progressbar("open_locker", "Briser le coffre-fort ..", math.random(27000, 37000), false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
...
AddEventHandler('qb-bankrobbery:client:robberyCall', function(type, key, streetLabel, coords)
    if PlayerJob.name == "police" and onDuty then 
        ...
        elseif type == "paleto" then
            ...
            TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
                timeOut = 10000,
                alertTitle = "Tentative de braquage à la Banque Robbery",
                coords = {
                    x = coords.x,
                    y = coords.y,
                    z = coords.z,
        ...
        elseif type == "pacific" then
            ...
            TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
                timeOut = 10000,
                alertTitle = "Tentative de braquage à la Banque Pacific",
                coords = {
                    x = coords.x,
                    y = coords.y,
                    z = coords.z,
