        ...
        if dist < 1.5 then
            if CurrentCops >= Config.MinimumThermitePolice then
                if not Config.PowerStations[closestStation].hit then
                    ...
                    QBCore.Functions.Notify("Il semble que les fusibles sont grillés.", "error")
                end
            else
                QBCore.Functions.Notify('Minimum de'..Config.MinimumThermitePolice..' policiers nécessaires', "error")
            end
        end
...
RegisterNUICallback('thermitesuccess', function()
    ...
    local coords = GetEntityCoords(PlayerPedId())
    while time > 0 do 
        QBCore.Functions.Notify("La charge explose dans " .. time .. "..")
        Citizen.Wait(1000)
        time = time - 1
    end
    local randTime = math.random(10000, 15000)
    ...
    if currentStation ~= 0 then
        QBCore.Functions.Notify("Les fusibles sont grillés", "success")
        TriggerServerEvent("qb-bankrobbery:server:SetStationStatus", currentStation, true)
    elseif currentGate ~= 0 then
        QBCore.Functions.Notify("La porte est ouverte", "success")
