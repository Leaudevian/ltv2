...
AddEventHandler('qb-bankrobbery:server:callCops', function(type, bank, streetLabel, coords)
    ...
    if type == "small" then
        ...
        bankLabel = "Fleeca"
        msg = "L\'alarme a été activée à "..bankLabel.. " " ..streetLabel.." (CAMERA ID: "..cameraId..")"
    elseif type == "paleto" then
        cameraId = Config.BigBanks["paleto"]["camId"]
        bankLabel = "Blaine County Savings"
        msg = "L\'alarme a été activée à "..bankLabel.. " Paleto Bay (CAMERA ID: "..cameraId..")"
    elseif type == "pacific" then
        bankLabel = "Pacific Standard Bank"
        msg = "L\'alarme a été activée à "..bankLabel.. " Alta St (CAMERA ID: 1/2/3)"
    end
    local alertData = {
        title = "Bank robbery",
...
QBCore.Functions.CreateUseableItem("thermite", function(source, item)
	...
	if Player.Functions.GetItemByName('lighter') ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', source, "Vous manquez la source d'allumage ", "error")
    end
end)
