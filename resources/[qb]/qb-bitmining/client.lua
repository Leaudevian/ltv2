...
Citizen.CreateThread(function()
	while true do
		...
		if #(pos - Config.shop["coords"]) < 5.0 then
			inRange = true
			DrawText3Ds(Config.shop["coords"], "~g~E~w~ - Ouvrir le magasin")
		...
		if active_machines ~= nil then
			for k, v in pairs(active_machines) do
				if #(pos - v.coords) < 3.0 then
					inRange = true
					DrawText3Ds(v.coords, "Minage fini dans : ~r~"..v.time)
		...
		if #(pos - Config.MiningLab["coords"]) < 3.0 then
			inRange = true
			DrawText3Ds(Config.MiningLab["coords"], "~g~E~w~ - Quitter")
			if IsControlJustPressed(0, 38) then
		...
		if #(pos - vector3(Config.ExitCoords.x,Config.ExitCoords.y,Config.ExitCoords.z)) < 3.0 then
			inRange = true
			DrawText3Ds(Config.ExitCoords, "~g~E~w~ - Entrer dans la ferme de minage")
			if IsControlJustPressed(0, 38) then
...
AddEventHandler("qb-cryptomining:client:installCPU", function(name, reward, item)
	...
	if #(pos - Config.MiningLab["coords"]) < 50.0 then
		...
		function() -- failure
			QBCore.Functions.Notify("Vous pouvez faire mieux que sa", "error")
   	 	end)
	else
		QBCore.Functions.Notify('Pas un emplacement approprié pour l\'installation','error')
	end
end)
...
function InstallCPU(name, reward, itemname)
	if #active_machines == 0 then
		...
		local machine = {}
		QBCore.Functions.Progressbar("cpuinstall", "Installation de RIG", 10000, false, true, {
			...
			ClearPedTasks(ped)
			QBCore.Functions.Notify("Installation arrêtée..", "error")
		end)
	else
		QBCore.Functions.Notify("Vous avez déjà une machine en cours d'exécution", "error")
	end
end
