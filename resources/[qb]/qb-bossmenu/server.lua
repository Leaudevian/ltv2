...
AddEventHandler("qb-bossmenu:server:withdrawMoney", function(amount)
    ...
    TriggerEvent('qb-log:server:CreateLog', 'bossmenu', 'Retrait d\'argent', "Retiré avec succès " .. amount .. '$ (' .. job .. ')', src)
end)
...
AddEventHandler("qb-bossmenu:server:depositMoney", function(amount)
    ...
    SaveResourceFile(GetCurrentResourceName(), "./accounts.json", json.encode(Accounts), -1)
    TriggerEvent('qb-log:server:CreateLog', 'bossmenu', 'Déposer d\'argent', "Déposé avec succès " .. amount .. '$ (' .. job .. ')', src)
end)
...
AddEventHandler('qb-bossmenu:server:updateGrade', function(target, grade)
    ...
    if Employee then
        if Employee.Functions.SetJob(Player.PlayerData.job.name, grade) then
            ...
            TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source, "Votre nouveau grade est [" ..grade.."].", "success")
        else
        ...
        if player[1] ~= nil then
            ...
            exports.oxmysql:insert('UPDATE players SET job=@job WHERE citizenid=@citizenid', {['@job'] = json.encode(employeejob), ['@citizenid'] = target})
            TriggerClientEvent('QBCore:Notify', src, "Grade changé avec succès!", "success")
        else
...
AddEventHandler('qb-bossmenu:server:fireEmployee', function(target)
    ...
    if Employee then
        if Employee.Functions.SetJob("unemployed", '0') then
            TriggerEvent('qb-log:server:CreateLog', 'bossmenu', 'Viré de son job', GetPlayerName(Employee.PlayerData.source) .. ' (' .. Player.PlayerData.job.name .. ') viré avec succés ', src)
            TriggerClientEvent('QBCore:Notify', src, "Viré avec succès!", "success")
            TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source , "Vous êtes viré !", "error")
        else
        ...
        if player[1] ~= nil then
            ...
            job.grade.name = nil
            job.grade.level = 0
            exports.oxmysql:insert('UPDATE players SET job=@job WHERE citizenid=@citizenid', {['@job'] = json.encode(job), ['@citizenid'] = target})
            TriggerClientEvent('QBCore:Notify', src, "Viré avec succès!", "success")
            TriggerEvent('qb-log:server:CreateLog', 'bossmenu', 'Viré', data.source .. ' (' .. Player.PlayerData.job.name .. ') viré avec succés', src)
        else
            TriggerClientEvent('QBCore:Notify', src, "le joueur n'existe pas", "error")
        end
    end
...
AddEventHandler('qb-bossmenu:server:giveJob', function(recruit)
    ...
    if Player.PlayerData.job.isboss == true then
        if Target and Target.Functions.SetJob(Player.PlayerData.job.name, 0) then
            TriggerClientEvent('QBCore:Notify', src, "Vous avez recruté " .. (Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname) .. " pour " .. Player.PlayerData.job.label .. "", "success")
            TriggerClientEvent('QBCore:Notify', Target.PlayerData.source , "Vous avez été recruté pour " .. Player.PlayerData.job.label .. "", "success")
            TriggerEvent('qb-log:server:CreateLog', 'bossmenu', 'Recruter', "Recruté avec succès " .. (Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname) .. ' (' .. Player.PlayerData.job.name .. ')', src)
        end
    end
end)
