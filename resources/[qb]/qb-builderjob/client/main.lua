                ...
                if MainDistance < 2 then
                    ...
                    if TaskData ~= nil then
                        if not BuilderData.ShowDetails then
                            DrawText3Ds(data.coords.x, data.coords.y, data.coords.z, '[E] Voir Détails')
                            DrawText3Ds(data.coords.x, data.coords.y, data.coords.z + 0.2, 'Exercices: '..TaskData.completed..' / '..TaskData.total)
                        else
                            DrawText3Ds(data.coords.x, data.coords.y, data.coords.z, '[E] Masquer Détails')
                            for k, v in pairs(Config.Projects[Config.CurrentProject].ProjectLocations["tasks"]) do
                                if v.completed then
                                    DrawText3Ds(data.coords.x, data.coords.y, data.coords.z + OffsetZ, v.label..': Complété')
                                else
                                    DrawText3Ds(data.coords.x, data.coords.y, data.coords.z + OffsetZ, v.label..': Pas achevé')
                                end
                                OffsetZ = OffsetZ + 0.2
                            end
                        end
                        ...
                        if TaskData.completed == TaskData.total then
                            DrawText3Ds(data.coords.x, data.coords.y, data.coords.z - 0.2, '[G] Finir le projet')
                ...
                if not v.completed or not v.IsBusy then
                    ...
                    if TaskDistance < 10 then
                        ...
                        if TaskDistance < 1.5 then
                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z + 0.25, '[E] Complété la tache')
                            if IsControlJustPressed(0, 38) then
