                    ...
                    if dist <= 7.5 then
                        if Driver == PlayerPed then
                            if not washingVehicle then
                                DrawText3Ds(Config.Locations[k]["coords"]["x"], Config.Locations[k]["coords"]["y"], Config.Locations[k]["coords"]["z"], '~g~E~w~ - Câliner votre Auto ('..Config.DefaultPrice..'$)')
                                if IsControlJustPressed(0, 38) then
                                    TriggerServerEvent('qb-carwash:server:washCar')
                                end
                            else
                                DrawText3Ds(Config.Locations[k]["coords"]["x"], Config.Locations[k]["coords"]["y"], Config.Locations[k]["coords"]["z"], 'Le câlin n\'est pas disponible ..')
                            end
                        end
                    end
                end
...
AddEventHandler('qb-carwash:client:washCar', function()
    ...
    QBCore.Functions.Progressbar("search_cabin", "Votre auto a été câlinée avec soin :)", math.random(4000, 8000), false, true, {
        ...
        QBCore.Functions.Notify("Câlin arrêtai suite à une main au pare-choc arrière :(", "error")
        washingVehicle = false
    end)
end)
