...
Citizen.CreateThread(function()
    for k, v in pairs (Config.Stores) do
        ...
        if Config.Stores[k].shopType == "surgeon" then
            ...
            BeginTextCommandSetBlipName("STRING")
            AddTextComponentString("Chirurgien")
		...
		local dist = #(pos - Config.Stores[k].coords)
                ...
                if dist < 30 then
                    if not creatingCharacter then
                        ...
                        if dist < 5 then
                            if Config.Stores[k].shopType == "clothing" then
                                 DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - Acheter des vêtements')
                            elseif Config.Stores[k].shopType == "barber" then
                                DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - Se faire couper les cheveux')
                            elseif Config.Stores[k].shopType == "surgeon" then
                                DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - Obtenir une chirurgie plastique')
                            end
                            if IsControlJustPressed(0, 38) then -- E
                                if Config.Stores[k].shopType == "clothing" then
                                    customCamLocation = nil
                                    openMenu({
                                        {menu = "character", label = "Vêtements", selected = true},
                                        {menu = "accessoires", label = "Accessoires", selected = false}
                                    })
                                elseif Config.Stores[k].shopType == "barber" then
                                    customCamLocation = nil
                                    openMenu({
                                        {menu = "clothing", label = "Cheveux", selected = true},
                                    })
                                elseif Config.Stores[k].shopType == "surgeon" then
                                    customCamLocation = nil
                                    openMenu({
                                        {menu = "clothing", label = "Caractéristiques", selected = true},
                                    })
                                end
                            end
                        end
                    ...
                    if not creatingCharacter then
                        ...
                        if dist < 2 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                DrawText3Ds(Config.ClothingRooms[k].coords.x, Config.ClothingRooms[k].coords.y, Config.ClothingRooms[k].coords.z + 0.3, '~g~E~w~ - Voir les vêtements')
                                if IsControlJustPressed(0, 38) then -- E
                                    ...
                                    QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
                                        openMenu({
                                            {menu = "roomOutfits", label = "Tenues", selected = true, outfits = Config.Outfits[PlayerData.job.name][gender]},
                                            {menu = "myOutfits", label = "Mes tenues", selected = false, outfits = result},
                                            {menu = "character", label = "Vêtements", selected = false},
                                            {menu = "accessoires", label = "Accessoires", selected = false}
                                        })
                                    end)
                                end
                                ...
                                if PlayerData.gang.name == Config.ClothingRooms[k].requiredJob then
                                    ...
                                    if IsControlJustPressed(0, 38) then -- E
                                        ...
                                        QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
                                            openMenu({
                                                {menu = "roomOutfits", label = "Tenues", selected = true, outfits = Config.Outfits[PlayerData.gang.name][gender]},
                                                {menu = "myOutfits", label = "Mes tenues", selected = false, outfits = result},
                                                {menu = "character", label = "Vêtements", selected = false},
                                                {menu = "accessoires", label = "Accessoires", selected = false}
                                            })
...
AddEventHandler('qb-clothing:client:openOutfitMenu', function()
    QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
        openMenu({
            {menu = "myOutfits", label = "Mes tenues", selected = true, outfits = result},
        })
    end)
end)
...
AddEventHandler('qb-clothing:client:openMenu', function()
    customCamLocation = nil
    openMenu({
        {menu = "character", label = "Personnage", selected = true},
        {menu = "clothing", label = "Tenues", selected = false},
        {menu = "accessoires", label = "Accessoires", selected = false}
    })
end)
...
RegisterNUICallback('TrackerError', function()
    QBCore.Functions.Notify("Vous ne pouvez pas retirer votre bracelet electronnique ^^ ..", "error")
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    ...
    QBCore.Functions.Notify("Vous avez supprimé votre tenue : "..data.outfitName.." !")
end)
...
AddEventHandler('qb-clothes:client:CreateFirstCharacter', function()
    QBCore.Functions.GetPlayerData(function(PlayerData)
        ...
        openMenu({
            {menu = "character", label = "Personnage", selected = true},
            {menu = "clothing", label = "Personnalisé", selected = false},
            {menu = "accessoires", label = "Accessoires", selected = false}
        })
...
AddEventHandler('qb-clothing:client:loadOutfit', function(oData)
    ...
    if oData.outfitName ~= nil then
        QBCore.Functions.Notify("Vous avez choisi "..oData.outfitName.."! Appuyez sur Confirmer pour confirmer la tenue.")
    end
end)
