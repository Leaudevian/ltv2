...
Citizen.CreateThread(function()
    while true do
        ...
        if distance < 6 then
            ...
            if distance < 2 then
                DrawText3Ds(-331.77, -2444.70, 7.36, "[G] Traitement de la feuille de coca")
...
Citizen.CreateThread(function()
    while true do
        ...
        if distance1 < 15 then
            ...
            if distance1 < 2 then
                DrawText3Ds(509.89, 6478.76, 29.77, "[G] Récolter")
                if IsControlJustPressed(0, 47) then
                    PrepareAnim()
                    PickMinigame()
                end
            ...
            if distance2 < 2 then
                DrawText3Ds(509.47, 6472.89, 29.77, "[G] Récolter")
                if IsControlJustPressed(0, 47) then
                    PrepareAnim()
                    PickMinigame()
                end
            ...
            if distance3 < 2 then
                DrawText3Ds(509.47, 6467.10, 29.74, "[G] Récolter")
                if IsControlJustPressed(0, 47) then
                    PrepareAnim()
                    PickMinigame()
                end
...
Citizen.CreateThread(function()
    while true do
        ...
        if distance < 6 then
            ...
            if distance < 2 then
                DrawText3Ds(-1078.21, -1678.42, 4.57, "[G] Transformation en crack")
                if IsControlJustPressed(0, 47) then
...
function pickProcess()
    QBCore.Functions.Progressbar("grind_coke", "Récolte des feuilles de coca ..", math.random(80000,120000), false, true, {
...
function cokeProcess()
    QBCore.Functions.Progressbar("grind_coke", "Traiter les feuilles de coke ..", math.random(10000, 12000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
...
function crackProcess()
    QBCore.Functions.Progressbar("grind_coke", "Transformation en Crack ..", math.random(10000, 12000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
...
function PickMinigame()
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            pickProcess()
            QBCore.Functions.Notify("Vous avez récolté une feuille de coca", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
        else    
            ...
            QBCore.Functions.Notify("Vous avez foiré la feuille de coca!", "error")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
            cokepicking = false
...
function ProcessMinigame(source)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            cokeProcess()
            QBCore.Functions.Notify("Vous faites de la coke!", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
        else    
            ...
            QBCore.Functions.Notify("Vous avez gâché le processus!", "error")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
            cokeprocess = false
...
function ProcessCrackMinigame(source)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            crackProcess()
            QBCore.Functions.Notify("Vous faites du crack!", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
        else    
...
function knockDoorAnim(home)
    ...
    if home then
        ...
        knockingDoor = false
        Citizen.Wait(1000)
        QBCore.Functions.Notify('Il semble que personne ne soit à la maison..', 'error', 3500)
    end
end
