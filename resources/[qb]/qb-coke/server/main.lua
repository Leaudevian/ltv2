            ...
            if cocaleaf ~= nil then
                if ItemList[Player.PlayerData.items[k].name] ~= nil then 
                    if Player.PlayerData.items[k].name == "cocaleaf" and Player.PlayerData.items[k].amount >= 2 then 
                        ...
                        TriggerClientEvent('QBCore:Notify', src, "Vous n\'avez pas assez de feuilles de coca", 'error')
                        break
                    end
                end
            else
                TriggerClientEvent('QBCore:Notify', src, "Vous n\'avez pas de feuille de coca", 'error')
                break
            end
        end
    end
...
AddEventHandler('qb-coke:server:processCrack', function()
    ...
    if Player.PlayerData.gang.name == "ballas" then
        if Player.PlayerData.items ~= nil then 
            ...
            TriggerClientEvent('QBCore:Notify', src, "Vous n\'avez rien...", "error")
        end
    else
        TriggerClientEvent('QBCore:Notify', src, "Vous pourriez avoir besoin de parler à un membre de gangs...", 'error')   
            ...
            if cokebaggy ~= nil then
                if DrugList[Player.PlayerData.items[k].name] ~= nil then 
                    if Player.PlayerData.items[k].name == "cokebaggy" and Player.PlayerData.items[k].amount >= 1 then 
                        ...
                        TriggerClientEvent('chatMessage', source, "Dealer Jacquies", "normal", 'Yo '..Player.PlayerData.firstname..', putain tu as eu '..Player.PlayerData.items[k].amount..'sacs de coke')
                        TriggerClientEvent('chatMessage', source, "Dealer Jacquies", "normal", 'Mal acheter tout cela pour '..amount..'$')
                        ...
                        TriggerClientEvent('QBCore:Notify', src, "Vous n\'avez pas de coke", 'error')
                        break
                    end
                end
            else
