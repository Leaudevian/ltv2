        ...
        if isLoggedIn and not keyPressed and GetLastInputMethod(0) then
            ...
            if next(keyMeta) ~= nil then
                if keyMeta[v[2]]["command"] ~= "" then
                    ...
                    QBCore.Functions.Notify('Il n\'y a toujours rien ['..v[2]..'] touche, /binds pour lier la commande', 'primary', 4000)
                end
            else
                QBCore.Functions.Notify('Vous n\'avez pas lié de commande, /binds pour lier la commande', 'primary', 4000)
            end
        end
    end, false)
...
RegisterNUICallback('save', function(data)
    ...
    QBCore.Functions.Notify('Config des touches sauvegardée!', 'success')
