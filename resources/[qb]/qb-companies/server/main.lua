            ...
            if companyName ~= "" then
                if IsNameAvailable(companyName) then
                    if cashAmount >= Config.CompanyPrice then
                        ...
                        TriggerClientEvent("qb-companies:client:setCompanies", -1, Config.Companies)
                        TriggerClientEvent('QBCore:Notify', src, "Félicitation, Vous êtes le propriétaire de : "..companyLabel)
                    else
                    ...
                    TriggerClientEvent('QBCore:Notify', src, 'La société : ' .. companyLabel .. ' est occupé..', 'error', 4000)
                end
            else
                TriggerClientEvent('QBCore:Notify', src, 'Le nom ne peut pas être vide..', 'error', 4000)
            end
        else
            TriggerClientEvent('QBCore:Notify', src, 'Le nom ne peut pas être vide', 'error', 4000)
        end
...
AddEventHandler('qb-companies:server:removeCompany', function(companyName)
    ...
    if companyName ~= nil then 
        if IsCompanyOwner(companyName, Player.PlayerData.citizenid) then
            ...
            TriggerClientEvent("qb-phone:client:InPhoneNotify", src, "Businesses", "success", "Tu as supprimé " .. companyLabel .. " !")
        else
            TriggerClientEvent('QBCore:Notify', src, 'Vous ne possédez pas cette entreprise ..', 'error', 4000)
        end
    else
        TriggerClientEvent('QBCore:Notify', src, 'Le nom ne peut pas être vide ..', 'error', 4000)
    end
...
AddEventHandler('qb-companies:server:quitCompany', function(companyName)
    ...
    if companyName ~= nil then 
        if IsEmployee(companyName, Player.PlayerData.citizenid) then
            ...
            TriggerClientEvent("qb-companies:client:setCompanies", -1, Config.Companies)
            TriggerClientEvent("qb-phone:client:setupCompanies", src)
            TriggerClientEvent("qb-phone:client:InPhoneNotify", src, "Businesses", "success", "Vous avez démissionné de" .. companyLabel .. "!")
    ...
    if OtherPlayer ~= nil then 
        if IsCompanyOwner(companyName, Player.PlayerData.citizenid) then
            if rank > 0 and rank < Config.MaxRank then
                ...
                TriggerClientEvent("qb-companies:client:setCompanies", -1, Config.Companies)
                TriggerClientEvent('QBCore:Notify', src, 'Tu as embauché ' .. OtherPlayer.PlayerData.firstname .. ' ' .. OtherPlayer.PlayerData.lastname .. ' (rang: '..rank..')')
                TriggerClientEvent('QBCore:Notify', OtherPlayer.PlayerData.source, 'Vous avez été embauché chez ' .. Config.Companies[companyName].label .. "(rang: " ..rank..")")
            else
        ...
        TriggerClientEvent('QBCore:Notify', src, 'La personne n\'est pas présente ...', 'error', 4000)
    end
end)
        ...
        if Config.Companies[companyName].employees[otherCitizenId] ~= nil then 
            if isPromote then
                ...
                if newRank < Config.MaxRank then
                    ...
                    TriggerClientEvent('QBCore:Notify', src, 'La personne ne peut pas être promu ..', 'error', 4000)
                end
            else
                local newRank = Config.Companies[companyName].employees[otherCitizenId].rank - 1
                if newRank > 0 then
            ...
            TriggerClientEvent('QBCore:Notify', src, 'Cette personne n\'est pas un employé de votre entreprise ..', 'error', 4000)
        end
    else
        TriggerClientEvent('QBCore:Notify', src, 'Vous ne possédez pas cette entreprise ..', 'error', 4000)
    end
