...
AddEventHandler('playerDropped', function()
	...
	if QBCore.Players[src] then
		...
		TriggerEvent('qb-log:server:CreateLog', 'joinleave', 'Déconnection', 'red', '**'.. GetPlayerName(src) .. '** ('..Player.PlayerData.license..') a quitté la ville..')
			...
			if Player then
				...
				if isGod or hasPerm or isPrincipal then
					if (QBCore.Commands.List[command].argsrequired and #QBCore.Commands.List[command].arguments ~= 0 and args[#QBCore.Commands.List[command].arguments] == nil) then
					    TriggerClientEvent('QBCore:Notify', src, 'Tous les arguments doivent être remplis!', 'error')
					else
						QBCore.Commands.List[command].callback(src, args)
					end
				else
					TriggerClientEvent('QBCore:Notify', src, 'Vous n\'avez pas accès à cette commande', 'error')
				end
			end
		end
	end
...
local function OnPlayerConnecting(name, setKickReason, deferrals)
    ...
    deferrals.update(string.format('Bonjour %s. Validation de votre licence Rockstar', name))
    ...
    deferrals.update(string.format('Bonjour %s. Nous vérifions si vous êtes bannis.', name))
    ...
    deferrals.update(string.format('Bienvenue %s à {Server Name}.', name))

    if not license then
        deferrals.done('Aucune licence de rockstar valide trouvée')
    elseif isBanned then
	    deferrals.done(Reason)
    elseif isLicenseAlreadyInUse then
        deferrals.done('Licence rockstar dupliquée')
    else
        deferrals.done()
        Wait(1000)
...
RegisterNetEvent('QBCore:server:CloseServer', function(reason)
    ...
    if QBCore.Functions.HasPermission(src, 'admin') or QBCore.Functions.HasPermission(src, 'god') then
        local reason = reason or 'Aucune raison spécifiée'
        QBCore.Config.Server.closed = true
        QBCore.Config.Server.closedReason = reason
        TriggerClientEvent('qbadmin:client:SetServerStatus', -1, true)
	else
		QBCore.Functions.Kick(src, 'Vous n\'avez pas la permission..', nil, nil)
    end
end)
...
RegisterNetEvent('QBCore:ToggleDuty', function()
	...
	if Player.PlayerData.job.onduty then
		...
		TriggerClientEvent('QBCore:Notify', src, 'Vous avez fini votre service!')
	else
		Player.Functions.SetJobDuty(true)
		TriggerClientEvent('QBCore:Notify', src, 'Vous avez pris votre service!')
	end
