...
function PaycheckLoop()
    ...
    for i=1, #Players, 1 do
        ...
        if Player.PlayerData.job and Player.PlayerData.job.payment > 0 then
            ...
            TriggerClientEvent('QBCore:Notify', Players[i], 'Vous avez reçu votre chèque de paie de '..Player.PlayerData.job.payment.."$")
        end
    end
...
function QBCore.Functions.Kick(source, reason, setKickReason, deferrals)
	local src = source
	reason = '\n'..reason..'\n🔸 Consulter notre Discord pour plus d\'informations: '..QBCore.Config.Server.discord
...
function QBCore.Functions.IsPlayerBanned(source)
    ...
    if result[1] then
        if os.time() < result[1].expire then
            ...
            message = 'Vous êtes bannis du serveur :\n'..result[1].reason..'\nIl expire dans '..timeTable.day.. '/' .. timeTable.month .. '/' .. timeTable.year .. ' ' .. timeTable.hour.. ':' .. timeTable.min .. '\n'
        else
