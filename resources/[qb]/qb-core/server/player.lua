...
function QBCore.Player.CheckPlayerData(source, PlayerData)
	...
	PlayerData.gang.label = PlayerData.gang.label or 'Aucun gang'
...
function QBCore.Player.CreatePlayer(PlayerData)
	...
	self.Functions.SetJob = function(job, grade)
		...
		if QBCore.Shared.Jobs[job] then
			...
			if QBCore.Shared.Jobs[job].grades[grade] then
				...
				self.PlayerData.job.grade = {}
				self.PlayerData.job.grade.name = 'Aucun grade'
	...
	self.Functions.SetGang = function(gang, grade)
		...
		if QBCore.Shared.Gangs[gang] then
			...
			if QBCore.Shared.Gangs[gang].grades[grade] then
				...
				self.PlayerData.gang.grade = {}
				self.PlayerData.gang.grade.name = 'Aucun grade'
	...
	self.Functions.AddMoney = function(moneytype, amount, reason)
		...
		if self.PlayerData.money[moneytype] then
			...
			if amount > 100000 then
				TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'AddMoney', 'lightgreen', '**'..GetPlayerName(self.PlayerData.source) .. ' (citizenid: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..')** '..amount .. '$ ('..moneytype..') ajouté, new '..moneytype..' balance: '..self.PlayerData.money[moneytype], true)
			else
				TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'AddMoney', 'lightgreen', '**'..GetPlayerName(self.PlayerData.source) .. ' (citizenid: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..')** '..amount .. '$ ('..moneytype..') ajouté, new '..moneytype..' balance: '..self.PlayerData.money[moneytype])
			end
	...
	self.Functions.RemoveMoney = function(moneytype, amount, reason)
		...
		if self.PlayerData.money[moneytype] then
			...
			self.Functions.UpdatePlayerData()
			if amount > 100000 then
				TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'RemoveMoney', 'red', '**'..GetPlayerName(self.PlayerData.source) .. ' (citizenid: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..')** '..amount .. '$ ('..moneytype..') retiré, new '..moneytype..' balance: '..self.PlayerData.money[moneytype], true)
			else
				TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'RemoveMoney', 'red', '**'..GetPlayerName(self.PlayerData.source) .. ' (citizenid: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..')** '..amount .. '$ ('..moneytype..') retiré, new '..moneytype..' balance: '..self.PlayerData.money[moneytype])
			end
	...
	self.Functions.AddItem = function(item, amount, slot, info)
		...
		if itemInfo == nil then TriggerClientEvent('QBCore:Notify', source, 'Item n\'existe pas', 'error') return end
		local amount = tonumber(amount)
		...
		if (totalWeight + (itemInfo['weight'] * amount)) <= QBCore.Config.Player.MaxWeight then
			if (slot and self.PlayerData.items[slot]) and (self.PlayerData.items[slot].name:lower() == item:lower()) and (itemInfo['type'] == 'item' and not itemInfo['unique']) then
				...
				self.Functions.UpdatePlayerData()
				TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'AddItem', 'green', '**'..GetPlayerName(self.PlayerData.source) .. ' (citizenid: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..')** a obtenu: [slot:' ..slot..'], itemname: ' .. self.PlayerData.items[slot].name .. ', quantité: ' .. amount ..', nouveau total: '.. self.PlayerData.items[slot].amount)
				return true
			...
			elseif (itemInfo['unique']) or (not slot or slot == nil) or (itemInfo['type'] == 'weapon') then
				for i = 1, QBConfig.Player.MaxInvSlots, 1 do
					if self.PlayerData.items[i] == nil then
						...
						self.Functions.UpdatePlayerData()
						TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'AddItem', 'green', '**'..GetPlayerName(self.PlayerData.source) .. ' (citizenid: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..')** a obtenu: [slot:' ..i..'], itemname: ' .. self.PlayerData.items[i].name .. ', quantité: ' .. amount ..', nouveau total: '.. self.PlayerData.items[i].amount)
			...
			TriggerClientEvent('QBCore:Notify', self.PlayerData.source, 'Votre inventaire est plein!', 'error')
		end
		return false
	end
	...
	self.Functions.RemoveItem = function(item, amount, slot)
		...
		if slot then
			if self.PlayerData.items[slot].amount > amount then
				self.PlayerData.items[slot].amount = self.PlayerData.items[slot].amount - amount
				self.Functions.UpdatePlayerData()
				TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**'..GetPlayerName(self.PlayerData.source) .. ' (citizenid: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..')** item retiré: [slot:' ..slot..'], itemname: ' .. self.PlayerData.items[slot].name .. ', quantité: ' .. amount ..', nouveau total: '.. self.PlayerData.items[slot].amount)
				return true
			else
				self.PlayerData.items[slot] = nil
				self.Functions.UpdatePlayerData()
				TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**'..GetPlayerName(self.PlayerData.source) .. ' (citizenid: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..')** item retiré: [slot:' ..slot..'], itemname: ' .. item .. ', quantité: ' .. amount ..', objet retiré')
				return true
			end
		else
	...
	self.Functions.SetInventory = function(items, dontUpdateChat)
		...
		TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'SetInventory', 'blue', '**'..GetPlayerName(self.PlayerData.source) .. ' (citizenid: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..')** objets obtenus : ' .. json.encode(items))
	end

	self.Functions.ClearInventory = function()
		self.PlayerData.items = {}
		self.Functions.UpdatePlayerData()
		TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'ClearInventory', 'red', '**'..GetPlayerName(self.PlayerData.source) .. ' (citizenid: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..')** Inventaire vidé')
	end
...
function QBCore.Player.Save(source)
	...
	if PlayerData then
		...
		QBCore.ShowSuccess(GetCurrentResourceName(), PlayerData.name ..' JOUEUR SAUVEGARDÉ!')
	else
		QBCore.ShowError(GetCurrentResourceName(), 'ERROR QBCORE.PLAYER.SAVE - DONNÉE DU JOUEUR VIDE!')
	end
end
...
function QBCore.Player.DeleteCharacter(source, citizenid)
	...
	if license == result then
		...
		TriggerEvent('qb-log:server:CreateLog', 'joinleave', 'Joueur supprimé', 'red', '**'.. GetPlayerName(src) .. '** '..license..' à supprimé **'..citizenid..'**..')
	else
		DropPlayer(src, 'You Have Been Kicked For Exploitation')
		TriggerEvent('qb-log:server:CreateLog', 'anticheat', 'Anti-Cheat', 'white', GetPlayerName(src)..' A été expulsé pour la suppression du personnage Exploit', false)
	end
end
