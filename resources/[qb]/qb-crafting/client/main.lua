	...
	while true do
		...
		if craftObject ~= 0 then
			...
			if #(pos - objectPos) < 1.5 then
				...
				DrawText3D(objectPos.x, objectPos.y, objectPos.z + 1.0, "~g~E~w~ - Fabriquer")
	...
	while true do
		...
		if distance < 10 then
			...
			if distance < 1.5 then
				DrawText3D(Config.AttachmentCrafting["location"].x, Config.AttachmentCrafting["location"].y, Config.AttachmentCrafting["location"].z, "~g~E~w~ - Fabriquer")
