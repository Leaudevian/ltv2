				...
				if dist < 15 then
					...
					if dist < 1.5 then
						if not Crypto.Exchange.RebootInfo.state then
							DrawText3Ds(Crypto.Exchange.coords, '~g~E~w~ - Lire la clé USB')
							...
							if IsControlJustPressed(0, 38) then
								QBCore.Functions.TriggerCallback('qb-crypto:server:HasSticky', function(HasItem)
									if HasItem then
										...
										QBCore.Functions.Notify('Vous n\'avez pas de Cryptostick ..', 'error')
									end
								end)
							end
						else
							DrawText3Ds(Crypto.Exchange.coords, 'Le système redémarre - '..Crypto.Exchange.RebootInfo.percentage..'%')
						end
					else
