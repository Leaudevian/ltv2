...
AddEventHandler('qb-diving:client:SetDivingLocation', function(DivingLocation)
    ...
    Citizen.CreateThread(function()
        ...
        AddTextComponentSubstringPlayerName('Zone de plongée')
                        ...
                        if CoralDistance <= 30 then
                            if not CoralLocation.PickedUp then
                                ...
                                if CoralDistance <= 1.5 then
                                    DrawText3D(CoralLocation.coords.x, CoralLocation.coords.y, CoralLocation.coords.z, '[E] Collecte de coraux')
                                    if IsControlJustPressed(0, 38) then
                                        -- loadAnimDict("pickup_object")
                                        local times = math.random(2, 5)
                                        CallCops()
                                        FreezeEntityPosition(Ped, true)
                                        QBCore.Functions.Progressbar("take_coral", "Collecte de coraux", times * 1000, false, true, {
...
AddEventHandler('qb-diving:server:CallCops', function(Coords, msg)
    ...
    BeginTextCommandSetBlipName('STRING')
    AddTextComponentString("911 - Site de plongée")
...
AddEventHandler('qb-diving:client:UseGear', function(bool)
    if bool then
        GearAnim()
        QBCore.Functions.Progressbar("equip_gear", "Mettre une tenue de plongée", 5000, false, true, {}, {}, {}, {}, function() -- Done
            DeleteGear()
            ...
            TriggerEvent('chatMessage', "SYSTEM", "error", "/divingsuit Enlever votre tenue de plongée")
        end)
    else
        if currentGear.enabled then
            ...
            QBCore.Functions.Progressbar("remove_gear", "Pull out a diving suit ..", 5000, false, true, {}, {}, {}, {}, function() -- Done
                ...
                ClearPedTasks(PlayerPedId())
                QBCore.Functions.Notify('Tu as enlevé ta tenue de plongée')
            end)
        else
            QBCore.Functions.Notify('Vous ne portez pas de tenue de plongée ..', 'error')
        end
    end
end)
