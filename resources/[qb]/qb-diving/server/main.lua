...
AddEventHandler('qb-diving:server:BuyBoat', function(boatModel, BerthId)
    ...
    elseif PlayerMoney.bank >= BoatPrice then
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Pas assez d\'argent, il vous manque '..missingMoney..'$', 'error')
    end
end)
...
AddEventHandler('qb-diving:server:SetBoatState', function(plate, state, boathouse, fuel)
    ...
    if result ~= nil then
        exports.oxmysql:insert('UPDATE player_boats SET state=@state, boathouse=@boathouse, fuel=@fuel WHERE plate=@plate AND citizenid=@citizenid', {
            ['@state'] = state,
            ['@boathouse'] = boathouse,
            ['@fuel'] = fuel,
            ['@plate'] = plate,
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
                local msg = "Ce corail peut être volé"
...
QBCore.Commands.Add("divingsuit", "Enlevez votre tenue de plongée", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
...
AddEventHandler('qb-diving:server:SellCoral', function()
    ...
    if HasCoral(src) then
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Vous n\'avez pas de corail à vendre..', 'error')
    end
end)
