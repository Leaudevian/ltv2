				...
				if QB.Doors[k].pickable then
					if QB.Doors[k].locked then
						if hasItem then
							...
							QBCore.Functions.Notify("Vous n\'avez pas de tournevis", "error")
						end
					else
						QBCore.Functions.Notify('La porte n\'est pas verrouillée', 'error', 2500)
					end
				else
					QBCore.Functions.Notify('Cette porte ne peut pas être forcée', 'error', 2500)
				end
			end
		end
    end, "screwdriverset")
...
function lockpickFinish(success)
    if success then
		QBCore.Functions.Notify('Succès!', 'success', 2500)
		setDoorLocking(closestDoorValue, closestDoorKey)
    else
        QBCore.Functions.Notify('Échoué', 'error', 2500)
    end
end
			...
			if distance < maxDistance then
				...
				if isAuthorized then
					if current.locked then
						displayText = "[~g~E~w~] - Fermé à clé"
					elseif not current.locked then
						displayText = "[~g~E~w~] - Ouvrir"
					end
				elseif not isAuthorized then
					if current.locked then
						displayText = "~r~Fermé à clé"
					elseif not current.locked then
						displayText = "~g~Ouvrir"
					end
				end

				if current.locking then
					if current.locked then
						displayText = "~g~Fermeture ..."
					else
						displayText = "~r~Ouverture ..."
					end
				end
				...
				if IsControlJustReleased(0, 38) then
					if isAuthorized then
						...
						QBCore.Functions.Notify('Pas autorisé', 'error')
					end
				end
			end
		end
