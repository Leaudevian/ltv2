        ...
        if CurrentCops >= Config.MinimumDrugSalePolice then
            if result ~= nil then
                ...
                if not cornerselling then
                    ...
                    QBCore.Functions.Notify('Vous avez commencé à vendre de la drogue')
                    startLocation = GetEntityCoords(PlayerPedId())
                    -- TaskStartScenarioInPlace(PlayerPedId(), "CODE_HUMAN_CROSS_ROAD_WAIT", 0, false)
                else
                    cornerselling = false
                    LocalPlayer.state:set("inv_busy", false, true)
                    QBCore.Functions.Notify('Vous avez cessé de vendre de la drogue')
                    -- ClearPedTasks(PlayerPedId())
                end
            else
                QBCore.Functions.Notify('Vous n\'avez pas de drogue avec vous..', 'error')
                LocalPlayer.state:set("inv_busy", false, true)
            end
        else
            QBCore.Functions.Notify("Il n\'y a pas assez de policier en service (".. Config.MinimumDrugSalePolice .." Requis)", "error")
        end
    end)
end)
...
function toFarAway()
    QBCore.Functions.Notify('Déplacé trop loin!', 'error')
    LocalPlayer.state:set("inv_busy", false, true)
    cornerselling = false
    hasTarget = false
    busySelling = false
...
AddEventHandler('qb-drugs:client:refreshAvailableDrugs', function(items)
    ...
    if #availableDrugs <= 0 then
        QBCore.Functions.Notify('Plus de drogue à vendre !', 'error')
        cornerselling = false
        LocalPlayer.state:set("inv_busy", false, true)
    end
end)
    ...
    if hasTarget then
        while pedDist < 1.5 do
            ...
            if getRobbed == 18 or getRobbed == 9 then
                ...
                QBCore.Functions.Notify('Vous avez été volé et perdu '..bagAmount..' sachet(\'s) '..availableDrugs[drugType].label, 'error')
                stealingPed = ped
                stealData = {
                ...
                if pedDist < 1.5 and cornerselling then
                    QBCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '~g~E~w~ '..bagAmount..'x '..currentOfferDrug.label..' pour '..randomPrice..'$ ? / ~g~G~w~ Décliner l\'offre')
                    ...
                    if IsControlJustPressed(0, 47) then
                        QBCore.Functions.Notify('Offre décliné!', 'error')
                        hasTarget = false
