                            ...
                            if IsControlJustPressed(0, 47) then
                                if dealer["name"] == "Mystery man" then
                                    ...
                                    if player ~= -1 and distance < 5.0 then
                                        ...
                                        QBCore.Functions.Progressbar("hospital_revive", "Réanimmer la personne..", 5000, false, true, {
                                            ...
                                            QBCore.Functions.Notify("Vous avez aidé une personne!")
                                        ...
                                        QBCore.Functions.Notify("Il n\'y a personne à proximité..", "error")
                                    end
                                else
                                    if waitingDelivery == nil then
                                        TriggerEvent("chatMessage", "Dealer "..Config.Dealers[currentDealer]["name"], "Ce sont les produits, on rester en contact par email")
                                        requestDelivery()
                                        interacting = false
                                        dealerIsHome = false
                                    else
                                        TriggerEvent("chatMessage", "Dealer "..Config.Dealers[currentDealer]["name"], "error", 'Vous devez toujours compléter une livraison, qu\'attendez-vous?!')
                                    end
                                end
                            end
                        end
...
function knockDoorAnim(home)
    ...
    if home then
        ...
        if Config.Dealers[currentDealer]["name"] == "Mystery man" then
            TriggerEvent("chatMessage", "Dealer "..Config.Dealers[currentDealer]["name"], "normal", 'Bonjour mon enfant, que puis-je faire pour vous ?')
        elseif Config.Dealers[currentDealer]["name"] == "Fred" then
            dealerIsHome = false
            TriggerEvent("chatMessage", "Dealer "..Config.Dealers[currentDealer]["name"], "normal", 'Malheureusement, je ne fais plus en affaires ... tu aurais dû me traiter mieux !')
        else
            TriggerEvent("chatMessage", "Dealer "..Config.Dealers[currentDealer]["name"], "normal", 'Yo '..myData.charinfo.firstname..', j\'peux faire quoi pour toi man ?')
        end
        -- knockTimeout()
    else
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "knock_door", 0.2)
        ...
        knockingDoor = false
        Citizen.Wait(1000)
        QBCore.Functions.Notify('On dirait que personne n\'est à la maison..', 'error', 3500)
    end
end
...
function requestDelivery()
    ...
    SetTimeout(2000, function()
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            ...
            message = "Voici toutes les infos sur la livraison, <br>Objets: <br> "..amount.."x "..QBCore.Shared.Items[waitingDelivery["itemData"]["item"]]["label"].."<br><br> sois à l'heure!!",
            button = {
                enabled = true,
...
function setMapBlip(x, y)
    SetNewWaypoint(x, y)
    QBCore.Functions.Notify('La route vers le lieux de livraison a été définie sur votre GPS.', 'success');
end
...
AddEventHandler('qb-drugs:client:setLocation', function(locationData)
    if activeDelivery == nil then
        ...
        QBCore.Functions.Notify('Vous avez toujours une livraison en cours...')
        return
    end
...
function deliverStuff(activeDelivery)
    if deliveryTimeout > 0 then
        ...
        QBCore.Functions.Progressbar("work_dropbox", "Delivering products..", 3500, false, true, {
            ...
            QBCore.Functions.Notify("Annulée..", "error")
        end)
    else
...
AddEventHandler('qb-drugs:client:robberyCall', function(msg, streetLabel, coords)
    ...
    AddTextComponentString("911: Trafic de drogue")
...
AddEventHandler('qb-drugs:client:sendDeliveryMail', function(type, deliveryData)
    if type == 'perfect' then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            ...
            subject = "Livraison",
            message = "Vous avez fait du bon travail, j'espère vous revoir ;)<br><br>Cordialement, "..Config.Dealers[deliveryData["dealer"]]["name"]
        })
    elseif type == 'bad' then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = Config.Dealers[deliveryData["dealer"]]["name"],
            subject = "Livraison",
            message = "J'ai reçu des plaintes concernant votre livraison, ne laissez pas cela se reproduire..."
        })
    elseif type == 'late' then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = Config.Dealers[deliveryData["dealer"]]["name"],
            subject = "Livraison",
            message = "Vous n\'êtes pas à l'heure. Avez-vous eu des choses plus importantes à faire que des affaires ?"
        })
    end
end)
...
AddEventHandler('qb-drugs:client:GotoDealer', function(DealerData)
    ...
    QBCore.Functions.Notify('Vous avez été téléporté : '.. DealerData["name"] .. ' Bonne chance!', 'success')
end)
