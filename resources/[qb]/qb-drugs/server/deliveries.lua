...
AddEventHandler('qb-drugs:server:updateDealerItems', function(itemData, amount, dealer)
    ...
    if Config.Dealers[dealer]["products"][itemData.slot].amount - 1 >= 0 then
        ...
        TriggerClientEvent("QBCore:Notify", src, "Cet objet n'est pas disponible.. Vous avez été remboursé.", "error")
    end
end)
...
AddEventHandler('qb-drugs:server:succesDelivery', function(deliveryData, inTime)
    ...
    if inTime then
        if Player.Functions.GetItemByName('weed_brick') ~= nil and Player.Functions.GetItemByName('weed_brick').amount >=
            ...
            TriggerClientEvent('QBCore:Notify', src, 'La commande a été livrée complètement', 'success')
            ...
            TriggerClientEvent('QBCore:Notify', src, 'Cela ne répond pas à la commande...', 'error')
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Vous êtes arrivé trop tard...', 'error')
...
AddEventHandler('qb-drugs:server:callCops', function(streetLabel, coords)
    local msg = "Une situation suspecte a été détecté à " .. streetLabel .. ", éventuellement trafique de drogues."
    local alertData = {
        title = "Drug Dealing",
        coords = {
            x = coords.x,
...
QBCore.Commands.Add('newdealer', 'Placer un dealer (Admin)', {{
    name = "name",
    help = "Nom du dealer"
}, {
    name = "min",
...
QBCore.Commands.Add('deletedealer', 'Supprimer un dealer (Admin)', {{
    name = "name",
    help = "Nom du dealer"
}}, true, function(source, args)
    local dealerName = args[1]
    ...
    if result[1] ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', source, "Dealer: " .. dealerName .. " A été supprimé", "success")
    else
        TriggerClientEvent('QBCore:Notify', source, "Dealer: " .. dealerName .. " N\'existe pas", "error")
    end
end, 'admin')

QBCore.Commands.Add('dealers', 'Voir tous les dealers (Admin)', {}, false, function(source, args)
    local DealersText = ""
    if Config.Dealers ~= nil and next(Config.Dealers) ~= nil then
        for k, v in pairs(Config.Dealers) do
            DealersText = DealersText .. 'Nom: ' .. v["name"] .. '<br>'
        end
        TriggerClientEvent('chat:addMessage', source, {
            template = '<div class="chat-message advert"><div class="chat-message-body"><strong>Liste des dealers: </strong><br><br> '..DealersText..'</div></div>',
            args = {}
        })
    else
        TriggerClientEvent('QBCore:Notify', source, 'Aucun dealer n\'a été placé.', 'error')
    end
end, 'admin')

QBCore.Commands.Add('dealergoto', 'Téléporter à un dealer (Admin)', {{name = 'name', help = 'Nom du Dealer'}}, true, function(source, args)
    ...
    if Config.Dealers[DealerName] ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', source, 'Ce dealer n\'existe pas.', 'error')
    end
end, 'admin')
...
AddEventHandler('qb-drugs:server:CreateDealer', function(DealerData)
    ...
    if result[1] ~= nil then
        TriggerClientEvent('QBCore:Notify', src, "Un dealer existe déjà avec ce nom..", "error")
    else
