TryToFish = function()
    QBCore.Functions.TriggerCallback('qb-fishing:GetItemData', function(count)
        if IsPedSwimming(cachedData["ped"]) then return QBCore.Functions.Notify("Vous ne pouvez pas nager et pêcher en même temps.", "error") end 
        if IsPedInAnyVehicle(cachedData["ped"]) then return QBCore.Functions.Notify("Vous devez quitter votre véhicule pour commencer à pêcher.", "error") end 
        if count ~= nil then
            if count == 0 then
                QBCore.Functions.Notify("Vous avez besoin d'une canne à pêche et d'appâts pour pêcher.", "primary")
            else
                ...
                if waterValidated then
                    ...
                    QBCore.Functions.Notify("Vous devez viser l\'eau pour pêcher", "primary")
                end
            end
        end
...
CastBait = function(rodHandle, castLocation)
    ...
    if not HasFishingBait() then
        QBCore.Functions.Notify('Vous n\'avez pas d\'appât!', 'error')
    ...
    while not IsControlJustPressed(0, 47) do
        ...
        ShowHelpNotification("Jeter votre ligne en appuyant sur ~INPUT_DETONATE~")

        if GetGameTimer() - startedCasting > 5000 then
            QBCore.Functions.Notify("Vous devez jeter l'appât.", "primary")
    ...
    QBCore.Functions.Notify("Un poisson mord à l\'hameçon...", "success", "10000")
    ...
    if caughtFish then
        ...
        QBCore.Functions.Notify("Le poisson s\'est détaché.", "error")
    end
...
TryToCatchFish = function()
    ...
    while true do
        ...
        ShowHelpNotification("Appuyer sur ~INPUT_CONTEXT~ dans la zone verte.")
...
IsInWater = function()
    ...
    QBCore.Functions.Notify("Vérification de la zone de pêche...", "success", "3000")
