		...
		if DoesEntityExist(cachedData["storeOwner"]) then
			...
			if dstCheck < 3.0 then
				if JobBusy == true then
					sleepThread = 5
					local displayText = not IsEntityDead(cachedData["storeOwner"]) and "Appuyer sur ~INPUT_CONTEXT~ pour vendre vos poissons au propriétaire." or "Le propriétaire est mort, et ne peut donc pas parler."
				...
				elseif JobBusy == false then
					sleepThread = 5
					local displayText = not IsEntityDead(cachedData["storeOwner"]) and "Appuyer sur ~INPUT_CONTEXT~ pour commencer à travailler."
					if IsControlJustPressed(0, 38) then
						JobBusy = true
						CreateBlips()
						Citizen.Wait(5000)
