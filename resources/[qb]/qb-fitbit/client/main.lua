...
RegisterNUICallback('setFoodWarning', function(data)
    ...
    QBCore.Functions.Notify('Fitbit: Avertissement de la faim fixé à '..foodValue..'%')
end)
...
RegisterNUICallback('setThirstWarning', function(data)
    ...
    QBCore.Functions.Notify('Fitbit: Avertissement de la soif fixé à '..thirstValue..'%')
end)
                ...
                if hasItem then
                    ...
                    if PlayerData.metadata["fitbit"].food ~= nil then
                        if PlayerData.metadata["hunger"] < PlayerData.metadata["fitbit"].food then
                            TriggerEvent("chatMessage", "FITBIT ", "warning", "Votre faim est à "..round(PlayerData.metadata["hunger"], 2).."%")
                    ...
                    if PlayerData.metadata["fitbit"].thirst ~= nil then
                        if PlayerData.metadata["thirst"] < PlayerData.metadata["fitbit"].thirst  then
                            TriggerEvent("chatMessage", "FITBIT ", "warning", "Votre soif est à "..round(PlayerData.metadata["thirst"], 2).."%")
                            PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)
                        end
                    end
                end
