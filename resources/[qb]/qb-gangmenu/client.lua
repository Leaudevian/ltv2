...
-- MENU
local menu = MenuV:CreateMenu(false, 'Menu du boss', 'topright', 155, 0, 0, 'size-125', 'none', 'menuv', 'main')
local menu2 = MenuV:CreateMenu(false, 'Argent de la société', 'topright', 155, 0, 0, 'size-125', 'none', 'menuv', 'society')
local menu3 = MenuV:CreateMenu(false, 'Gestion des employés', 'topright', 155, 0, 0, 'size-125', 'none', 'menuv', 'employees')
local menu4 = MenuV:CreateMenu(false, 'Menu de recrutement', 'topright', 155, 0, 0, 'size-125', 'none', 'menuv', 'recruit')
...
local menu_button1 = menu:AddButton({
    ...
    label = 'Recruter',
    value = menu4,
    description = 'Embaucher de nouveaux membres'
})
...
local menu_button4 = menu:AddButton({
    ...
    value = menu2,
    description = 'Voir/gérer l\'argent de la société'
})
local menu_button5 = menu2:AddButton({
    icon = '💲',
    label = '',
    value = nil,
    description = 'Solde actuelle de la société'
})
...
menu_button6:On("select", function()
    local result = LocalInput('Montant de retrait', 255, '')
    if result ~= nil then
...
menu_button7:On("select", function()
    local result = LocalInput('Montant du dépôt', 255, '')
    if result ~= nil then
        ...
        if PlayerGang.name ~= nil then
            ...
            for k, v in pairs(Config.Gangs) do
                if k == PlayerGang.name and PlayerGang.isboss then
                    if #(pos - v) < 1.0 then
                        inRange = true
                        DrawText3D(v, "~g~E~w~ - Menu du boss")
...
function UpdateSociety()
    QBCore.Functions.TriggerCallback('qb-gangmenu:server:GetAccount', function(cb)
        menu_button5.Label = 'Solde de la société: $' ..comma_value(cb)
    end, PlayerGang.name)
end
...
function ManageEmployees(employee)
    ...
    buttons = {
        ...
        [3] = {
            ...
            label = "Viré",
            value = "Fire",
            description = "Viré " .. employee.name
        }
    }
        ...
        local menu_button9 = manageroptions:AddButton({
            ...
            select = function(btn)
                ...
                if values == 'promote' then
                    local result = LocalInput('Nouveau grade', 255, '')
                    if result ~= nil then
