...
AddEventHandler("qb-gangmenu:server:withdrawMoney", function(amount)
    ...
    TriggerEvent('qb-log:server:CreateLog', 'bossmenu', 'Retirer de l\'argent', "Retirait avec succès de" .. amount .. '$ (' .. gang .. ')', src)
end)
...
AddEventHandler("qb-gangmenu:server:depositMoney", function(amount)
    ...
    SaveResourceFile(GetCurrentResourceName(), "./accounts.json", json.encode(Accounts), -1)
    TriggerEvent('qb-log:server:CreateLog', 'bossmenu', 'Déposer de l\'argent', "Déposé avec succès " .. amount .. '$ (' .. gang .. ')', src)
end)
...
AddEventHandler('qb-gangmenu:server:updateGrade', function(target, grade)
    ...
    if Employee then
        if Employee.Functions.SetGang(Player.PlayerData.gang.name, grade) then
            TriggerClientEvent('QBCore:Notify', src, "Grade changé avec succès!", "success")
            TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source, "Votre grade de gang est maintenant [" ..grade.."].", "success")
        else
            TriggerClientEvent('QBCore:Notify', src, "Ce grade n'existe pas", "error")
        end
    else
        ...
        if player[1] ~= nil then
            ...
            exports.oxmysql:insert('UPDATE players SET gang=@gang WHERE citizenid=@citizenid', {['@gang'] = json.encode(employeegang), ['@citizenid'] = target})
            TriggerClientEvent('QBCore:Notify', src, "Grade changé avec succès!", "success")
        else
            TriggerClientEvent('QBCore:Notify', src, "Le joueur n\'existe pas", "error")
        end
    end
end)
...
AddEventHandler('qb-gangmenu:server:fireEmployee', function(target)
    ...
    if Employee then
        if Employee.Functions.SetGang("none", '0') then
            TriggerEvent('qb-log:server:CreateLog', 'bossmenu', 'Viré du gang', "Viré avec succès " .. GetPlayerName(Employee.PlayerData.source) .. ' (' .. Player.PlayerData.gang.name .. ')', src)
            TriggerClientEvent('QBCore:Notify', src, "Viré avec succès!", "success")
            TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source , "Vous avez été viré!", "error")
        else
            TriggerClientEvent('QBCore:Notify', src, "Contacter les développeurs du serveur", "error")
        end
    else
        local player = exports.oxmysql:fetchSync('SELECT * FROM players WHERE citizenid=@citizenid LIMIT 1', {['@citizenid'] = target})
        if player[1] ~= nil then
            ...
            gang.grade.name = nil
            gang.grade.level = 0
            exports.oxmysql:insert('UPDATE players SET gang=@gang WHERE citizenid=@citizenid', {['@gang'] = json.encode(gang), ['@citizenid'] = target})
            TriggerClientEvent('QBCore:Notify', src, "Viré avec succès !", "success")
            TriggerEvent('qb-log:server:CreateLog', 'bossmenu', 'Fire', "Viré avec succès " .. target.source .. ' (' .. Player.PlayerData.gang.name .. ')', src)
        else
            TriggerClientEvent('QBCore:Notify', src, "Le joueur n\'existe pas", "error")
        end
    end
...
AddEventHandler('qb-gangmenu:server:giveJob', function(recruit)
    ...
    if Target and Target.Functions.SetGang(Player.PlayerData.gang.name, 0) then
        TriggerClientEvent('QBCore:Notify', src, "Vous avez recruté " .. (Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname) .. " pour " .. Player.PlayerData.gang.label .. "", "success")
        TriggerClientEvent('QBCore:Notify', Target.PlayerData.source , "Vous avez été recruté pour " .. Player.PlayerData.gang.label .. "", "success")
        TriggerEvent('qb-log:server:CreateLog', 'bossmenu', 'Recruit', "Recruté avec succès " .. (Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname) .. ' (' .. Player.PlayerData.gang.name .. ')', src)
    end
end)
