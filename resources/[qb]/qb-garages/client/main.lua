        ...
        if OutsideVehicles[vehicle.plate] ~= nil then
            ...
            QBCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
                QBCore.Functions.TriggerCallback('qb-garage:server:GetVehicleProperties', function(properties)
                    ...
                    QBCore.Functions.Notify("Véhicule:Moteur " .. enginePercent .. "% Carosserie: " .. bodyPercent.. "% Carburant: "..currentFuel.. "%", "primary", 4500)
...
function MenuGarage()
    ...
    MenuTitle = "Garage"
    ClearMenu()
    Menu.addButton("Mes véhicules", "VehicleList", nil)
    Menu.addButton("Fermer le menu", "close", nil)
end
...
function GangMenuGarage()
    ...
    MenuTitle = "Garage"
    ClearMenu()
    Menu.addButton("Mes véhicules", "GangVehicleList", nil)
    Menu.addButton("Fermer le menu", "close", nil)
end
...
function MenuDepot()
    ...
    MenuTitle = "Impound"
    ClearMenu()
    Menu.addButton("Dépôt de véhicules", "DepotList", nil)
    Menu.addButton("Fermer le menu", "close", nil)
end
...
function MenuHouseGarage(house)
    ...
    ClearMenu()
    Menu.addButton("Mes véhicules", "HouseGarage", house)
    Menu.addButton("Fermer le menu", "close", nil)
end
...
function HouseGarage(house)
    QBCore.Functions.TriggerCallback("qb-garage:server:GetHouseVehicles", function(result)
        ped = PlayerPedId();
        MenuTitle = "Dépôt de véhicules :"
        ClearMenu()

        if result == nil then
            QBCore.Functions.Notify("Vous n\'avez pas de véhicules dans ce garage", "error", 5000)
            closeMenuFull()
        else
            ...
            for k, v in pairs(result) do
                ...
                Menu.addButton(QBCore.Shared.Vehicles[v.vehicle]["name"], "TakeOutGarageVehicle", v, v.state, " Moteur: " .. enginePercent.."%", " Carosserie: " .. bodyPercent.."%", " Carburant: "..currentFuel.."%")
            end
        end
...
function DepotList()
    QBCore.Functions.TriggerCallback("qb-garage:server:GetDepotVehicles", function(result)
        ...
        if result == nil then
            QBCore.Functions.Notify("Il n\'y a pas de véhicules dans la fourrière", "error", 5000)
            closeMenuFull()
        else
            ...
            for k, v in pairs(result) do
                ...
                Menu.addButton(QBCore.Shared.Vehicles[v.vehicle]["name"], "TakeOutDepotVehicle", v, v.state .. " ($"..v.depotprice..",-)", " Moteur: " .. enginePercent.."%", " Carosserie: " .. bodyPercent.."%", " Carburant: "..currentFuel.."%")
            end
        end
    ...
    QBCore.Functions.TriggerCallback("qb-garage:server:GetUserVehicles", function(result)
        ...
        if result == nil then
            ...
            for k, v in pairs(result) do
                ...
                Menu.addButton(QBCore.Shared.Vehicles[v.vehicle]["name"], "TakeOutVehicle", v, v.state, " Moteur: " .. enginePercent .. "%", " Carosserie: " .. bodyPercent.. "%", " Carburant: "..currentFuel.. "%")
            end
        end
    ...
    QBCore.Functions.TriggerCallback("qb-garage:server:GetUserVehicles", function(result)
        ...
        if result == nil then
            ...
            for k, v in pairs(result) do
                ...
                Menu.addButton(QBCore.Shared.Vehicles[v.vehicle]["name"], "TakeOutGangVehicle", v, v.state, " Moteur: " .. enginePercent .. "%", " Carosserie: " .. bodyPercent.. "%", " Carburant: "..currentFuel.. "%")
            end
        end
...
function TakeOutVehicle(vehicle)
    ...
    elseif vehicle.state == "Out" then
        QBCore.Functions.Notify("Ce véhicule est dans le dépôt", "error", 2500)
    elseif vehicle.state == "Impound" then
        QBCore.Functions.Notify("Ce véhicule a été mis en fourrière par la police", "error", 4000)
    end
end
    ...
    while true do
        ...
        for k, v in pairs(Garages) do
            ...
            if putDist <= 25 and IsPedInAnyVehicle(ped) then
                ...
                if putDist <= 1.5 then
                    DrawText3Ds(Garages[k].putVehicle.x, Garages[k].putVehicle.y, Garages[k].putVehicle.z + 0.5, '~g~E~w~ - Se garé')
                    if IsControlJustPressed(0, 38) then
                        ...
                        QBCore.Functions.TriggerCallback('qb-garage:server:checkVehicleOwner', function(owned)
                            if owned then
                                ...
                                QBCore.Functions.Notify("Véhicule garé dans, "..Garages[k].label, "primary", 4500)
                            else
                                QBCore.Functions.Notify("Personne ne possède ce véhicule", "error", 3500)
                            end
                        end, plate)
                    end
                end
         ...
         for k, v in pairs(GangGarages) do
            ...
            if PlayerGang.name == GangGarages[k].job then
                ...
                if putDist <= 25 and IsPedInAnyVehicle(ped) then
                    ...
                    if putDist <= 1.5 then
                        DrawText3Ds(GangGarages[Name].putVehicle.x, GangGarages[Name].putVehicle.y, GangGarages[Name].putVehicle.z + 0.5, '~g~E~w~ - Se garé')
                        if IsControlJustPressed(0, 38) then
                            local curVeh = GetVehiclePedIsIn(ped)
                            local plate = GetVehicleNumberPlateText(curVeh)
                            QBCore.Functions.TriggerCallback('qb-garage:server:checkVehicleOwner', function(owned)
                                if owned then
                                    ...
                                    if DoesEntityExist(curVeh) then
                                        QBCore.Functions.Notify("Veuillez vérifier qu'il n'y ai personne à l'intérieur de la voiture.", "error", 4500)
                                    else
                                    ...
                                    QBCore.Functions.Notify("Véhicule garé dans, "..GangGarages[Name].label, "primary", 4500)
                                end
                                else
                                    QBCore.Functions.Notify("Personne ne possède ce véhicule", "error", 3500)
                                end
                ...
                if takeDist <= 15 then
                    ...
                    if takeDist < 2.0 then
                        ...
                        elseif IsPedInAnyVehicle(ped) then
                            DrawText3Ds(HouseGarages[currentHouseGarage].takeVehicle.x, HouseGarages[currentHouseGarage].takeVehicle.y, HouseGarages[currentHouseGarage].takeVehicle.z + 0.5, '~g~E~w~ - Se garé')
                            if IsControlJustPressed(0, 38) then
                                local curVeh = GetVehiclePedIsIn(ped)
                                local plate = GetVehicleNumberPlateText(curVeh)
                                QBCore.Functions.TriggerCallback('qb-garage:server:checkVehicleHouseOwner', function(owned)
                                    if owned then
                                        ...
                                        QBCore.Functions.Notify("Véhicule garé dans, "..HouseGarages[currentHouseGarage], "primary", 4500)
                                    end
                                    else
                                        QBCore.Functions.Notify("Personne ne possède ce véhicule", "error", 3500)
                                    end
