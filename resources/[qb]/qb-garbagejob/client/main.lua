        ...
        while Earnings > 0 do
            ...
            if distance < 20 then
                ...
                if distance < 1.5 then
                    DrawText3D(coords.x, coords.y, coords.z, "~g~E~w~ - Obtenir son salaire")
                    if IsControlJustPressed(0, 38) then
                        TriggerServerEvent('qb-garbagejob:server:PayShit', Earnings, location)
                        Earnings = 0
                    end
                elseif distance < 5 then
                    DrawText3D(coords.x, coords.y, coords.z, "Bureau de paie")
                end
            end
        ...
        if isLoggedIn then
            if PlayerJob.name == "garbage" then
                if distance < 10.0 then
                    ...
                    if distance < 1.5 then
                        if InVehicle then
                            DrawText3D(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z, "~g~E~w~ - Garage de camion à ordures")
                            if IsControlJustReleased(0, 38) then
                                QBCore.Functions.TriggerCallback('qb-garbagejob:server:CheckBail', function(DidBail)
                                    if DidBail then
                                        BringBackCar()
                                        QBCore.Functions.Notify("Vous avez 250$, - Dépôt remboursé!")
                                    else
                                        QBCore.Functions.Notify("Vous n\'avez aucun dépôt payé sur ce véhicule..")
                                    end
                                end)
                            end
                        else
                            DrawText3D(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z, "~g~E~w~ - Camion à ordures")
                            if IsControlJustReleased(0, 38) then
                                QBCore.Functions.TriggerCallback('qb-garbagejob:server:HasMoney', function(HasMoney)
                                    if HasMoney then
                                        ...
                                        QBCore.Functions.SpawnVehicle("trash2", function(veh)
                                            ...
                                            IsWorking = true
                                            SetGarbageRoute()
                                            QBCore.Functions.Notify("Vous avez 250$, - Dépôt payé!")
                                            QBCore.Functions.Notify("Vous avez commencé à travailler, emplacement marqué sur votre GPS!")
                                        end, coords, true)
                                    else
                                        QBCore.Functions.Notify("Vous n\'avez pas assez d'argent pour le dépôt.. Les coûts de dépôt est de 1000$,-")
                                    end
                                end)
                            end
                        end
                        ...
                        if DeliveryBlip ~= nil then
                            ...
                            if Distance < 20 or hasGarbageBag then
                                ...
                                if not hasGarbageBag then
                                    if CanTakeBag then
                                        if Distance < 1.5 then
                                            DrawText3D2(DeliveryData.coords, "~g~E~w~ - Attraper un sac à ordures")
                                        ...
                                        elseif Distance < 10 then
                                            DrawText3D2(DeliveryData.coords, "Ici pour attraper un sac à ordures.")
                                        end
                                    end
                                else
                                    if DoesEntityExist(GarbageVehicle) then
                                        if Distance < 10 then
                                            DrawText3D2(DeliveryData.coords, "Mettre le sac dans votre camion..")
                                        end
                                        ...
                                        if TruckDist < 2 then
                                            DrawText3D(Coords.x, Coords.y, Coords.z, "~g~E~w~ - Déposé le sac à ordures")
                                            if IsControlJustPressed(0, 51) then
                                                hasGarbageBag = false
                                                ...
                                                if (AmountOfBags - 1) == 0 then
                                                    ...
                                                    if (GarbageLocation + 1) <= AmountOfLocations then
                                                        ...
                                                        SetGarbageRoute()
                                                        QBCore.Functions.Notify("Tous les sacs à ordures sont récupérés, passez à l'emplacement suivant!")
                                                    else
                                                        -- Finished work
                                                        QBCore.Functions.Notify("Vous avez fini votre tourné! Retourner au dépôt.")
                                                        IsWorking = false
                                                        RemoveBlip(DeliveryBlip)
                                                        SetRouteBack()
                                                    end
                                                    ...
                                                    if AmountOfBags > 1 then
                                                        QBCore.Functions.Notify("Il y a toujours "..AmountOfBags.." sacs à gauche!")
                                                    else
                                                        QBCore.Functions.Notify("Il y a encore "..AmountOfBags.." sacs là-bas!")
                                                    end
                                                    hasGarbageBag = false
                                                end
                                                DeliverAnim()
                                        ...
                                        elseif TruckDist < 10 then
                                            DrawText3D(Coords.x, Coords.y, Coords.z, "Reste ici..")
                                        end
                                    else
                                        DrawText3D2(DeliveryData.coords, "Vous n\'avez pas de camion..")
                                    end
                                end
                            end
                        end
