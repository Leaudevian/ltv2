...
function UpdateBlip()
    Citizen.CreateThread(function()
        ...
        AddTextComponentSubstringPlayerName("Stand de hot-dog")
                ...
                if distance < 10 then
                    ...
                    if not IsWorking then
                        if distance < OffsetData.Distance then
                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "[E] Commencer à travailler")
                            if IsControlJustPressed(0, 38) then
                                StartWorking()
                            end
                        elseif distance < 3 then
                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "Commencer à travailler")
                        end
                    else
                        if distance < OffsetData.Distance then
                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "[E] Arrêter de travailler")
                            if IsControlJustPressed(0, 38) then
                                StopWorking()
                            end
                        elseif distance < 3 then
                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "Arrêter de travailler")
                        end
                    end
                end
            end
...
function StartWorking()
    QBCore.Functions.TriggerCallback('qb-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            ...
            UpdateUI()
            CheckLoop()
            QBCore.Functions.Notify('Vous avez déposé 250$ de caution !', 'success')
        else
            QBCore.Functions.Notify('Vous n\'avez pas d\'argent sur le dépôt..', 'error')
        end
    end)
end
                ...
                if ClosestObject ~= nil and ClosestObject == StandObject then
                    ...
                    if ObjectDistance < 1.0 then
                        if not IsPushing then
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[G] Saisir la bar')
                            ...
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[G] Lacher la bar')
                            if IsControlJustPressed(0, 47) then
                                LetKraamLose()
                            end
                        end
                    elseif ObjectDistance < 3.0 then
                        DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, 'Saisir la bar')
                    end
                end
            else
                break
        ...
        while true do
            ...
            if StandObject ~= nil then
                if ClosestObject ~= nil and ClosestObject == StandObject then
                    ...
                    if ObjectDistance < 1.0 then
                        if SellingData.Enabled then
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[E] Hot-dog préparé [Vente: ~g~Vendre~w~]')
                        else
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[E] Hot-dog préparé [Vente: ~r~Ne pas vendre~w~]')
                        end
                        if IsControlJustPressed(0, 38) then
                            StartHotdogMinigame()
                        end
...
function ToggleSell()
    ...
    if StandObject ~= nil then
        if dist < 5.0 then
            ...
            QBCore.Functions.Notify('Vous êtes trop loin de votre stand de Hot-Dog..', 'error')
        end
    else
        QBCore.Functions.Notify('Vous n\'avez pas de stand Hot-Dog', 'error')
    end
end
    ...
    while pedDist < OffsetData.Distance and SellingData.HasTarget do
        ...
        if PlayerDist < 4 then
            if SellingData.Hotdog ~= nil then
                ...
                QBCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '[7] Vendre '..HotdogsForSale..' Hot-Dog(\'s) pour '..(HotdogsForSale * SellingPrice)..'$ / [8] Refusé')
                if IsControlJustPressed(0, 161) or IsDisabledControlJustPressed(0, 161) then
                    QBCore.Functions.Notify(HotdogsForSale..'x Hot-Dog(\'s) vendu pour '..(HotdogsForSale * SellingPrice)..'$', 'success')
                ...
                if IsControlJustPressed(0, 162) or IsDisabledControlJustPressed(0, 162) then
                    QBCore.Functions.Notify('Vous avez refusé la vente !', 'error')
                    SellingData.HasTarget = false
...
function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        ...
        if Config.MyLevel == 1 then
            QBCore.Functions.Notify('Vous avez '..Config.Stock[Quality].Label..' Hot-Dog !')
            ...
            if Luck == LuckyNumber then
                QBCore.Functions.Notify('Vous avez réalisé '..LuckyAmount..' '..Config.Stock[Quality].Label..' Hot-Dog\'s !')
                Config.Stock[Quality].Current = Config.Stock[Quality].Current + LuckyAmount
            else
                QBCore.Functions.Notify('Vous avez réalisé '..Config.Stock[Quality].Label..' Hot-Dog !')
                Config.Stock[Quality].Current = Config.Stock[Quality].Current + 1
            end
        end
    else
        QBCore.Functions.Notify('Vous n\'avez pas ('..Config.Stock[Quality].Label..') plus à ce sujet devant le conseil..')
    end
    PreparingFood = false
end
    ...
    if DoesEntityExist(StandObject) then
        QBCore.Functions.TriggerCallback('qb-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                ...
                QBCore.Functions.Notify('Vous avez un dépôt de 250$ remboursé!', 'success')
            else
                QBCore.Functions.Notify('Quelque chose est mon ami..', 'error')
            end
        end)
    else
        QBCore.Functions.Notify('Votre stand de Hot-Dog n\'est pas là où je suis aveugle, vous ne recevrez pas votre dépôt!', 'error')
        IsWorking = false
        StandObject = nil
        IsPushing = false
        IsSelling = false
...
AddEventHandler('qb-hotdogjob:staff:DeletStand', function()
    ...
    if Object ~= nil then
        ...
        if ObjectDistance <= 5 then
            ...
            Citizen.Wait(100)            
            DeleteEntity(Object)
            QBCore.Functions.Notify('Stand de Hot-Dog rangé!')
        end
    end
end)
