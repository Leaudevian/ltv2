...
AddEventHandler('qb-houses:client:addGarage', function()
    if closesthouse ~= nil then 
        ...
        QBCore.Functions.Notify("Pas de maison à proximité..", "error")
    end
end)
    ...
    if dist < 1.5 then
        if hasKey then
            if Config.Houses[closesthouse].locked then
                ...
                QBCore.Functions.Notify("La maison est ouverte!", "success", 2500)
            else
                TriggerServerEvent('qb-houses:server:lockHouse', true, closesthouse)
                QBCore.Functions.Notify("La maison est fermée!", "error", 2500)
            end
        else
            QBCore.Functions.Notify("Vous n\'avez pas les clés de la maison...", "error", 3500)
        end
    else
        QBCore.Functions.Notify("Il n\'y a pas de porte à proximité", "error", 3500)
    end
end)
                ...
                if hasKey then
                    ...
                    if not inside then
                        if closesthouse ~= nil then
                            if #(pos - dist2) < 1.5 then
                                DrawText3Ds(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, '~b~/enter~w~ - Entrée')
                            end
                        end
                    end
                    ...
                    if CurrentDoorBell ~= 0 then
                        if #(pos - vector3(Config.Houses[closesthouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[closesthouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[closesthouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z)) < 1.5 then
                            DrawText3Ds(Config.Houses[closesthouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[closesthouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[closesthouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z + 0.35, '~g~G~w~ - Inviter')
                            ...
                            if POIOffsets ~= nil then
                                if POIOffsets.exit ~= nil then
                                    if #(pos - vector3(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z)) < 1.5 then
                                        DrawText3Ds(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z, '~g~E~w~ - Sortir')
                                        DrawText3Ds(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z - 0.1, '~g~H~w~ - Caméra')
                        ...
                        if closesthouse ~= nil then
                            if #(pos - vector3(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z)) < 1.5 then
                                if not viewCam and Config.Houses[closesthouse].locked then
                                    DrawText3Ds(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, '~g~E~w~ - Voir la maison')
                                    if IsControlJustPressed(0, 38) then -- E
                                        TriggerServerEvent('qb-houses:server:viewHouse', closesthouse)
                                    end
                                end
                    ...
                    if CurrentHouse ~= nil then
                        if stashLocation ~= nil then
                            if #(pos - vector3(stashLocation.x, stashLocation.y, stashLocation.z)) < 1.5 then
                                DrawText3Ds(stashLocation.x, stashLocation.y, stashLocation.z, '~g~E~w~ - Planque')
                                if IsControlJustPressed(0, 38) then -- E
                            ...
                            elseif #(pos - vector3(stashLocation.x, stashLocation.y, stashLocation.z)) < 3 then
                                DrawText3Ds(stashLocation.x, stashLocation.y, stashLocation.z, 'Planque')
                            end
                        end
                    end
                end
                    ...
                    if CurrentHouse ~= nil then
                        if outfitLocation ~= nil then
                            if #(pos - vector3(outfitLocation.x, outfitLocation.y, outfitLocation.z)) < 1.5 then
                                DrawText3Ds(outfitLocation.x, outfitLocation.y, outfitLocation.z, '~g~E~w~ - Tenues')
                                if IsControlJustPressed(0, 38) then -- E
                                    TriggerEvent('qb-clothing:client:openOutfitMenu')
                                end
                            elseif #(pos - vector3(outfitLocation.x, outfitLocation.y, outfitLocation.z)) < 3 then
                                DrawText3Ds(outfitLocation.x, outfitLocation.y, outfitLocation.z, 'Tenues')
                            end
                        end
                    end
                end
                    ...
                    if CurrentHouse ~= nil then
                        if logoutLocation ~= nil then
                            if #(pos - vector3(logoutLocation.x, logoutLocation.y, logoutLocation.z)) < 1.5 then
                                DrawText3Ds(logoutLocation.x, logoutLocation.y, logoutLocation.z, '~g~E~w~ - Changer de joueur')
                                if IsControlJustPressed(0, 38) then -- E
                                    DoScreenFadeOut(250)
                            ...
                            elseif #(pos - vector3(logoutLocation.x, logoutLocation.y, logoutLocation.z)) < 3 then
                                DrawText3Ds(logoutLocation.x, logoutLocation.y, logoutLocation.z, 'Changer de joueur')
                            end
                        end
                    end
                end
...
AddEventHandler('qb-houses:client:giveHouseKey', function(data)
    ...
    if player ~= -1 and distance < 2.5 and closesthouse ~= nil then
        ...
        if housedist < 10 then
            ...
            QBCore.Functions.Notify("Vous n\'êtes pas assez proche de la porte..", "error")
        end
    elseif closesthouse == nil then
        QBCore.Functions.Notify("Il n\'y a pas de maison à proximité", "error")
    else
        QBCore.Functions.Notify("Rien autour!", "error")
    end
end)
        ...
        if housedist < 5 then
            QBCore.Functions.TriggerCallback('qb-houses:server:getHouseOwner', function(result)
                if QBCore.Functions.GetPlayerData().citizenid == result then
                    ...
                    QBCore.Functions.Notify("Vous n\'êtes pas propriétaire de la maison..", "error")
                end
            end, closesthouse)
        else
            QBCore.Functions.Notify("Vous n\'êtes pas assez proche de la porte..", "error")
...
function HouseKeysMenu()
    ...
    QBCore.Functions.TriggerCallback('qb-houses:server:getHouseKeyHolders', function(holders)
        ped = PlayerPedId();
        MenuTitle = "Porte-clés:"
        ClearMenu()
        if holders == nil or next(holders) == nil then
            QBCore.Functions.Notify("Aucune clés trouvé..", "error", 3500)
            closeMenuFull()
        else
...
function optionMenu(citizenData)
    ...
    ClearMenu()
    Menu.addButton("Retirer la clé", "removeHouseKey", citizenData) 
    Menu.addButton("Retour", "HouseKeysMenu",nil)
end
...
function removeOutfit(oData)
    ...
    QBCore.Functions.Notify(oData.outfitname.." est retiré", "success", 2500)
    closeMenuFull()
end
...
AddEventHandler('qb-houses:client:setLocation', function(data)
    ...
    if inside then
        if hasKey then
            ...
            QBCore.Functions.Notify('Vous ne possédez pas cette maison', 'error')
        end
    else    
        QBCore.Functions.Notify('Tu n\'es pas dans une maison', 'error')
    end
end)
                        ...
                        if not Config.Houses[closesthouse].IsRaming then
                            ...
                            Skillbar.Start({
                                ...
                                if RamsDone + 1 >= Config.RamsNeeded then
                                    TriggerServerEvent('qb-houses:server:lockHouse', false, closesthouse)
                                    QBCore.Functions.Notify('Ça a fonctionné la porte est maintenant ouverte.', 'success')
                                ...
                                QBCore.Functions.Notify('Il a échoué réessayer.', 'error')
                                DoRamAnimation(false)
                            end)
                            TriggerServerEvent('qb-houses:server:SetRamState', true, closesthouse)
                        else
                            QBCore.Functions.Notify('Quelqu\'un travaille déjà sur la porte..', 'error')
                        end
                    else
                        QBCore.Functions.Notify('19/5000 Cette maison est déjà ouverte..', 'error')
                    end
                else
                    QBCore.Functions.Notify('Vous n\'êtes pas près d\'une maison..', 'error')
                end
            else
                QBCore.Functions.Notify('Il n\'y a pas de force de police présente..', 'error')
            end
        end)
    else
        QBCore.Functions.Notify('Vous n\'êtes pas près d\'une maison..', 'error')
...
AddEventHandler('qb-houses:client:ResetHouse', function()
    ...
    if closesthouse ~= nil then
        ...
        if Config.Houses[closesthouse].IsRammed then
            ...
            TriggerServerEvent('qb-houses:server:lockHouse', true, closesthouse)
            RamsDone = 0
            QBCore.Functions.Notify('Tu as encore fermé la maison..', 'success')
        else
            QBCore.Functions.Notify('Cette porte n\'est pas cassée ..', 'error')
        end
    end
end)
