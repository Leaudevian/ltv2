                ...
                if dist < 1.0 then
                    ...
                    if dist < 0.6 then
                        ...
                        if not Config.Locations["takeables"][spot].isBusy and not Config.Locations["takeables"][spot].isDone then
                            DrawText3Ds(Config.Locations["takeables"][spot].x, Config.Locations["takeables"][spot].y,Config.Locations["takeables"][spot].z, '~g~E~w~ Prendre')
                                    ...
                                    if Config.Locations["thermite"].isDone then 
                                        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
                                            if hasItem then
                                                ...
                                                QBCore.Functions.Notify("Il vous manque un lockpick avancé", "error")
                                            end
                                        end, "advancedlockpick")
                                    else
                                        QBCore.Functions.Notify("La sécurité est toujours active..", "error")
                                    end
                                else
                                    QBCore.Functions.Notify("Pas assez de policiers", "error")
                                end
                            end
                        end
                    else
...
function lockpickDone(success)
    ...
    if success then
        ...
        if math.random(1, 100) <= 40 and IsWearingHandshoes() then
            TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
            QBCore.Functions.Notify("Vous avez déchiré votre gant..")
        end
...
function GrabItem(spot)
    ...
    QBCore.Functions.Progressbar("grab_ifruititem", "Déconnecter l'élément", 10000, false, true, {
        ...
        TriggerServerEvent('qb-ifruitstore:server:PoliceAlertMessage', 'Des gens essaient de voler des objets au magasin Ifruit', pos, true)
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
        TriggerServerEvent('qb-ifruitstore:server:setSpotState', "isBusy", false, spot)
...
AddEventHandler('qb-ifruitstore:client:PoliceAlertMessage', function(msg, coords, blip)
    if blip then
        ...
        AddTextComponentString("911 - Situation suspecte dans le magasin Ifruit")
...
RegisterNUICallback('thermitefailed', function()
    ...
    TriggerServerEvent('qb-ifruitstore:server:PoliceAlertMessage', 'Des gens essaient de voler des objets au magasin Ifruit', coords, true)
end)
    ...
    if PlayerJob.name == "police" then 
        local store = "iFruitStore"
            ...
            TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
                timeOut = 10000,
                alertTitle = "Tentative de vol au magasin Ifruit",
                coords = {
                    x = coords.x,
                    y = coords.y,
                    z = coords.z,
