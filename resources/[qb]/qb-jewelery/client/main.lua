                        ...
                        if dist < 30 then
                            ...
                            if dist < 0.6 then
                                if not Config.Locations[case]["isBusy"] and not Config.Locations[case]["isOpened"] then
                                    DrawText3Ds(Config.Locations[case]["coords"]["x"], Config.Locations[case]["coords"]["y"], Config.Locations[case]["coords"]["z"], '[E] Casser la vitrine')
                                        ...
                                        QBCore.Functions.TriggerCallback('qb-jewellery:server:getCops', function(cops)
                                            if cops >= Config.RequiredCops then
                                                if validWeapon() then
                                                    ...
                                                    QBCore.Functions.Notify('Votre arme n\'est pas assez forte..', 'error')
                                                end
                                            else
                                                QBCore.Functions.Notify('Pas assez de policiers ('.. Config.RequiredCops ..') Requis', 'error')
                                            end                
                                        end)
                                    end
                                end
                            ...
                            if storeDist < 2 then
                                if not firstAlarm then
                                    if validWeapon() then
                                        local data = {displayCode = '112', description = 'Activité suspecte', isImportant = 0, recipientList = {'police'}, length = '10000', infoM = 'fa-info-circle', info = 'Bijouterie Vangelico'}
...
function smashVitrine(k)
    ...
    elseif math.random(1, 100) <= 5 and IsWearingHandshoes() then
        TriggerServerEvent("evidence:server:CreateFingerDrop", plyCoords)
        QBCore.Functions.Notify("Vous avez laissé une empreinte digitale sur le verre", "error")
    end
    ...
    QBCore.Functions.Progressbar("smash_vitrine", "Stocking a display", Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
        ...
        local data = {displayCode = '211A', description = 'Robbery', isImportant = 1, recipientList = {'police'}, length = '10000', infoM = 'fa-info-circle', info = 'Bijouterie Vangelico'}
        local dispatchData = {dispatchData = data, caller = 'Alarm', coords = vector3(-633.9, -241.7, 38.1)}
        TriggerServerEvent('wf-alerts:svNotify', dispatchData)
        smashing = false
...
AddEventHandler('qb-jewellery:client:PoliceAlertMessage', function(title, coords, blip)
    if blip then
        ...
        AddTextComponentString("911 - Situation suspecte à la bijouterie")
...
Citizen.CreateThread(function()
    ...
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentSubstringPlayerName("Bijouterie Vangelico")
