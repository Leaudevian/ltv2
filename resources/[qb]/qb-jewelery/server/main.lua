...
AddEventHandler('qb-jewellery:server:vitrineReward', function()
    ...
    if otherchance == odd then
        ...
        if Player.Functions.AddItem(Config.VitrineRewards[item]["item"], amount) then
            ...
            TriggerClientEvent('QBCore:Notify', src, 'Votre inventaire est plein..', 'error')
        end
    else
...
AddEventHandler('qb-jewellery:server:PoliceAlertMessage', function(title, coords, blip)
    ...
    local alertData = {
        ...
        description = "Robbéreux possible en cours sur le magasin de bijoux VANGELICO<br>Caméras disponible: 31, 32, 33, 34",
    }
