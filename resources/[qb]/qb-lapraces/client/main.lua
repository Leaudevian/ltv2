...
AddEventHandler('qb-lapraces:client:StartRaceEditor', function(RaceName)
    if not RaceData.InCreator then
        ...
        QBCore.Functions.Notify('Vous faites déjà une course.', 'error')
    end 
end)
        ...
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                ...
                if IsControlJustPressed(0, 162) or IsDisabledControlJustPressed(0, 162) then
                    if CreatorData.Checkpoints ~= nil and next(CreatorData.Checkpoints) ~= nil then
                        ...
                        QBCore.Functions.Notify('Vous n\'avez pas encore placé de CheckPoints..', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, 311) or IsDisabledControlJustPressed(0, 311) then
                    if CreatorData.Checkpoints ~= nil and #CreatorData.Checkpoints >= 2 then
                        ...
                        QBCore.Functions.Notify('Vous devez avoir au moins 10 CheckPoints', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, 40) or IsDisabledControlJustPressed(0, 40) then
                    if CreatorData.TireDistance + 1.0 ~= 16.0 then
                        ...
                        QBCore.Functions.Notify('Vous ne pouvez pas aller plus haut que 15')
                    end
                end
                ...
                if IsControlJustPressed(0, 39) or IsDisabledControlJustPressed(0, 39) then
                    if CreatorData.TireDistance - 1.0 ~= 1.0 then
                        ...
                        QBCore.Functions.Notify('Vous ne pouvez pas aller plus bas que 2')
                    end
                end
            else
                local coords = GetEntityCoords(PlayerPedId())
                DrawText3Ds(coords.x, coords.y, coords.z, 'Vous devez être dans un véhicule')
            end
            ...
            if IsControlJustPressed(0, 163) or IsDisabledControlJustPressed(0, 163) then
                if not CreatorData.ConfirmDelete then
                    ...
                    QBCore.Functions.Notify('Appuyez sur [9] à nouveau pour confirmer', 'error', 5000)
                else
                    ...
                    QBCore.Functions.Notify('Race-editor quitté!', 'error')
...
function SaveRace()
    ...
    QBCore.Functions.Notify('Course: '..CreatorData.RaceName..' est sauvegardé!', 'success')
...
function DeleteCheckpoint()
    ...
    if RaceData.ClosestCheckpoint ~= 0 then
        if CreatorData.Checkpoints[RaceData.ClosestCheckpoint] ~= nil then
            ...
            QBCore.Functions.Notify('Tu ne peux pas aller vite', 'error')
        end
    else
        QBCore.Functions.Notify('Tu ne peux pas aller vite', 'error')
    end
...
AddEventHandler('qb-lapraces:client:JoinRace', function(Data, Laps)
    if not RaceData.InRace then
        ...
        QBCore.Functions.Notify('Tu es déjà dans une course..', 'error')
    end
end)
...
AddEventHandler('qb-lapraces:client:RaceCountdown', function()
    ...
    if CurrentRaceData.RaceId ~= nil then
        while Countdown ~= 0 do
            if CurrentRaceData.RaceName ~= nil then
                if Countdown == 10 then
                    QBCore.Functions.Notify('La course commencera dans 10 secondes', 'error', 2500)
        ...
        QBCore.Functions.Notify('Vous n\'êtes pas actuellement dans une course..', 'error')
    end
end)
...
function FinishRace()
    ...
    if CurrentRaceData.BestLap ~= 0 then
        QBCore.Functions.Notify('Course terminée en '..SecondsToClock(CurrentRaceData.TotalTime)..', avec le meilleur tour en : '..SecondsToClock(CurrentRaceData.BestLap))
    else
        QBCore.Functions.Notify('Course terminée en '..SecondsToClock(CurrentRaceData.TotalTime))
    end
    for k, v in pairs(CurrentRaceData.Checkpoints) do
        if CurrentRaceData.Checkpoints[k].blip ~= nil then
            RemoveBlip(CurrentRaceData.Checkpoints[k].blip)
...
AddEventHandler('qb-lapraces:client:PlayerFinishs', function(RaceId, Place, FinisherData)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            QBCore.Functions.Notify(FinisherData.PlayerData.charinfo.firstname..' a finis à la place : '..Place, 'error', 3500)
        end
    end
end)
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            QBCore.Functions.Notify('Revenir au début ou vous serez explusé de la course: '..ToFarCountdown..'s', 'error', 500)
                        else
