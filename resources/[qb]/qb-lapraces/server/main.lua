...
AddEventHandler('qb-lapraces:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    ...
    if Races[RaceData.RaceId].Records ~= nil and next(Races[RaceData.RaceId].Records) ~= nil then
        if BLap < Races[RaceData.RaceId].Records.Time then
            ...
            exports.oxmysql:insert('UPDATE lapraces SET records=@records WHERE raceid=@raceid', {['@records'] = json.encode(Races[RaceData.RaceId].Records), ['@raceid'] = RaceData.RaceId})
            TriggerClientEvent('qb-phone:client:RaceNotify', src, 'Vous avez gagné la course '..RaceData.RaceName..' avec un temps de : '..SecondsToClock(BLap)..'!')
        end
    else
        Races[RaceData.RaceId].Records = {
            Time = BLap,
...
AddEventHandler('qb-lapraces:server:CreateLapRace', function(RaceName)
    ...
    if IsWhitelisted(Player.PlayerData.citizenid) then
        if IsNameAvailable(RaceName) then
            ...
            TriggerClientEvent('QBCore:Notify', source, 'Il y a déjà une course avec ce nom.', 'error')
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Vous n\'êtes pas autorisé à créer une course', 'error')
    end
end)
...
AddEventHandler('qb-lapraces:server:JoinRace', function(RaceData)
    ...
    if CurrentRace ~= nil then
        ...
        if (AmountOfRacers - 1) == 0 then
            ...
            TriggerClientEvent('QBCore:Notify', src, 'Vous étiez le seul dans la course, la course a été annulé.', 'error')
    ...
    if creatorsource ~= Player.PlayerData.source then
        TriggerClientEvent('qb-phone:client:RaceNotify', creatorsource, string.sub(Player.PlayerData.charinfo.firstname, 1, 1)..'. '..Player.PlayerData.charinfo.lastname..' a rejoint la course!')
    end
end)
...
AddEventHandler('qb-lapraces:server:LeaveRace', function(RaceData)
    ...
    local creatorsource = QBCore.Functions.GetPlayerByCitizenId(AvailableRaces[AvailableKey].SetupCitizenId).PlayerData.source
    if creatorsource ~= Player.PlayerData.source then
        TriggerClientEvent('qb-phone:client:RaceNotify', creatorsource, string.sub(Player.PlayerData.charinfo.firstname, 1, 1)..'. '..Player.PlayerData.charinfo.lastname..' a fini la course!')
    end
    local AmountOfRacers = 0
    for k, v in pairs(Races[RaceData.RaceId].Racers) do
        AmountOfRacers = AmountOfRacers + 1
    ...
    if Races[RaceId] ~= nil then
        if not Races[RaceId].Waiting then
            if not Races[RaceId].Started then
                ...
                TriggerClientEvent('QBCore:Notify', source, 'La course a déjà commencée', 'error')
            end
        else
            TriggerClientEvent('QBCore:Notify', source, 'La course a déjà commencée', 'error')
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'La course n\'existe pas :(', 'error')
    end
end)
...
AddEventHandler('qb-lapraces:server:StartRace', function(RaceId)
    ...
    if RaceId ~= nil then
        if AvailableRaces[AvailableKey].SetupCitizenId == MyPlayer.PlayerData.citizenid then
            ...
            TriggerClientEvent('QBCore:Notify', src, 'Vous n\'êtes pas le créateur de la course..', 'error')
        end
    else
...
QBCore.Commands.Add("togglesetup", "Activer/Désactiver les courses", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)

    if IsWhitelisted(Player.PlayerData.citizenid) then
        Config.RaceSetupAllowed = not Config.RaceSetupAllowed
        if not Config.RaceSetupAllowed then
            TriggerClientEvent('QBCore:Notify', source, 'Plus aucune autre courses ne peut être créé!', 'error')
        else
            TriggerClientEvent('QBCore:Notify', source, 'Les courses peuvent être créées à nouveau!', 'success')
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Vous n\'avez pas été autorisé à faire cela.', 'error')
    end
end)

QBCore.Commands.Add("cancelrace", "Annuler la course..", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    ...
    if IsWhitelisted(Player.PlayerData.citizenid) then
        ...
        if RaceName ~= nil then
            ...
            if Races[RaceId].Started then
                ...
                TriggerClientEvent('QBCore:Notify', source, 'Cette course n\'a pas encore commencé.', 'error')
            end
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Vous n\'avez pas été autorisé à faire cela.', 'error')
