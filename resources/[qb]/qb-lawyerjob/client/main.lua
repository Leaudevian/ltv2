...
AddEventHandler("qb-justice:client:showLawyerLicense", function(sourceId, data)
    ...
    if #(pos - sourcePos) < 2.0 then
        TriggerEvent('chat:addMessage', {
            template = '<div class="chat-message advert"><div class="chat-message-body"><strong>{0}:</strong><br><br> <strong>Pass-ID:</strong> {1} <br><strong>Prénom:</strong> {2} <br><strong>Nom de famille:</strong> {3} <br><strong>BSN:</strong> {4} </div></div>',
