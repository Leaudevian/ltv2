            ...
            if PlayerJob.name == "mechanic" then
                ...
                if onDuty then
                    if StashDistance < 20 then
                        ...
                        if StashDistance < 1 then
                            DrawText3Ds(Config.Locations["stash"].x, Config.Locations["stash"].y, Config.Locations["stash"].z, "[E] Ouvrir le Stock")
                ...
                if onDuty then
                    if VehicleDistance < 20 then
                        ...
                        if VehicleDistance < 1 then
                            ...
                            if InVehicle then
                                DrawText3Ds(Config.Locations["vehicle"].x, Config.Locations["vehicle"].y, Config.Locations["vehicle"].z, '[E] Ranger un véhicule de service')
                                if IsControlJustPressed(0, 38) then
                                    DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
                                end
                            else
                                DrawText3Ds(Config.Locations["vehicle"].x, Config.Locations["vehicle"].y, Config.Locations["vehicle"].z, '[E] Sortir un véhicule de service')
                                if IsControlJustPressed(0, 38) then
                                    if IsControlJustPressed(0, 38) then
                                        VehicleList()
                ...
                if OnDutyDistance < 20 then
                    ...
                    if OnDutyDistance < 1 then
                        if onDuty then
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "[E] Fin de journée")
                        else
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "[E] Démarrer la journée")
                        end
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent("QBCore:ToggleDuty")
                        end
                    ...
                    for k, v in pairs(Config.Plates) do
                        if v.AttachedVehicle == nil then
                                ...
                                if PlateDistance < 2 then
                                    ...
                                    if IsPedInAnyVehicle(PlayerPedId()) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z + 0.3, "[E] Placez le véhicule sur la plate-forme")
                                            if IsControlJustPressed(0, 38) then
                                                DoScreenFadeOut(150)
                                                Wait(150)
                                            ...
                                            QBCore.Functions.Notify("Vous ne pouvez pas mettre des vélos/motos sur la plate-forme!", "error")
                                        end
                                    end
                                end
                            end
                            ...
                            if PlateDistance < 3 then
                                inRange = true
                                DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "[E] Ouvrir le menu")
                                if IsControlJustPressed(0, 38) then
                                    OpenMenu()
                                    Menu.hidden = not Menu.hidden
                                end
...
function VehicleOptions()
    ClearMenu()
    Menu.addButton("Descendre le véhicule", "UnattachVehicle", nil)
    -- Menu.addButton("Check Status", "CheckStatus", nil)
    Menu.addButton("Pièces du véhicule", "PartsMenu", nil)
    Menu.addButton("Fermer le menu", "CloseMenu", nil)
end
...
function PartsMenu()
    ...
    Menu.addButton("Retour", "VehicleOptions", nil) 
    Menu.addButton("Fermer le menu", "CloseMenu", nil) 
end
...
function PartMenu(part)
    ClearMenu()
    Menu.addButton("Réparer ("..QBCore.Shared.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x)", "RepairPart", part)
    Menu.addButton("Retour", "VehicleOptions", nil)
    Menu.addButton("Fermer le menu", "CloseMenu", nil) 
end
...
function NoDamage(part)
    ClearMenu()
    Menu.addButton("Il n'y a aucuns dommage sur cette pièce !", "PartsMenu", part)
    Menu.addButton("Retour", "VehicleOptions", nil)
    Menu.addButton("Fermer le menu", "CloseMenu", nil) 
end
...
function RepairPart(part)
    ...
    QBCore.Functions.TriggerCallback('qb-inventory:server:GetStashItems', function(StashItems)
        ...
        if hasitem and countitem >= PartData.costs then
            ...
            QBCore.Functions.Progressbar("repair_part", "Réparation " ..Config.ValuesLabels[part], math.random(5000, 10000), false, true, {
                ...
                QBCore.Functions.Notify("Réparation annulée", "error")
            end)
        else
            QBCore.Functions.Notify('Il n\'y a pas assez de matériau', 'error')
        end
    end, "mechanicstash")
end
...
AddEventHandler('qb-vehicletuning:client:RepaireeePart', function(part)
    ...
    QBCore.Functions.Notify(Config.ValuesLabels[part].." est réparé!")
end)
            ...
            if #(pos - vehpos) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then 
                        ...
                        QBCore.Functions.Notify("Statut inconnu", "error")
                    end
                else
                    QBCore.Functions.Notify("Pas un véhicule valide", "error")
                end
            else
                QBCore.Functions.Notify("Vous n\'êtes pas assez proche du véhicule", "error")
            end
        else
            QBCore.Functions.Notify("Vous devez être dans le véhicule", "error")
        end
    else
        QBCore.Functions.Notify("Vous devez être en dehors du véhicule", "error")
    end
end)
...
AddEventHandler('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
            ...
            QBCore.Functions.Notify("Vous n'êtes pas le conducteur ou vous êtes sur un vélo", "error")
        end
    else
                ...
                if #(pos - vehpos) < 5.0 then
                    if not IsThisModelABicycle(GetEntityModel(veh)) then
                        ...
                        if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                            ...
                            QBCore.Functions.Progressbar("repair_advanced", "Réparation du Véhicule", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                            ...
                            QBCore.Functions.Notify("Pas une partie valide", "error")
                        end
                    else
                        QBCore.Functions.Notify("Pas un véhicule valide", "error")
                    end
...
function SendStatusMessage(statusList)
    if statusList ~= nil then 
        TriggerEvent('chat:addMessage', {
            template = '<div class="chat-message normal"><div class="chat-message-body"><strong>{0}:</strong><br><br> <strong>'.. Config.ValuesLabels["engine"] ..' (moteur):</strong> {1} <br><strong>'.. Config.ValuesLabels["body"] ..' (carosserie):</strong> {2} <br><strong>'.. Config.ValuesLabels["radiator"] ..' (radiateur):</strong> {3} <br><strong>'.. Config.ValuesLabels["axle"] ..' (transmission):</strong> {4}<br><strong>'.. Config.ValuesLabels["brakes"] ..' (freins):</strong> {5}<br><strong>'.. Config.ValuesLabels["clutch"] ..' (embrayage):</strong> {6}<br><strong>'.. Config.ValuesLabels["fuel"] ..' (réservoir):</strong> {7}</div></div>',
