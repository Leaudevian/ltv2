        ...
        if InsideMethlab then
            if #(pos - vector3(Config.Locations["exit"].coords.x, Config.Locations["exit"].coords.y, Config.Locations["exit"].coords.z)) < 20 then
                ...
                if #(pos - vector3(Config.Locations["exit"].coords.x, Config.Locations["exit"].coords.y, Config.Locations["exit"].coords.z)) < 1 then
                    DrawText3Ds(Config.Locations["exit"].coords.x, Config.Locations["exit"].coords.y, Config.Locations["exit"].coords.z, '~g~E~w~ - Sortir du labo')
        ...
        if InsideMethlab then
            if #(pos - vector3(Config.Locations["break"].coords.x, Config.Locations["break"].coords.y, Config.Locations["break"].coords.z)) < 20 then
                ...
                if #(pos - vector3(Config.Locations["break"].coords.x, Config.Locations["break"].coords.y, Config.Locations["break"].coords.z)) < 1 then
                    DrawText3Ds(Config.Locations["break"].coords.x - 0.06, Config.Locations["break"].coords.y + 0.90, Config.Locations["break"].coords.z, '~g~G~w~ - Casser la méthamphétamine ')
                ...
                if  not machineStarted then
                    if not loadIngredients then
                        if #(pos - vector3(Config.Tasks["Furnace"].coords.x, Config.Tasks["Furnace"].coords.y, Config.Tasks["Furnace"].coords.z)) < 1 then
                            DrawText3Ds(Config.Tasks["Furnace"].coords.x, Config.Tasks["Furnace"].coords.y,  Config.Tasks["Furnace"].coords.z + 0.2, '[G] Charger les ingrédients')
                            if IsControlJustPressed(0, 47) then
                        ...
                        if not finishedMachine then
                            if #(pos - vector3(Config.Tasks["Furnace"].coords.x, Config.Tasks["Furnace"].coords.y, Config.Tasks["Furnace"].coords.z)) < 1 then
                                DrawText3Ds(Config.Tasks["Furnace"].coords.x, Config.Tasks["Furnace"].coords.y,  Config.Tasks["Furnace"].coords.z + 0.2, '[E] Démarrer la machine')
                                if IsControlJustPressed(0, 38) then
                                    StartMachine()
                                end
                            end
                            ...
                            if #(pos - vector3(Config.Tasks["Furnace"].coords.x, Config.Tasks["Furnace"].coords.y, Config.Tasks["Furnace"].coords.z)) < 1 then
                                DrawText3Ds(Config.Tasks["Furnace"].coords.x, Config.Tasks["Furnace"].coords.y,  Config.Tasks["Furnace"].coords.z + 0.2, '[E] Récupérer la méthamphétamine')
                                if IsControlJustPressed(0, 38) then
...
function BreakMinigame()
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            ...
            QBCore.Functions.Notify("Échec", "error")
            ClearPedTasks(PlayerPedId())
    end)
end

function breakingMeth(amount)
    QBCore.Functions.Progressbar("break_meth", "Casser la méthamphétamine ..", Config.BreakMethTimer, false, true, {
...
function ProcessMinigame()
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            ...
            QBCore.Functions.Notify("Vous avez chargé les ingrédients!", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
    ...
    if ClosestMethlab == Config.CurrentLab then
        ...
        if dist < 1 then
            if labkey == ClosestMethlab then
                ...
                QBCore.Functions.Notify('Ce n\'est pas la bonne clé..', 'error')
            end
        end
    end
end)
            ...
            if MethDoor <= 6 then
                ...
                if MethDoor <= 1 then
                     if not interacting then
                        DrawText3Ds(321.72,-305.87,52.50, '~r~ Fermé à clé')
                            ...
                            if dist < 1 then
                                 if labkey == ClosestMethlab then
                                    EnterMethlab()
                                     ...
                                     QBCore.Functions.Notify('On dirait qu\'il faut une clé...', 'error')
                                 end
                            end
                        end
                     end
