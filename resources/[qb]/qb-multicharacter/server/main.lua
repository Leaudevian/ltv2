...
AddEventHandler('qb-multicharacter:server:disconnect', function()
    ...
    DropPlayer(src, "Vous êtes déconnecté")
end)
...
AddEventHandler('qb-multicharacter:server:loadUserData', function(cData)
    ...
    if QBCore.Player.Login(src, cData.citizenid) then
        print('^2[qb-core]^7 '..GetPlayerName(src)..' (Citizen ID: '..cData.citizenid..') a été chargé avec succès!')
        ...
        TriggerEvent("qb-log:server:CreateLog", "connexion", "Connexion", "green", "**".. GetPlayerName(src) .. "** ("..cData.citizenid.." | "..src..") a rejoint la ville..")
	end
end)
...
AddEventHandler('qb-multicharacter:server:createCharacter', function(data)
    ...
    if QBCore.Player.Login(src, false, newData) then
        print('^2[qb-core]^7 '..GetPlayerName(src)..' a rejoint avec succès!')
        QBCore.Commands.Refresh(src)
        loadHouseData()
...
QBCore.Commands.Add("logout", "Déconnecté mon joueur (Admin uniquement)", {}, false, function(source, args)
    QBCore.Player.Logout(source)
    TriggerClientEvent('qb-multicharacter:client:chooseChar', source)
end, "admin")

QBCore.Commands.Add("closeNUI", "Fermer Multi Nui", {}, false, function(source, args)
