...
function DoDropOff()
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if success then
			...
			Citizen.Wait(2000)
			QBCore.Functions.Notify('La livraison a été effectué, Votre GPS a été mis à jour pour la prochaine livraison', 'success')
		else
...
AddEventHandler("oxydelivery:client", function()
	...
	while tasking do
		...
		if dstcheck < 40.0 and not pedCreated and (oxyVehicle == veh or dstcheck2 < 15.0) then
			...
			DeleteCreatedPed()
			CreateOxyPed()
			QBCore.Functions.Notify('Vous êtes proche du point de livraison')
		end
		toolong = toolong - 1
		if toolong < 0 then
			...
			tasking = false
			OxyRun = false
			QBCore.Functions.Notify('Tu as pris trop de temps', 'error')
		end
	...
	while true do
		...
		if OxyRun then
			if not DoesEntityExist(oxyVehicle) or GetVehicleEngineHealth(oxyVehicle) < 200.0 or GetVehicleBodyHealth(oxyVehicle) < 200.0 then
				OxyRun = false
				tasking = false
				QBCore.Functions.Notify("Le concessionnaire ne vous donne plus d'emplacements en raison de l'état de sa voiture", "error")
			else
				if tasking then
					Citizen.Wait(30000)
				else
...
AddEventHandler("oxydelivery:startDealing", function()
	...
	firstdeal = true
	CreateOxyVehicle()
	QBCore.Functions.Notify('Une voiture a été trouvée. Votre GPS sera mis à jour son emplacement', 'success')
end)
