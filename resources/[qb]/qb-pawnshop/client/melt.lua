			...
			if #(pos - Config.MeltLocation) < 1.5 then
                if not Config.IsMelting then
                    if Config.CanTake then
                        DrawText3D(Config.MeltLocation.x, Config.MeltLocation.y, Config.MeltLocation.z, "~g~E~w~ - Récupérer des lingots d'or")
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent("qb-pawnshop:server:getGoldBars")
                        end
                    else
                        DrawText3D(Config.MeltLocation.x, Config.MeltLocation.y, Config.MeltLocation.z, "~g~E~w~ - Faire fondre des objets en or")
                        if IsControlJustReleased(0, 38) then 
                            local waitTime = math.random(10000, 15000)
                            ScrapAnim(1000)
                            QBCore.Functions.Progressbar("drop_golden_stuff", "Fonte en cours", 1000, false, true, {
                ...
                elseif Config.IsMelting and Config.MeltTime > 0 then
                    DrawText3D(Config.MeltLocation.x, Config.MeltLocation.y, Config.MeltLocation.z, "Fonte: " .. Config.MeltTime..'s')
                end
			end
		end
		if not inRange then
            ...
            if #(pos - Config.SellGold) < 1.5 then
                if GetClockHours() >= 9 and GetClockHours() <= 18 then
                    ...
                    elseif sellItemsSet and hasGold then
                        DrawText3D(Config.SellGold.x, Config.SellGold.y, Config.SellGold.z, "~g~E~w~ - Vendre des lingots d'or")
                        if IsControlJustReleased(0, 38) then
                            local lockpickTime = 20000
                            ScrapAnim(lockpickTime)
                            QBCore.Functions.Progressbar("sell_gold", "Vente d\'or", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                openingDoor = false
                                ClearPedTasks(PlayerPedId())
                                QBCore.Functions.Notify("Vente annulé", "error")
                            end)
                        end
                    else
                        DrawText3D(Config.SellGold.x, Config.SellGold.y, Config.SellGold.z, "Vous n\'avez pas d'or sur vous")
                    end
                    
                else
                    DrawText3D(Config.SellGold.x, Config.SellGold.y, Config.SellGold.z, "Pawnshop fermé")
                end
			end
		end
        if not inRange then
