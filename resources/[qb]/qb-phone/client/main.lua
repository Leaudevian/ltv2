...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen and isLoggedIn then
        ...
        if not IsHandcuffed and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] then
            ...
            QBCore.Functions.Notify("Action indisponible pour le moment ...", "error")
        end
    end
end)

RegisterKeyMapping('phone', 'Ouvrir son téléphone', 'keyboard', 'M')
...
function OpenPhone()
    QBCore.Functions.TriggerCallback('qb-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            QBCore.Functions.Notify("Vous n\'avez pas de téléphone", "error")
        end
    end)
end
...
RegisterNUICallback('SharedLocation', function(data)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Whatsapp",
            text = "L\'emplacement a été défini!",
        ...
        if PhoneData.isOpen then
            if SenderNumber ~= PhoneData.PlayerData.charinfo.phone then
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Whatsapp",
                        text = "Nouveau message de "..IsNumberInContacts(SenderNumber).."!",
                        icon = "fab fa-whatsapp",
                        color = "#25D366",
                        timeout = 1500,
                    },
                ...
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Whatsapp",
                        text = "Pourquoi envoyez-vous des messages à vous-même?",
                        icon = "fab fa-whatsapp",
                        color = "#25D366",
                        timeout = 4000,
                    },
...
AddEventHandler('qb-phone:client:NewMailNotify', function(MailData)
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Mail",
                text = "Vous avez un nouveau mail de "..MailData.sender,
...
AddEventHandler('qb-phone:client:UpdateAdverts', function(Adverts, LastAd)
    PhoneData.Adverts = Adverts
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Advertisement",
                text = "Une nouvelle annonce a été postée par "..LastAd,
                icon = "fas fa-ad",
                color = "#ff8f1a",
                timeout = 2500,
            },
...
AddEventHandler('qb-phone:client:BillingEmail', function(data, paid, name)
    if paid then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Service de facturation',
            subject = 'Facture payée',
            message = 'La facture a été payée par '..name..' Pour un montant de '..data.amount..'$',
        })
    else
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Service de facturation',
            subject = 'Facture refusée',
            message = 'La facture a été refusée par '..name..' Pour un montant de '..data.amount..'$',
        })
    end
end)
...
AddEventHandler('qb-phone:client:TransferMoney', function(amount, newmoney)
    PhoneData.PlayerData.money.bank = newmoney
        SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "QBank", text = "&#36;"..amount.." a été ajouté à votre compte!", icon = "fas fa-university", color = "#8c7ae6", }, })
...
AddEventHandler('qb-phone:client:UpdateTweets', function(src, Tweets, NewTweetData)
    ...
    if src ~= MyPlayerId then
            SendNUIMessage({
                action = "PhoneNotification",
                PhoneNotify = {
                    title = "Nouveau Tweet (@"..NewTweetData.firstName.." "..NewTweetData.lastName..")", 
...
AddEventHandler('qb-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
    Config.PhoneApplications["twitter"].Alerts = AppAlerts
        SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Vous avez été mentionné dans un tweet!", text = TweetMessage.message, icon = "fab fa-twitter", color = "#1DA1F2", }, })
...
function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        SendNUIMessage({ 
            ...
            PhoneNotify = { 
                title = "Phone", 
                text = "Vous n\'avez pas d'appel entrant...", 
                icon = "fas fa-phone", 
                color = "#e84118", 
            }, 
        })
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    QBCore.Functions.Notify('Localisation GPS: '..data.alert.title)
        ...
        if v.name == Name and v.number == Number then
            ...
            --if PhoneData.isOpen then
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Phone",
                        text = "Vous avez supprimé le contact!", 
...
AddEventHandler('qb-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    --if PhoneData.isOpen then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Phone",
                text = "Vous avez un nouveau contact suggéré!", 
                icon = "fa fa-phone-alt",
                color = "#04b543",
                timeout = 1500,
            },
...
AddEventHandler('qb-phone:client:RemoveBankMoney', function(amount)
    --if PhoneData.isOpen then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Bank",
                text = amount.."$ a été retiré de votre compte!", 
                icon = "fas fa-university", 
                color = "#ff002f",
                timeout = 3500,
            },
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            QBCore.Functions.Notify('Vous êtes trop loin de la course. Le GPS a été réglé sur la course.', 'error', 5000)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    QBCore.Functions.Notify("GPS a été réglé sur " .. data.HouseData.HouseData.adress .. "!", "success")
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    QBCore.Functions.Notify('GPS a été défini!', 'success')
end)
