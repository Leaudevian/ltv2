...
$(document).on('click', '.bank-app-account', function(e){
    ...
    QB.Phone.Notifications.Add("fas fa-university", "QBank", "Numéro de compte.copié!", "#badc58", 1750);
});
    ...
    if (iban != "" && amount != "") {
            $.post('https://qb-phone/CanTransferMoney', JSON.stringify({
                ...
                if (data.TransferedMoney) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-university", "QBank", "Vous avez transféré &#36; "+amount+"!", "#badc58", 1500);
                } else {
                    QB.Phone.Notifications.Add("fas fa-university", "QBank", "Vous n\'avez pas assez d\'argent!", "#badc58", 1500);
                }
                QB.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
            });
    } else {
        QB.Phone.Notifications.Add("fas fa-university", "QBank", "Remplissez tous les champs!", "#badc58", 1750);
    }
});
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('https://qb-phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                QB.Phone.Notifications.Add("fas fa-university", "QBank", "Vous avez payé &#36;"+InvoiceData.amount+"!", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        QB.Phone.Notifications.Add("fas fa-university", "QBank", "Il n'y a pas de compte bancaire attaché à ce numéro!", "#badc58", 2500);
    }
    QB.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});