SetupLawyers = function(data) {
    ...
    if (data.length > 0) {
        ...
        if (lawyers.length > 0) {
            ...
            var element = '<div class="lawyer-list"><div class="no-lawyers">Il n\'y a pas d\'avocats disponibles.</div></div>'
            $(".lawyers-list").append(element);
        }
        ...
        if (realestate.length > 0) {
            ...
            var element = '<div class="lawyer-list"><div class="no-lawyers">Il n\'y a pas d\'agents immobiliers disponibles.</div></div>'
            $(".lawyers-list").append(element);
        }
        ...
        if (mechanic.length > 0) {
            ...
            var element = '<div class="lawyer-list"><div class="no-lawyers">Il n\'y a pas de mécaniciens disponible.</div></div>'
            $(".lawyers-list").append(element);
        }
        ...
        if (taxi.length > 0) {
            ...
            var element = '<div class="lawyer-list"><div class="no-lawyers">Il n\'y a pas de taxis disponible.</div></div>'
            $(".lawyers-list").append(element);
        }
        ...
        if (police.length > 0) {
            ...
            var element = '<div class="lawyer-list"><div class="no-lawyers">Il n\'y a pas de policier disponible.</div></div>'
            $(".lawyers-list").append(element);
        }
        ...
        if (ambulance.length > 0) {
            ...
            var element = '<div class="lawyer-list"><div class="no-lawyers">Il n\'y a pas d\'ambulance disponible.</div></div>'
            $(".lawyers-list").append(element);
        }
    } else {
        $(".lawyers-list").append('<h1 style="font-size:1.641025641025641vh; padding:1.0256410256410255vh; color:#fff; margin-top:0; border-top-left-radius: .5vh; border-top-right-radius: .5vh; width:100%; display:block; background-color: rgb(42, 137, 214);">Lawyers (' + lawyers.length + ')</h1>');
                ...
                if (status.CanCall) {
                    if (!status.InCall) {
                        if (QB.Phone.Data.AnonymousCall) {
                            QB.Phone.Notifications.Add("fas fa-phone", "Phone", "Vous avez commencé un appel anonyme!");
                        }
                        ...
                        QB.Phone.Notifications.Add("fas fa-phone", "Phone", "Vous êtes déjà en ligne!");
                    }
                } else {
                    QB.Phone.Notifications.Add("fas fa-phone", "Phone", "Cette personne est déjà en ligne");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Phone", "Cette personne n'est pas disponible!");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-phone", "Phone", "Vous ne pouvez pas appeler votre propre numéro!");
        }
    });
});
