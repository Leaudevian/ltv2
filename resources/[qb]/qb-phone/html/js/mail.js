...
QB.Phone.Functions.SetupMails = function(Mails) {
    ...
    if (Mails !== null && Mails !== undefined) {
        if (Mails.length > 0) {
            ...
            $(".mail-list").html('<p class="nomails">Vous n\'avez pas de mails..</p>');
        }

    }
}

var MonthFormatting = ["Janvier", "Février", "Mars", "Avril", "Mai", "Juin", "Juillet", "Août", "Septembre", "Octobre", "Novembre", "Décembre"];
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        QB.Phone.Notifications.Add("fas fa-ad", "Advertisement", "Vous ne pouvez pas poster une annonce vide!", "#ff8f1a", 2000);
    }
});
...
QB.Phone.Functions.RefreshAdverts = function(Adverts) {
    ...
    if (Adverts.length > 0 || Adverts.length == undefined) {
        ...
        $(".advert-list").html("");
        var element = '<div class="advert"><span class="advert-sender">Il n\'y a pas encore d\'annonce!</span></div>';
        $(".advert-list").append(element);
    }
}