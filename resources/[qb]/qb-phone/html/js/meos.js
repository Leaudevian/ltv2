...
$(document).on('click', '.person-search-result', function(e){
    ...
    var OpenElement = '<div class="person-search-result-name">Nom: '+ClickedPersonData.firstname+' '+ClickedPersonData.lastname+'</div> <div class="person-search-result-bsn">CSN: '+ClickedPersonData.citizenid+'</div> <div class="person-opensplit"></div> &nbsp; <div class="person-search-result-dob">Date de naissance: '+ClickedPersonData.birthdate+'</div> <div class="person-search-result-number">Numéro de téléphone: '+ClickedPersonData.phone+'</div> <div class="person-search-result-nationality">Nationalité: '+ClickedPersonData.nationality+'</div> <div class="person-search-result-gender">Sexe: '+Gender+'</div> &nbsp; <div class="person-search-result-apartment"><span id="'+ClickedPersonId+'">Appartement: '+appartementData.label+'</span> <i class="fas fa-map-marker-alt appartment-adress-location" id="'+ClickedPersonId+'"></i></div> &nbsp; <div class="person-search-result-warned">Signalé: '+IsWarrant+'</div> <div class="person-search-result-driverslicense">Permis de conduire: '+HasLicense+'</div>';
    ...
    if (OpenedPerson === null) {
        ...
        var PreviousElement = '<div class="person-search-result-name">Nom: '+PreviousPersonData.firstname+' '+PreviousPersonData.lastname+'</div> <div class="person-search-result-bsn">CSN: '+PreviousPersonData.citizenid+'</div>';
...
$(document).on('click', '.person-search-result-apartment > span', function(e){
    ...
    QB.Phone.Notifications.Add("fas fa-university", "MDT", "Numéro de la maison copié!", "#badc58", 1750);
...
$(document).on('click', '.person-search-result-house', function(e){
    ...
    var OpenElement = '<div class="person-search-result-name">Propriétaire: '+ClickedHouseData.charinfo.firstname+' '+ClickedHouseData.charinfo.lastname+'</div><div class="person-search-result-bsn">Maison: '+ClickedHouseData.label+'</div> <div class="person-opensplit"></div> &nbsp; <div class="person-search-result-dob">Adresse: '+ClickedHouseData.label+' &nbsp; <i class="fas fa-map-marker-alt house-adress-location" id="'+ClickedHouseId+'"></i></div> <div class="person-search-result-number">Étage: '+ClickedHouseData.tier+'</div> <div class="person-search-result-nationality">Garage: ' + GarageLabel + '</div>';
    ...
    if (OpenedHouse === null) {
        ...
        var PreviousElement = '<div class="person-search-result-name">Propriétaire: '+PreviousPersonData.charinfo.firstname+' '+PreviousPersonData.charinfo.lastname+'</div> <div class="person-search-result-bsn">Maison: '+PreviousPersonData.label+'</div>';
        ...
        $.post('https://qb-phone/FetchSearchResults', JSON.stringify({
            ...
            if (result != null) {
                ...
                $.each(result, function (i, person) {
                    var PersonElement = '<div class="person-search-result" id="person-'+i+'"><div class="person-search-result-name">Nom: '+person.firstname+' '+person.lastname+'</div> <div class="person-search-result-bsn">CSN: '+person.citizenid+'</div> </div>';
                    $(".person-search-results").append(PersonElement);
                    $("#person-"+i).data("PersonData", person);
                });
            } else {
                QB.Phone.Notifications.Add("politie", "MDT", "Aucun résultats!");
                $(".person-search-results").html("");
            }
        });
    } else {
        ...
        $.post('https://qb-phone/FetchPlayerHouses', JSON.stringify({
            ...
            if (result != null) {
                ...
                $.each(result, function (i, house) {
                    var PersonElement = '<div class="person-search-result-house" id="personhouse-'+i+'"><div class="person-search-result-name">Propriétaire: '+house.charinfo.firstname+' '+house.charinfo.lastname+'</div> <div class="person-search-result-bsn">Maison: '+house.label+'</div></div>';
                    $(".person-search-results").append(PersonElement);
        ...
        $.post('https://qb-phone/FetchVehicleResults', JSON.stringify({
            ...
            if (result != null) {
                ...
                $.each(result, function (i, vehicle) {
                    ...
                    var VehicleElement = '<div class="vehicle-search-result"> <div class="vehicle-search-result-name">'+vehicle.label+'</div> <div class="vehicle-search-result-plate">Immatriculation: '+vehicle.plate+'</div> <div class="vehicle-opensplit"></div> &nbsp; <div class="vehicle-search-result-owner">Propriétaire: '+vehicle.owner+'</div> &nbsp; <div class="vehicle-search-result-apk">MOT: '+APK+'</div> <div class="vehicle-search-result-warrant">Signalé: '+Flagged+'</div> </div>'
...
$(document).on('click', '.scan-search-vehicle', function(e){
    ...
    $.post('https://qb-phone/FetchVehicleScan', JSON.stringify({}), function(vehicle){
        if (vehicle != null) {
            ...
            QB.Phone.Notifications.Add("politie", "MDT", "Aucun véhicule à proximité!");
...
AddPoliceAlert = function(data) {
    ...
    if (data.alert.coords != undefined && data.alert.coords != null) {
        AlertElement = '<div class="meos-alert" id="alert-'+randId+'"> <span class="meos-alert-new" style="margin-bottom: 1vh;">NOUVEAU</span> <p class="meos-alert-type">Alerte: '+data.alert.title+'</p> <p class="meos-alert-description">'+data.alert.description+'</p> <hr> <div class="meos-location-button">LOCALISER</div> </div>';
    } else {
        AlertElement = '<div class="meos-alert" id="alert-'+randId+'"> <span class="meos-alert-new" style="margin-bottom: 1vh;">NOUVEAU</span> <p class="meos-alert-type">Alerte: '+data.alert.title+'</p> <p class="meos-alert-description">'+data.alert.description+'</p></div>';
    }
    $(".meos-recent-alerts").html('<div class="meos-recent-alert" id="recent-alert-'+randId+'"><span class="meos-recent-alert-title">Alerte: '+data.alert.title+'</span><p class="meos-recent-alert-description">'+data.alert.description+'</p></div>');
...
$(document).on('click', '.meos-recent-alert', function(e){
    ...
    if (alertData.coords != undefined && alertData.coords != null) {
        ...
        QB.Phone.Notifications.Add("politie", "MDT", "Cette alerte n'a pas de localisation GPS!");
    }
});
...
$(document).on('click', '.meos-clear-alerts', function(e){
    $(".meos-alerts").html("");
    $(".meos-recent-alerts").html('<div class="meos-recent-alert"> <span class="meos-recent-alert-title">Vous n\'avez pas d\'alertes!</span></div>');
    QB.Phone.Notifications.Add("politie", "MDT", "Toutes les alertes ont été supprimées!");
});
