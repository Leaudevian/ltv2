...
$(document).on('click', '.racing-race', function(e){
    ...
    if (!RaceData.RaceData.Started || IsRacer) {
        ...
        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "La course a déjà commencé..", "#1DA1F2");
    }
});
...
function SetupRaces(Races) {
    ...
    if (Races.length > 0) {
        ...
        $.each(Races, function(i, race){
            var Locked = '<i class="fas fa-unlock"></i> Pas encore commencé';
            if (race.RaceData.Started) {
                Locked = '<i class="fas fa-lock"></i> Commencé';
            }
            var LapLabel = "";
            if (race.Laps == 0) {
                LapLabel = "SPRINT"
                ...
                if (race.Laps == 1) {
                    LapLabel = race.Laps + " Tour";
                } else {
                    LapLabel = race.Laps + " Tours";
                }
            }
            ...
            var Buttons = '<div class="race-buttons"> <div class="race-button" id="join-race" data-toggle="racetooltip" data-placement="left" title="Rejoindre"><i class="fas fa-sign-in-alt"></i></div>';
            if (InRace) {
                if (!Creator) {
                    Buttons = '<div class="race-buttons"> <div class="race-button" id="quit-race" data-toggle="racetooltip" data-placement="right" title="Quitter"><i class="fas fa-sign-out-alt"></i></div>';
                } else {
                    if (!race.RaceData.Started) {
                        Buttons = '<div class="race-buttons"> <div class="race-button" id="start-race" data-toggle="racetooltip" data-placement="left" title="Démarrer"><i class="fas fa-flag-checkered"></i></div><div class="race-button" id="quit-race" data-toggle="racetooltip" data-placement="right" title="Quitter"><i class="fas fa-sign-out-alt"></i></div>';
                    } else {
                        Buttons = '<div class="race-buttons"> <div class="race-button" id="quit-race" data-toggle="racetooltip" data-placement="right" title="Quitter"><i class="fas fa-sign-out-alt"></i></div>';
                    }
                }
                ...
                if (InDistance) {
                    $.post('https://qb-phone/IsBusyCheck', JSON.stringify({
                        ...
                        if (!IsBusy) {
                            ...
                            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Vous êtes dans un éditeur..", "#1DA1F2");
                        }
                    });
                }
            })
        } else {
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Tu es déjà dans une course..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '.dropdown .dropdown-menu li', function(e) {
    $.post('https://qb-phone/GetTrackData', JSON.stringify({
        ...
        $(".racing-setup-information-creator").html('Créateur: ' + CreatorTag);
            ...
            if (!data.IsBusy) {
                $.post('https://qb-phone/IsBusyCheck', JSON.stringify({
                    ...
                    if (!InRace) {
                        ...
                        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Tu es dans une course..", "#1DA1F2");
                    }
                });
            } else {
                QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Vous avez déjà configurer  une piste..", "#1DA1F2");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Vous n'avez pas les droits pour faire des pistes de course..", "#1DA1F2");
        }
    });
});
        ...
        $.post('https://qb-phone/IsAuthorizedToCreateRaces', JSON.stringify({
            ...
            if (data.IsAuthorized) {
                if (data.IsNameAvailable) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Ce nom n'est pas disponible..", "#1DA1F2");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Vous n'avez aucun droit pour créer des pistes de course..", "#1DA1F2");
            }
        });
    } else {
        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Vous devez entrer un nom de piste..", "#1DA1F2");
    }
});
                            ...
                            $.post('https://qb-phone/CanRaceSetup', JSON.stringify({}), function(CanSetup){
                                if (CanSetup) {
                                    ...
                                    $(".racing-setup").animate({
                                        ...
                                        $(".racing-setup-information-distance").html('Sélectionnez une piste');
                                        $(".racing-setup-information-creator").html('Sélectionnez une piste');
                                        $(".racing-setup-information-wr").html('Sélectionnez une piste');
                                        $(".racing-setup-laps").val("");
                                        $('.dropdown').find('input').removeAttr('value');
                                        $('.dropdown').find('span').text("Sélectionnez une piste");
                                    });
                                } else {
                                    QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Il ne peut y avoir qu'une ..", "#1DA1F2");
                                }
                            });
                        } else {
                            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Remplir un nombre de tours..", "#1DA1F2");
                        }
                    } else {
                        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Vous n'avez pas sélectionné de piste..", "#1DA1F2");
                    }
                }
            })
        } else {
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Vous avez déjà une course active..", "#1DA1F2");
        }
    });
});
        ...
        if (Races !== null) {
            ...
            $.each(Races, function(i, race){
                if (race.LastLeaderboard.length > 0) {
                    var elem = '<div class="racing-leaderboard-item" id="leaderboard-item-'+i+'"> <span class="racing-leaderboard-item-name"><i class="fas fa-flag-checkered"></i> '+race.RaceName+'</span> <span class="racing-leaderboard-item-info">Cliquez pour plus de détails</span> </div>'
