...
QB.Phone.Functions.GetLastMessage = function(messages) {
    ...
    $.each(messages[messages.length - 1], function(i, msg){
        ...
        if(LastMessageData.message == '') 'Hmm, je ne devrais pas pouvoir faire ça...'
    });
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    if (Message !== null && Message !== undefined && Message !== "") {
        ...
        QB.Phone.Notifications.Add("fab fa-whatsapp", "Whatsapp", "Vous ne pouvez pas envoyer un message vide!", "#25D366", 1750);
    }
});
    ...
    if (OpenedChatData.number !== null) {
        if(e.which === 13){
            ...
            if (Message !== null && Message !== undefined && Message !== "") {
                ...
                if (clean == '') clean = 'Hmm, je ne devrais pas pouvoir faire ça...'
                $.post('https://qb-phone/SendMessage', JSON.stringify({
                    ChatNumber: OpenedChatData.number,
                    ChatDate: GetCurrentDateKey(),
                    ChatMessage: clean,
    ...
    if (cData) {
        ...
        $.each(cData.messages, function(i, chat){
            ...
            $.each(cData.messages[i].messages, function(index, message){
                ...
                if (message.message == '') message.message = 'Hmm, je ne devrais pas pouvoir faire ça...'
                var Sender = "me";
                if (message.sender !== QB.Phone.Data.PlayerData.citizenid) { Sender = "other"; }
                var MessageElement
                if (message.type == "message") {
                    ...
                    MessageElement = '<div class="whatsapp-openedchat-message whatsapp-openedchat-message-'+Sender+' whatsapp-shared-location" data-x="'+message.data.x+'" data-y="'+message.data.y+'"><span style="font-size: 1.2vh;"><i class="fas fa-thumbtack" style="font-size: 1vh;"></i> Localisation</span><div class="whatsapp-openedchat-message-time">'+message.time+'</div></div><div class="clearfix"></div>'
                }
        ...
        var NewDateYear = NewDate.getFullYear();
        var DateString = ""+NewDateDOM+"-"+(NewDateMonth+1)+"-"+NewDateYear;
        var ChatDiv = '<div class="whatsapp-openedchat-messages-'+DateString+' unique-chat"><div class="whatsapp-openedchat-date">AUJOURD\'HUI</div></div>';
