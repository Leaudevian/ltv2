...
QBCore.Functions.CreateCallback('qb-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    ...
    if SenderPly and Config.BillingCommissions[society] then
        ...
        invoiceMailData = {
            ...
            message = string.format('Vous avez reçu une commission de %s$ quand %s %s payé une facture de %s$.', commission, Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
    elseif not SenderPly and Config.BillingCommissions[society] then
        invoiceMailData = {
            sender = 'Billing Department',
            subject = 'Bill Paid',
            message = string.format('%s %s a payé une facture de %s$', Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
    end
        ...
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerClientEvent('QBCore:Notify', source, 'Facture envoyée avec succès', 'success')
                    TriggerClientEvent('QBCore:Notify', billed.PlayerData.source, 'New Invoice Received')
                else
                    TriggerClientEvent('QBCore:Notify', source, 'Doit être un montant valide supérieur à 0', 'error')
                end
            else
                TriggerClientEvent('QBCore:Notify', source, 'Vous ne pouvez pas vous l\'envoyer à vous même', 'error')
            end
        else
            TriggerClientEvent('QBCore:Notify', source, 'Joueur hors ligne', 'error')
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Pas d\'accès', 'error')
    end
end)
...
AddEventHandler('qb-phone:server:TransferMoney', function(iban, amount)
    ...
    if result[1] ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Ce numéro de compte n\'existe pas!", "error")
    end
end)
...
QBCore.Functions.CreateCallback('qb-phone:server:ScanPlate', function(source, cb, plate)
    ...
    if plate ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Aucun véhicule à proximité', 'error')
        cb(nil)
    end
end)
...
QBCore.Functions.CreateCallback('qb-phone:server:CanTransferMoney', function(source, cb, amount, iban)
    ...
    if (Player.PlayerData.money.bank - amount) >= 0 then
        ...
        if result[1] ~= nil then
            ...
            TriggerClientEvent('QBCore:Notify', source, "Ce numéro de compte n'existe pas!", "error")
            cb(false)
        end
    end
end)
...
QBCore.Commands.Add("setmetadata", "Définir les métadonnées du joueur (God uniquement)", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
