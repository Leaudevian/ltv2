StatusList = {
	["fight"] = "Mains endomagées",
	["widepupils"] = "Pupilles dilatées",
	["redeyes"] = "Yeux rouges",
	["weedsmell"] = "Pue la beuh sa mère",
	["gunpowder"] = "Traces de poudre à canon sur les vêtements",
	["chemicals"] = "Sent les produits chimiques",
	["heavybreath"] = "Respire fortement",
	["sweat"] = "Transpire beaucoup",
    ["handbleed"] = "Mains ensanglantées",
	["confused"] = "Confus",
	["alcohol"] = "Sent l\'alcool",
	["heavyalcohol"] = "Sent beaucoup l\'alcool",
}
...
AddEventHandler("evidence:client:ClearBlooddropsInArea", function()
	...
	local blooddropList = {}
	QBCore.Functions.Progressbar("clear_blooddrops", "Nettoyer le sang...", 5000, false, true, {
		...
		if Blooddrops ~= nil and next(Blooddrops) ~= nil then 
			...
			QBCore.Functions.Notify("Sang nettoyé:)")
		end
    end, function() -- Cancel
        QBCore.Functions.Notify("Sang pas nettoyé...", "error")
    end)
end)
...
AddEventHandler('evidence:client:AddCasing', function(casingId, weapon, coords, serie)
    Casings[casingId] = {
		type = weapon,
		serie = serie ~= nil and serie or "Numéro de série invisible...",
		coords = {
			x = coords.x,
			y = coords.y, 
			z = coords.z - 0.9,
...
AddEventHandler("evidence:client:ClearCasingsInArea", function()
	local pos = GetEntityCoords(PlayerPedId())
	local casingList = {}
	QBCore.Functions.Progressbar("clear_casings", "Nettoyage des douilles...", 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
		...
		if Casings ~= nil and next(Casings) ~= nil then 
			...
			QBCore.Functions.Notify("Douilles nettoyées :)")
		end
    end, function() -- Cancel
        QBCore.Functions.Notify("Douilles non nettoyées", "error")
    end)
end)
	...
	while true do 
		...
		if CurrentCasing ~= nil and CurrentCasing ~= 0 then 
			...
			if #(pos - vector3(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z)) < 1.5 then
				DrawText3D(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z, "~g~G~w~ - Douilles de balles ~b~#"..Casings[CurrentCasing].type)
		...
		if CurrentBlooddrop ~= nil and CurrentBlooddrop ~= 0 then 
			...
			if #(pos - vector3(Blooddrops[CurrentBlooddrop].coords.x, Blooddrops[CurrentBlooddrop].coords.y, Blooddrops[CurrentBlooddrop].coords.z)) < 1.5 then
				DrawText3D(Blooddrops[CurrentBlooddrop].coords.x, Blooddrops[CurrentBlooddrop].coords.y, Blooddrops[CurrentBlooddrop].coords.z, "~g~G~w~ - Sang ~b~#"..DnaHash(Blooddrops[CurrentBlooddrop].citizenid))
				if IsControlJustReleased(0, 47) then
		...
		if CurrentFingerprint ~= nil and CurrentFingerprint ~= 0 then 
			...
			if #(pos - vector3(Fingerprints[CurrentFingerprint].coords.x, Fingerprints[CurrentFingerprint].coords.y, Fingerprints[CurrentFingerprint].coords.z)) < 1.5 then
				DrawText3D(Fingerprints[CurrentFingerprint].coords.x, Fingerprints[CurrentFingerprint].coords.y, Fingerprints[CurrentFingerprint].coords.z, "~g~G~w~ - Empreintes digitales ")
				if IsControlJustReleased(0, 47) then
