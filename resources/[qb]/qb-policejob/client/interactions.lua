...
AddEventHandler('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if IsEntityPlayingAnim(playerPed, "missminuteman_1ig_2", "handsup_base", 3) or IsEntityPlayingAnim(playerPed, "mp_arresting", "idle", 3) or IsTargetDead(playerId) then
            QBCore.Functions.Progressbar("robbing_player", "Arrestation en cours...", math.random(5000, 7000), false, true, {
...
AddEventHandler('police:client:JailPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(time) > 0 then
            ...
            QBCore.Functions.Notify("Le temps doit être supérieur à 0...", "error")
        end
    else
        QBCore.Functions.Notify("Personne à proximité !", "error")
    end
    ...
    if not IsPedRagdoll(PlayerPedId()) then
        ...
        if player ~= -1 and distance < 1.5 then
            ...
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(PlayerPedId())) then
                ...
                QBCore.Functions.Notify("Vous ne pouvez pas menotter quelqu'un dans un véhicule", "error")
            end
        else
            QBCore.Functions.Notify("Personne à proximité !", "error")
        end
        ...
        if player ~= -1 and distance < 1.5 then
            QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                if result then 
                    ...
                    QBCore.Functions.Notify("Vous ne pouvez pas vous mettre vous-même les menottes", "error")
                end
            end, "handcuffs")
        else
            QBCore.Functions.Notify("Personne à proximité !", "error")
...
AddEventHandler('police:client:GetCuffed', function(playerId, isSoftcuff)
    if not isHandcuffed then
        ...
        if not isSoftcuff then
            ...
            QBCore.Functions.Notify("Vous êtes arrêtés!")
        else
            cuffType = 49
            GetCuffedAnimation(playerId)
            QBCore.Functions.Notify("Vous êtes arrêtés, mais vous pouvez parler")
        end
    else
        isHandcuffed = false
        isEscorted = false
        ...
        ClearPedTasksImmediately(PlayerPedId())
        QBCore.Functions.Notify("Vous êtes libérés ! (libéré,délivré!)")
    end
end)
