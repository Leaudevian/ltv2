...
AddEventHandler('police:client:spawnCone', function()
    QBCore.Functions.Progressbar("spawn_object", "Placer un objet...", 2500, false, true, {
...
AddEventHandler('police:client:deleteObject', function()
    ...
    if dist < 5.0 then
        QBCore.Functions.Progressbar("remove_object", "Retirer un object...", 2500, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
...
AddEventHandler('police:client:SpawnSpikeStrip', function()
    if #SpawnedSpikes + 1 < SpikeConfig.MaxSpikes then
        ...
        QBCore.Functions.Notify('Il n\'y a plus de herses', 'error')
    end
end)
            ...
            if ClosestSpike ~= nil then
                ...
                if dist < 4 then
                    if not IsPedInAnyVehicle(PlayerPedId()) then
                        if PlayerJob.name == "police" and PlayerJob.onduty then
                            DrawText3D(pos.x, pos.y, pos.z, '[E] Enlever la herse')
