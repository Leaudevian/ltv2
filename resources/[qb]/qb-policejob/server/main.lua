...
AddEventHandler('police:server:TakeOutImpound', function(plate)
    ...
    TriggerClientEvent('QBCore:Notify', src, "Le véhicule a été sorti de la fourrière!")  
end)
...
AddEventHandler('police:server:EscortPlayer', function(playerId)
    ...
    if EscortPlayer ~= nil then
        if (Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance" or Player.PlayerData.job.name == "doctor") or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"]) then
            ...
            TriggerClientEvent('chatMessage', src, "SYSTEM", "error", "La personne est morte ou n'a pas été arrêtée!")
        end
    end
end)
...
AddEventHandler('police:server:BillPlayer', function(playerId, price)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer ~= nil then
            ...
            TriggerClientEvent('QBCore:Notify', OtherPlayer.PlayerData.source, "Vous avez reçu "..price.."$")
        end
    end
end)
...
AddEventHandler('police:server:JailPlayer', function(playerId, time)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer ~= nil then
            ...
            TriggerClientEvent('QBCore:Notify', src, "Vous avez mis la personne en prison pour "..time.." mois")
        end
    end
end)
...
AddEventHandler('police:server:SeizeCash', function(playerId)
    ...
    if SearchedPlayer ~= nil then 
        ...
        TriggerClientEvent('QBCore:Notify', SearchedPlayer.PlayerData.source, "Votre argent a été confisquée..")
    end
end)
...
AddEventHandler('police:server:SeizeDriverLicense', function(playerId)
    ...
    if SearchedPlayer ~= nil then
        ...
        if driverLicense then
            ...
            TriggerClientEvent('QBCore:Notify', SearchedPlayer.PlayerData.source, "Votre permis de conduire a été confisqué..")
        else
            TriggerClientEvent('QBCore:Notify', src, "Vous ne pouvez pas confisqué ce permis..", "error")
        end
    end
end)
...
AddEventHandler('police:server:RobPlayer', function(playerId)
    ...
    if SearchedPlayer ~= nil then 
        ...
        TriggerClientEvent('QBCore:Notify', SearchedPlayer.PlayerData.source, "On vous a volé "..money.."$")
	TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, "Vous avez perdu "..money.."$")
    end
end)
...
AddEventHandler('police:server:Impound', function(plate, fullImpound, price, body, engine, fuel)
    ...
    if IsVehicleOwned(plate) then
        if not fullImpound then
            ...
            TriggerClientEvent('QBCore:Notify', src, "Véhicule mis en fourrière pour "..price.."$ !")
        else
            ...
            TriggerClientEvent('QBCore:Notify', src, "Véhicule saisi !")
        end
    end
end)
...
AddEventHandler('police:server:TakeOutImpound', function(plate)
    local src = source       
    exports.oxmysql:execute('UPDATE player_vehicles SET state = @state WHERE plate = @plate', {['@state'] = 0, ['@plate'] = plate})
    TriggerClientEvent('QBCore:Notify', src, "Votre véhicule a été sorti de la fourrière !")
...
AddEventHandler('evidence:server:AddBlooddropToInventory', function(bloodId, bloodInfo)
    ...
    if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Vous devez avoir un sac de preuves vide avec vous", "error")
    end
end)
...
AddEventHandler('police:server:SetTracker', function(targetId)
    ...
    if TrackerMeta then
        ...
        TriggerClientEvent('QBCore:Notify', targetId, 'Bracelet électronique désactivé', 'error', 5000)
        TriggerClientEvent('QBCore:Notify', source, 'Vous avez enlevé le bracelet électronique de '..Target.PlayerData.charinfo.firstname.." "..Target.PlayerData.charinfo.lastname, 'error', 5000)
        TriggerClientEvent('police:client:SetTracker', targetId, false)
    else
        Target.Functions.SetMetaData("tracker", true)
        TriggerClientEvent('QBCore:Notify', targetId, 'Vous mettez un bracelet électronique', 'error', 5000)
        TriggerClientEvent('QBCore:Notify', source, 'Vous avez mis un bracelet électronique à '..Target.PlayerData.charinfo.firstname.." "..Target.PlayerData.charinfo.lastname, 'error', 5000)
...
AddEventHandler('police:server:SendTrackerLocation', function(coords, requestId)
    ...
    local msg = "La localisation de "..Target.PlayerData.charinfo.firstname.." "..Target.PlayerData.charinfo.lastname.." est indiquée sur votre GPS."

    local alertData = {
        title = "Emplacement du bracelet",
        coords = {x = coords.x, y = coords.y, z = coords.z},
        description = msg
    }
...
QBCore.Commands.Add("spikestrip", "Placer une herse (seulement policier)", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player ~= nil then 
        if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
...
QBCore.Commands.Add("grantlicense", "Donner une license à quelqu'un", {{name="id", help="ID du joueur"},{name="license", help="Type de license"}}, true, function(source, args)
    local source = source
    local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.grade.level >= 2 then
        if args[2] == "driver" or args[2] == "weapon" then
            ...
            if SearchedPlayer ~= nil then
                ...
                TriggerClientEvent('QBCore:Notify', SearchedPlayer.PlayerData.source, "Vous avez obtenu une licence", "success", 5000)
                TriggerClientEvent('QBCore:Notify', source, "Vous avez obtenu une licence", "success", 5000)
            end
        else
            TriggerClientEvent('QBCore:Notify', source, "Type de license invalide", "error")
        end
    else
        TriggerClientEvent('QBCore:Notify', source, "Tu dois être sergent pour donner une license !", "error")
    end
end)

QBCore.Commands.Add("revokelicense", "Retirer la license de quelqu'un", {{name="id", help="ID du joueur"},{name="license", help="Type de la license"}}, true, function(source, args)
    local source = source
    local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.grade.level >= 2 then
        if args[2] == "driver" or args[2] == "weapon" then
            ...
            if SearchedPlayer ~= nil then
                ...
                SearchedPlayer.Functions.SetMetaData("licences", licenseTable)
                TriggerClientEvent('QBCore:Notify', SearchedPlayer.PlayerData.source, "Votre license vous a été retirée", "error", 5000)
                TriggerClientEvent('QBCore:Notify', source, "La license a été retirée", "success", 5000)
            end
        else
            TriggerClientEvent('QBCore:Notify', source, "Type de license invalide", "error")
        end
    else
        TriggerClientEvent('QBCore:Notify', source, "Tu dois être sergent pour retirer une license !", "error")
    end
end)
...
QBCore.Commands.Add("pobject", "Placer/Supprimer un objet (Police uniquement)", {{name="type", help="Type d\'objet ou 'delete' pour supprimer"}}, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.job.name == "police" then
        ...
        TriggerClientEvent('QBCore:Notify', source, 'Seulement pour les services d\'urgence', 'error')
    end
end)

QBCore.Commands.Add("cuff", "Arrêter un joueur (Police uniquement)", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
QBCore.Commands.Add("palert", "Faire une alerte police", {{name="alert", help="The Police alert"}}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    ...
    if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
        if args[1] ~= nil then
            ...
            TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "Vous devez entrer un message !")
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Seulement pour les services d\'urgence', 'error')
    end
...
QBCore.Commands.Add("escort", "Escorter un joueur", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Commands.Add("callsign", "Se donner un panneau", {{name="name", help="Nom du panneau"}}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    Player.Functions.SetMetaData("callsign", table.concat(args, " "))
end)

QBCore.Commands.Add("clearcasings", "Retirer les objets de la zone (police uniquement)", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
QBCore.Commands.Add("jail", "Envoyez en prison (police uniquement)", {{name="id", help="ID du joueur"},{name="time", help="Temps passé en prison"}}, true, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
        ...
        if time > 0 then
            ...
            TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "Le temps doit être supérieur à 0")
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Seulement pour les services d\'urgence', 'error')
    end
...
QBCore.Commands.Add("unjail", "Libéré de prison (Police uniquement)", {{name="id", help="ID du joueur"}}, true, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
        local playerId = tonumber(args[1])
...
QBCore.Commands.Add("clearblood", "Nettoyer la zone du sang (Police uniqement)", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
QBCore.Commands.Add("seizecash", "Saisir de l'argent (Police uniquement)", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
QBCore.Commands.Add("sc", "Brassard de Police (Police uniquement)", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
QBCore.Commands.Add("cam", "Regarder les caméras de surveillance (Police uniquement)", {{name="camid", help="Camera ID"}}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
QBCore.Commands.Add("flagplate", "Signaler une immatriculation (Police uniquement)", {{name="plate", help="Immatriculation"}, {name="reason", help="Raison du signalement du véhicule"}}, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.job.name == "police" then
        ...
        TriggerClientEvent('QBCore:Notify', source, "Véhicule ("..args[1]:upper()..") signalé pour : "..table.concat(reason, " "))
    else
        TriggerClientEvent('QBCore:Notify', source, 'Seulement pour les services d\'urgence', 'error')
    end
end)

QBCore.Commands.Add("unflagplate", "Dé-signaler une immatriculation (Police uniquement)", {{name="plate", help="Immatriculation"}}, true, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
        if Plates ~= nil and Plates[args[1]:upper()] ~= nil then
            if Plates[args[1]:upper()].isflagged then
                Plates[args[1]:upper()].isflagged = false
                TriggerClientEvent('QBCore:Notify', source, "Véhicule ("..args[1]:upper()..") n'est plus signalé")
            else
                TriggerClientEvent('chatMessage', source, "REPORTING ROOM", "error", "Le véhicule n'est pas signalé !")
            end
        else
            TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "Le véhicule n'est pas signalé !")
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Seulement pour les services d\'urgence', 'error')
    end
...
QBCore.Commands.Add("plateinfo", "Vérifier si la plaque a été signalée (Police uniquement)", {{name="plate", help="Immatriculation"}}, true, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
        if Plates ~= nil and Plates[args[1]:upper()] ~= nil then
            if Plates[args[1]:upper()].isflagged then
                TriggerClientEvent('chatMessage', source, "REPORTING ROOM", "normal", "Véhicule ("..args[1]:upper()..") a été signalé pour : "..Plates[args[1]:upper()].reason)
            else
                TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "Le véhicule n'est pas signalé !")
            end
        else
...
QBCore.Commands.Add("depot", "Faire une facture (Police uniquement)", {{name="price", help="Montant que la personne a payé"}}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
QBCore.Commands.Add("impound", "Mettre en fourrière un véhicule (Police uniquement)", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
QBCore.Commands.Add("paytow", "Payer le chauffeur de remorquage (Police, EMS seulement)", {{name="id", help="ID du joueur"}}, true, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance" then
        ...
        if OtherPlayer ~= nil then
            if OtherPlayer.PlayerData.job.name == "tow" then
                ...
                TriggerClientEvent('chatMessage', OtherPlayer.PlayerData.source, "SYSTEM", "warning", "Tu as reçu 500 $ pour ton service")
                TriggerClientEvent('QBCore:Notify', source, 'Vous avez payé le remorqueur')
            else
                TriggerClientEvent('QBCore:Notify', source, 'Cette personne n\'est pas un remorqueur', "error")
            end
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Seulement pour les services d\'urgence', 'error')
...
QBCore.Commands.Add("paylawyer", "Payer l'avocat (Police, Avocat uniquement)", {{name="id", help="ID du joueur"}}, true, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "judge" then
        ...
        if OtherPlayer ~= nil then
            if OtherPlayer.PlayerData.job.name == "lawyer" then
                ...
                TriggerClientEvent('chatMessage', OtherPlayer.PlayerData.source, "SYSTEM", "warning", "Tu reçois 500 $ pour ton travail")
                TriggerClientEvent('QBCore:Notify', source, 'Vous avez payé l\'avocat')
            else
                TriggerClientEvent('QBCore:Notify', source, 'La personne n\'est pas avocat', "error")
            end
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Seulement pour les services d\'urgence', 'error')
...
QBCore.Commands.Add("radar", "Activer le radar (Police uniquement)", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" then
...
QBCore.Commands.Add("911", "Envoyer un message aux services d'urgence", {{name="message", help="Votre message"}}, true, function(source, args)
    ...
    if Player.Functions.GetItemByName("phone") ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', source, 'Tu n\'as pas de téléphone', 'error')
    end
end)

QBCore.Commands.Add("911a", "Envoyer un message anonyme aux services d'urgence (sans localisation)", {{name="message", help="Votre message"}}, true, function(source, args)
    local message = table.concat(args, " ")
    local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Commands.Add("911r", "Envoyer une réponse à l'alerte", {{name="id", help="ID de l'alerte"}, {name="message", help="Votre message"}}, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Commands.Add("anklet", "Mettre un bracelet électronique (Police uniquement)", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Commands.Add("ankletlocation", "Voir la localisation d'un bracelet électronique", {{"csn", "CSN du bracelet"}}, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
        ...
        if args[1] ~= nil then
            ...
            if Target ~= nil then
                if Target.PlayerData.metadata["tracker"] then
                    ...
                    TriggerClientEvent('QBCore:Notify', source, 'Cette personne n\'a pas de bracelet électronique', 'error')
                end
            end
        end
    else
...
QBCore.Commands.Add("removeanklet", "Enlever un bracelet électronique (Police uniquement)", {{"bsn", "BSN du joueur"}}, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Commands.Add("ebutton", "Répondre à un appel (Police, EMS, Mécaniciens uniquement)", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    if ((Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance" or Player.PlayerData.job.name == "doctor") and Player.PlayerData.job.onduty) then
...
QBCore.Commands.Add("takedrivinglicense", "Saisir le permis de conduire (Police uniquement)", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    if ((Player.PlayerData.job.name == "police") and Player.PlayerData.job.onduty) then
...
QBCore.Commands.Add("takedna", "Prélever l\'ADN de la personne", {{"id", "ID du joueur"}}, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    local OtherPlayer = QBCore.Functions.GetPlayer(tonumber(args[1]))
    if ((Player.PlayerData.job.name == "police") and Player.PlayerData.job.onduty) and OtherPlayer ~= nil then
        if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
            ...
            TriggerClientEvent('QBCore:Notify', source, "Vous devez avoir un sac de preuve vide", "error")
        end
    end
end)
