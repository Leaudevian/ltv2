...
function ToggleClothing(whic, extra)
	...
	if which ~= "Mask" then
		if not Gender then Notify("Ce modèle PED ne permet pas cette option.") return false end -- We cancel the command here if the person is not using a multiplayer model.
	end
	...
	if not Toggle.Table.Standalone then -- "Standalone" is for things that dont require a variant, like the shoes just need to be switched to a specific drawable. Looking back at this i should have planned ahead, but it all works so, meh!
		...
		Notify("Il ne semble pas y avoir de variantes.") return
	else
		if not LastEquipped[which] then
	...
	Notify("Vous portez déjà un chapeau.") return false
end
...
function ToggleProps(whic)
	...
	if not Prop.Variants then
		...
		Notify("Il n'y a rien à supprimer.") return false
	else
		local Gender = IsMpPed(Ped)
		if not Gender then Notify("Ce modèle PED ne permet pas cette option.") return false end -- We dont really allow for variants on ped models, Its possible, but im pretty sure 95% of ped models dont really have variants.
		...
		Notify("Il ne semble pas y avoir de variantes.") return false
	end
end
