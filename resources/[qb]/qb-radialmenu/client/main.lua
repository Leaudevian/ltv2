...
AddEventHandler('qb-radialmenu:client:openDoor', function(data)
    ...
    if closestVehicle ~= 0 then
        ...
        QBCore.Functions.Notify('Il n\'y a pas de véhicule en vue...', 'error', 2500)
    end
end)
        ...
        if GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
            if DoesExtraExist(veh, extra) then 
                if IsVehicleExtraTurnedOn(veh, extra) then
                    ...
                    QBCore.Functions.Notify('Extra ' .. extra .. ' Désactivé', 'error', 2500)
                else
                    enginehealth = GetVehicleEngineHealth(veh)
                    bodydamage = GetVehicleBodyHealth(veh)
                    ...
                    SetVehicleEngineHealth(veh, enginehealth)
                    SetVehicleBodyHealth(veh, bodydamage)
                    QBCore.Functions.Notify('Extra ' .. extra .. ' Activé', 'success', 2500)
                end    
            else
                QBCore.Functions.Notify('Extra ' .. extra .. ' n\'est pas présent sur ce véhicule ', 'error', 2500)
            end
        else
            QBCore.Functions.Notify('Vous n\'êtes pas le conducteur !', 'error', 2500)
        end
    end
end)
    ...
    if not HasHarnass then
        ...
        if IsSeatFree then
            if kmh <= 100.0 then
                ...
                QBCore.Functions.Notify('Le véhicule va trop vite..')
            end
        else
            QBCore.Functions.Notify('Ce siège est occupé..')
        end
    else
        QBCore.Functions.Notify('Vous avez la ceinture, enlevé là pour vous déplacer..', 'error')
    end
end)
