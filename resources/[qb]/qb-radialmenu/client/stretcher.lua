...
AddEventHandler('qb-radialmenu:client:TakeStretcher', function()
    ...
    if Vehicle ~= 0 then
        ...
        if Obj ~= nil or Obj ~= 0 then
            ...
            QBCore.Functions.Notify("Quelque chose a mal tourné..", 'error')
        end
    else
        QBCore.Functions.Notify("Tu n'es pas près d'une ambulance..", 'error')
    end
end)
...
AddEventHandler('qb-radialmenu:client:RemoveStretcher', function()
    ...
    if StretcherObject~= nil then
        ...
        if Dist < 3.0 then
            ...
            QBCore.Functions.Notify('Tu es trop loin!', 'error')
        end
    end
end)
        ...
        if StretcherObject ~= nil then
            ...
            if Distance <= 1.0 then
                if not IsAttached then
                    DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z, 'Appuyer sur ~g~E~w~ pour prendre la civière / ~g~H~w~ pour la verrouiller')
                    ...
                    DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z, 'Appuyer sur  ~g~E~w~ - pour pousser la civière')
                    if IsControlJustPressed(0, 51) then
                        DetachStretcher()
                        IsAttached = false
                    end
                ...
                if not IsLayingOnBed then
                    if not IsAttached then
                        DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z + 0.2, 'Appuyer sur  ~g~G~w~ - pour se coucher sur la civière')
            ...
            elseif Distance <= 2 then
                if not IsLayingOnBed then
                    ...
                    if not IsAttached then
                        DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z + 0.2, 'Appuyer sur  ~g~G~w~ - pour ranger la civière')
                        if IsControlJustPressed(0, 47) or IsDisabledControlJustPressed(0, 47) then
                            GetOffBrancard()
                        end
                    end
...
AddEventHandler('qb-radialmenu:client:Result', function(IsBusy, type)
    ...
    if type == "lay" then
        if not IsBusy then
            ...
            QBCore.Functions.Notify("Cette civière est déjà utilisée!", "error")
            IsLayingOnBed = false
        end
    else
        if not IsBusy then
