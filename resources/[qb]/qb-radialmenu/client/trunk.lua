...
AddEventHandler('qb-trunk:client:KidnapTrunk', function()
    ...
    if (distance ~= -1 and distance < 2) then
        if isKidnapping then
            ...
            QBCore.Functions.Notify('Vous n\'avez pas kipnapé cette personne!', 'error')
        end
    end
end)
                    ...
                    if not isBusy then
                        if not isKidnapped then
                            -- if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                QBCore.Functions.Notify('Vous êtes dans le coffre.', 'success', 4000)
                                TrunkCam(true)
                            ...
                            if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                                ...
                                QBCore.Functions.Notify('Le coffre est fermé?', 'error', 2500)
                            end
                        end
                    else
                        QBCore.Functions.Notify('Déjà quelqu\'un dans ce coffre?', 'error', 2500)
                    end 
                else
                    QBCore.Functions.Notify('Votre déjà dans le coffre', 'error', 2500)
                end 
            else
                QBCore.Functions.Notify('Vous ne pouvez pas entrer dans ce coffre..', 'error', 2500)
            end
        end, plate)
    else
        QBCore.Functions.Notify('Vous ne pouvez pas entrer dans ce coffre..', 'error', 2500)
...
AddEventHandler('qb-trunk:client:GetIn', function(isKidnapped)
    ...
    if closestVehicle ~= 0 then
                    ...
                    if not inTrunk then
                        if not isBusy then
                            if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                Citizen.Wait(500)
                                SetVehicleDoorShut(closestVehicle, 5, false)
                                QBCore.Functions.Notify('Vous êtes déjà dans le coffre.', 'Goodluck', 4000)
                                TrunkCam(true)
                            else
                                QBCore.Functions.Notify('Est le tronc fermé?', 'error', 2500)
                            end
                        else
                            QBCore.Functions.Notify('Déjà quelqu\'un dans ce coffre?', 'error', 2500)
                        end 
                    else
                        QBCore.Functions.Notify('Vous êtes déjà dans le coffre', 'error', 2500)
                    end 
                else
                    QBCore.Functions.Notify('Vous ne pouvez pas entrer dans ce coffre..', 'error', 2500)
                end
        ...
        QBCore.Functions.Notify('Il n\'y a pas de véhicule à proximité.', 'error', 2500)
    end
end)
        ...
        if inTrunk then
            if not isKidnapped then
                ...
                if DoesEntityExist(vehicle) then
                    DrawText3Ds(drawPos.x, drawPos.y, drawPos.z + 0.75, '[E] Sortir du coffre')
                    ...
                    if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                        DrawText3Ds(drawPos.x, drawPos.y, drawPos.z + 0.5, '[G] Fermer le coffre')
                        ...
                        DrawText3Ds(drawPos.x, drawPos.y, drawPos.z + 0.5, '[G] Ouvrir le coffre')
                        if IsControlJustPressed(0, 47) then
                            if not IsVehicleSeatFree(vehicle, -1) then
