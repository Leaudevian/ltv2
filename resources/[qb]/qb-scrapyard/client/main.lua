...
Citizen.CreateThread(function()
	for id, scrapyard in pairs(Config.Locations) do
        ...
        AddTextComponentSubstringPlayerName("Ferrailleur")
		...
		if closestScrapyard ~= 0 then
				...
				if IsPedInAnyVehicle(PlayerPedId()) then
					...
					if vehicle ~= 0 and vehicle ~= nil then 
						...
						if #(pos - vector3(vehpos.x, vehpos.y, vehpos.z)) < 2.5 and not isBusy then
							DrawText3Ds(vehpos.x, vehpos.y, vehpos.z, "~g~E~w~ - Démontage du véhicule")
							if IsControlJustReleased(0, 38) then
								if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
									if IsVehicleValid(GetEntityModel(vehicle)) then 
										ScrapVehicle(vehicle)
									else
										QBCore.Functions.Notify("Ce véhicule ne peut pas être mis au rebut.", "error")
									end
								else
									QBCore.Functions.Notify("Vous n'êtes pas le conducteur", "error")
								end
							end
						end
					end
			...
			if #(pos - vector3(Config.Locations[closestScrapyard]["list"].x, Config.Locations[closestScrapyard]["list"].y, Config.Locations[closestScrapyard]["list"].z)) < 1.5 then
				if not IsPedInAnyVehicle(PlayerPedId()) and not emailSend then
					DrawText3Ds(Config.Locations[closestScrapyard]["list"].x, Config.Locations[closestScrapyard]["list"].y, Config.Locations[closestScrapyard]["list"].z, "~g~E~w~ - Liste de véhicules e-mail")
					if IsControlJustReleased(0, 38) then
						CreateListEmail()
					end
				end
	...
	if Config.CurrentVehicles ~= nil and next(Config.CurrentVehicles) ~= nil then 
		...
		SetTimeout(math.random(15000, 20000), function()
			...
			TriggerServerEvent('qb-phone:server:sendNewMail', {
				...
				subject = "Liste de véhicules",
				message = "Vous ne pouvez démolir qu'un certain nombre de véhiculess.<br />Vous pouvez garder tout ce que vous démolissez pour vous tant que vous ne me dérangez pas.<br /><br /><strong>Liste de véhicules:</strong><br />".. vehicleList,
				button = {}
			})
		end)
	else
		QBCore.Functions.Notify("Vous n'êtes pas autorisé à démolir des véhicules", "error")
	end
end
...
function ScrapVehicle(vehicle)
	...
	QBCore.Functions.Progressbar("scrap_vehicle", "Démolition du Véhicule", scrapTime, false, true, {
