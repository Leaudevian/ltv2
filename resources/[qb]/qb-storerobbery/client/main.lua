    ...
    while true do
        ...
        for k, v in pairs(Config.Registers) do
            ...
            if dist <= 1 and Config.Registers[k].robbed then
                inRange = true
                DrawText3Ds(Config.Registers[k][1].xyz, 'La caisse est vide !')
            end
        end
        if not inRange then
            Citizen.Wait(2000)
                ...
                if dist < 3 then
                    ...
                    if dist < 1.0 then
                        if not Config.Safes[safe].robbed then
                            DrawText3Ds(Config.Safes[safe][1].xyz, '~g~E~w~ - Essayer une combinaison')
                            if IsControlJustPressed(0, 38) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    QBCore.Functions.Notify("Pas assez de policiers (".. Config.MinimumStoreRobberyPolice .." Requis)", "error")
                                end
                            end
                        else
                            DrawText3Ds(Config.Safes[safe][1].xyz, 'Coffre ouvert')
                        end
                    end
                end
            end
    ...
    for k, v in pairs(Config.Registers) do
        ...
        if dist <= 1 and not Config.Registers[k].robbed then
            if CurrentCops >= Config.MinimumStoreRobberyPolice then
                ...
                QBCore.Functions.Notify("Pas assez de policier (2 Requis)", "error")
            end
        end
    end
end)
