        ...
        if Races ~= nil then
                ...
                for k, race in pairs(Races) do
                    if Races[k] ~= nil then
                        if #(pos - vector3(Races[k].startx, Races[k].starty, Races[k].startz)) < 15.0 and not Races[k].started then
                            DrawText3Ds(Races[k].startx, Races[k].starty, Races[k].startz, "[~g~H~w~] Rejoindre la course (~g~"..Races[k].amount.."$,-~w~)")
            ...
            if RaceId ~= 0 and not InRace then
                if #(pos - vector3(Races[RaceId].startx, Races[RaceId].starty, Races[RaceId].startz)) < 15.0 and not Races[RaceId].started then
                    DrawText3Ds(Races[RaceId].startx, Races[RaceId].starty, Races[RaceId].startz, "La course va bientôt commencer")
                end
            end
            -- In race en gestart
            if RaceId ~= 0 and InRace then
                if #(pos - vector3(Races[RaceId].endx, Races[RaceId].endy, pos.z)) < 250.0 and Races[RaceId].started then
                    DrawText3Ds(Races[RaceId].endx, Races[RaceId].endy, pos.z + 0.98, "LIGNE D'ARRIVEE")
            ...
            if ShowCountDown then
                if #(pos - vector3(Races[RaceId].startx, Races[RaceId].starty, Races[RaceId].startz)) < 15.0 and Races[RaceId].started then
                    DrawText3Ds(Races[RaceId].startx, Races[RaceId].starty, Races[RaceId].startz, "La course commence dans ~g~"..RaceCount)
                end
            end
        end
    end
...
AddEventHandler('qb-streetraces:RaceDone', function(race, winner)
    if RaceId ~= 0 and RaceId == race then
        RaceId = 0
        InRace = false
        QBCore.Functions.Notify("La course est terminée! Le gagnant est "..winner.. "!")
    end
end)
...
AddEventHandler('qb-streetraces:CreateRace', function(amount)
    ...
    if DoesBlipExist(WaypointHandle) then
        ...
        if #(pos - vector3(cx, cy, groundZ)) > 500.0 then
            ...
            QBCore.Functions.Notify("La cource a été créer pour "..amount.."$", "success")
        else
            QBCore.Functions.Notify("La position finale est trop proche", "error")
        end
    else
        QBCore.Functions.Notify("Vous devez déposer un marqueur", "error")
    end
end)
