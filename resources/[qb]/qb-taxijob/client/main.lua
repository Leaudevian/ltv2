    ...
    if whitelistedVehicle() then
       -- if NpcData.CountDown == 180 then
            if not NpcData.Active then
                ...
                QBCore.Functions.Notify('Le PNJ est indiquer sur votre GPS', 'success')
                        ...
                        if dist < 20 then
                            ...
                            if dist < 5 then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    ...
                                    QBCore.Functions.Notify("Amener le PNJ à l'emplacement scpécifié")
                                    if NpcData.NpcBlip ~= nil then
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
                                    GetDeliveryLocation()
                ...
                QBCore.Functions.Notify('Tu fais déjà une missin PNJ')
            end
      --  else
         --   QBCore.Functions.Notify('No NPCs Are Available')
       -- end
    else
        QBCore.Functions.Notify("Tu n'es pas dans un Taxi")
    end
end)
            ...
            if dist < 20 then
                ...
                if dist < 5 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        QBCore.Functions.Notify('La personne à été déposée', 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
                    ...
                    if vehDist < 30 then
                        ...
                        if vehDist < 1.5 then
                            if whitelistedVehicle() then
                                ...
                                DrawText3D(Config.Location.x, Config.Location.y, Config.Location.z + 0.3, '[E] Véhicules de travail')
                                if IsControlJustReleased(0, 38) then
                                    TaxiGarage()
...
AddEventHandler('qb-taxi:client:toggleMeter', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then
            ...
            QBCore.Functions.Notify("Ce véhicule n'a pas de compteur de Taxi", 'error')
        end
    else
...
AddEventHandler('qb-taxi:client:enableMeter', function()
    ...
    if meterIsOpen then
        ...
        QBCore.Functions.Notify("Le compteur de Taxi n'es pas activer", 'error')
    end
end)
...
AddEventHandler('qb-taxi:client:toggleMuis', function()
    ...
    if meterIsOpen then
        ...
        QBCore.Functions.Notify('Pas de compteur de Taxi en vue', 'error')
    end
end)
...
function VehicleList()
    ...
    for k, v in pairs(Config.AllowedVehicles) do
        Menu.addButton(Config.AllowedVehicles[k].label, "TakeVehicle", k, "Garage", " Motrice: 100%", " Carosserie: 100%", " Carburant: 100%")
    end
...
Citizen.CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("Taxis")
