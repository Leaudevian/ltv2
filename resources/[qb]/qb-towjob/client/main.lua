...
AddEventHandler('jobs:client:ToggleNpc', function()
    if QBCore.Functions.GetPlayerData().job.name == "tow" then
        if CurrentTow ~= nil then 
            QBCore.Functions.Notify("Fini ton travail d'abord", "error")
            return
        end
        NpcOn = not NpcOn
        if NpcOn then
...
AddEventHandler('qb-tow:client:TowVehicle', function()
    ...
    if isTowVehicle(vehicle) then
        if CurrentTow == nil then 
            ...
            if NpcOn and CurrentLocation ~= nil then
                if GetEntityModel(targetVehicle) ~= GetHashKey(CurrentLocation.model) then
                    QBCore.Functions.Notify("Ce n'es pas le bon Véhicule", "error")
                    return
                end
            end
            if not IsPedInAnyVehicle(PlayerPedId()) then
                if vehicle ~= targetVehicle then
                    ...
                    if #(towPos - targetPos) < 11.0 then
                        QBCore.Functions.Progressbar("towing_vehicle", "Hissage du Véhicule...", 5000, false, true, {
                            ...
                            if NpcOn then
                                RemoveBlip(CurrentBlip)
                                QBCore.Functions.Notify("Amener le véhicule au Depot", "success", 5000)
                            ...
                            QBCore.Functions.Notify("Véhicule Remoqué")
                        end, function() -- Cancel
                            StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
            ...
            QBCore.Functions.Progressbar("untowing_vehicle", "Enlever le Véhicule", 5000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                CurrentTow = nil
                QBCore.Functions.Notify("Véhicule Enlever")
            end, function() -- Cancel
                StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
        ...
        QBCore.Functions.Notify("Vous devez être dans un véhicule de Remorquage", "error")
    end
end)
                ...
                if #(pos - vector3(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z)) < 10.0 then
                    ...
                    if #(pos - vector3(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z)) < 1.5 then
                        if IsPedInAnyVehicle(PlayerPedId(), false) then
                            DrawText3D(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z, "~g~E~w~ - Ranger le véhicule")
                        else
                            DrawText3D(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z, "~g~E~w~ - Véhicules")
                        end
...
function deliverVehicle(vehicle)
    ...
    JobsDone = JobsDone + 1
    VehicleSpawned = false
    QBCore.Functions.Notify("Vous avez délivrer un Véhicule", "success")
    QBCore.Functions.Notify("Un nouveau véhicule peut être Remorquer")
...
function VehicleList(isDown)
    ...
    ClearMenu()
    for k, v in pairs(Config.Vehicles) do
        Menu.addButton(Config.Vehicles[k], "TakeOutVehicle", k, "Garage", " Moteur: 100%", " Carrosserie: 100%", " Carburant: 100%")
    end
