...
RegisterNUICallback('EnterPincode', function(d)
    ...
    if tonumber(d.pin) == data.pincode then
        ...
        QBCore.Functions.Notify('Ce code est incorrect', 'error')
    end
end)
            ...
            if InsideTraphouse then
                ...
                if ExitDistance < 20 then
                    ...
                    if ExitDistance < 1 then
                        DrawText3Ds(data.coords["enter"].x + POIOffsets.exit.x, data.coords["enter"].y + POIOffsets.exit.y, data.coords["enter"].z - Config.MinZOffset + POIOffsets.exit.z, '~b~E~w~ - Partir')
                ...
                if InteractDistance < 20 then
                    ...
                    if InteractDistance < 1 then
                        if not IsKeyHolder then
                            DrawText3Ds(data.coords["interaction"].x, data.coords["interaction"].y, data.coords["interaction"].z + 0.2, '~b~H~w~ - Voir l\'inventaire')
                            DrawText3Ds(data.coords["interaction"].x, data.coords["interaction"].y, data.coords["interaction"].z, '~b~E~w~ - Prendre (~g~5000$~w~)')
                            if IsControlJustPressed(0, 38) then
                            ...
                            DrawText3Ds(data.coords["interaction"].x, data.coords["interaction"].y, data.coords["interaction"].z + 0.2, '~b~H~w~ - Voir l\'inventaire')
                            DrawText3Ds(data.coords["interaction"].x, data.coords["interaction"].y, data.coords["interaction"].z, '~b~E~w~ - Prendre (~g~'..data.money..'$~w~)')
                            if IsHouseOwner then
                                DrawText3Ds(data.coords["interaction"].x, data.coords["interaction"].y, data.coords["interaction"].z - 0.2, '~b~/multikeys~w~ [id] - Prendre les clés')
                                DrawText3Ds(data.coords["interaction"].x, data.coords["interaction"].y, data.coords["interaction"].z - 0.4, '~b~G~w~ - Voir le code PIN')
                                if IsControlJustPressed(0, 47) then
                                    QBCore.Functions.Notify('Code PIN: '..data.pincode)
                                end
                            end
                            if IsControlJustPressed(0, 74) then
                                local TraphouseInventory = {}
...
AddEventHandler('qb-traphouse:client:TakeoverHouse', function(TraphouseId)
    ...
    QBCore.Functions.Progressbar("takeover_traphouse", "Prendre ...", math.random(1000, 3000), false, true, {
        ...
        QBCore.Functions.Notify("Acquisitions annulées", "error")
    end)
end)
...
function AddKeyHolder(CitizenId, Traphouse)
    if #Config.TrapHouses[Traphouse].keyholders <= 6 then
        if not HasCitizenIdHasKey(CitizenId, Traphouse) then
            ...
            QBCore.Functions.Notify(CitizenId..' A été ajouté à la maison!')
        else
            QBCore.Functions.Notify(CitizenId..' Cette personne a déjà les clés!')
        end
    else
        QBCore.Functions.Notify('Vous pouvez donner jusqu\'à 6 personnes accès à la maison!')
    end
