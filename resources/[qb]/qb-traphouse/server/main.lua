    ...
    if Config.TrapHouses[TraphouseId] ~= nil then
        ...
        if Config.TrapHouses[TraphouseId].keyholders == nil then
            ...
            if #Config.TrapHouses[TraphouseId].keyholders + 1 <= 6 then
                ...
                TriggerClientEvent('QBCore:Notify', src, 'Vous n\'avez pas de slot libre')
            end
        end
    else
        TriggerClientEvent('QBCore:Notify', src, 'Une erreur s\'est produite')
    end
end)
...
QBCore.Commands.Add("multikeys", "Donnée les clés de la maison", {{name = "id", help = "ID du joueur"}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if TargetData ~= nil then
        if Traphouse ~= nil then
            if not HasCitizenIdHasKey(TargetData.PlayerData.citizenid, Traphouse) then
                ...
                TriggerClientEvent('QBCore:Notify', src, 'Cette personne a déjà les clés', 'error')
            end
        else
            TriggerClientEvent('QBCore:Notify', src, 'Vous ne possédez pas de maison ou n \'êtes pas le propriétaire', 'error')
        end
    else
        TriggerClientEvent('QBCore:Notify', src, 'Cette personne n\'est pas en ville', 'error')
    end
end)
...
AddEventHandler('qb-traphouse:server:TakeMoney', function(TraphouseId)
    ...
    if Config.TrapHouses[TraphouseId].money ~= 0 then
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Il n\'y a pas d\'argent dans le placard', 'error')
    end
end)
