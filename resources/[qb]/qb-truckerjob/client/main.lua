            ...
            if PlayerJob.name == "trucker" then
                ...
                if #(pos - vector3(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z)) < 10.0 then
                    ...
                    if #(pos - vector3(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z)) < 1.5 then
                        if IsPedInAnyVehicle(PlayerPedId(), false) then
                            DrawText3D(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z, "~g~E~w~ - Garé le véhicule")
                        else
                            DrawText3D(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z, "~g~E~w~ - Véhicule")
                        end
                            ...
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
                                    if isTruckerVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
                                        ...
                                        QBCore.Functions.Notify("Ce n'es pas un Véhicule commercial", 'error')
                                    end
                                else
                                    QBCore.Functions.Notify('vous devez être le conducteur pour faire sa...')
                                end
                            else
                                MenuGarage()
                ...
                if #(pos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 4.5 then
                    if #(pos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 1.5 then
                        DrawText3D(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z, "~g~E~w~ - Obtenir son salaire")
                        if IsControlJustReleased(0, 38) then
                            if JobsDone > 0 then
				    ...
				    ClearAllBlipRoutes()
                                ...
                                QBCore.Functions.Notify("Vous n'avez pas encore Travailler...", "error")
                            end
                        end
                    elseif #(pos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 2.5 then
                        DrawText3D(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z, "Bureau de paie")
                    end  
                end
                    ...
                    if #(pos - vector3(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z)) < 40.0 then
                        if not IsPedInAnyVehicle(PlayerPedId()) then
                            if not hasBox then
                                ...
                                if isTruckerVehicle(vehicle) and CurrentPlate == GetVehicleNumberPlateText(vehicle) then
                                    ...
                                    if #(pos - vector3(trunkpos.x, trunkpos.y, trunkpos.z)) < 1.5 and not isWorking then
                                        DrawText3D(trunkpos.x, trunkpos.y, trunkpos.z, "~g~E~w~ - Prendre le colis")
                                        if IsControlJustReleased(0, 38) then
                                            isWorking = true
                                            QBCore.Functions.Progressbar("work_carrybox", "Prendre un colis", 2000, false, true, {
                                                ...
                                                isWorking = false
                                                StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
                                                QBCore.Functions.Notify("Annuler", "error")
                                            end)
                                        end
                                    else
                                        DrawText3D(trunkpos.x, trunkpos.y, trunkpos.z, "Prendre le colis")
                                    end
                                end
                            elseif hasBox then
                                if #(pos - vector3(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z)) < 1.5 and not isWorking then
                                    DrawText3D(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z, "~g~E~w~ - Livrer le colis")
                                    if IsControlJustReleased(0, 38) then
                                        isWorking = true
                                        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
                                        Citizen.Wait(500)
                                        TriggerEvent('animations:client:EmoteCommandStart', {"bumbin"})
                                        QBCore.Functions.Progressbar("work_dropbox", "Livraison du colis", 2000, false, true, {
                                            disableMovement = true,
                                            disableCarMovement = true,
                                            disableMouse = false,
                                            disableCombat = true,
                                            ...
                                            if currentCount == CurrentLocation.dropcount then
                                                ...
                                                QBCore.Functions.Notify("Vous avez déposer tout les colis, allez au prochain points")
						    ...
						    ClearAllBlipRoutes()
                                    ...
                                    DrawText3D(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z, "Livrer les colis")
                                end
                            end
                        end
                    end
...
function getNewLocation()
    ...
    if location ~= 0 then
        ...
        QBCore.Functions.Notify("Vous avez finis votre tournée de livraison .. Il est l'heure de votre paye!")
        if CurrentBlip ~= nil then
            RemoveBlip(CurrentBlip)
	    ClearAllBlipRoutes()
            CurrentBlip = nil
...
function VehicleList(isDown)
    ...
    ClearMenu()
    for k, v in pairs(Config.Vehicles) do
        Menu.addButton(Config.Vehicles[k], "TakeOutVehicle", k, "Garage", " Moteur: 100%", " Carosserie: 100%", " Carburant: 100%")
    end
