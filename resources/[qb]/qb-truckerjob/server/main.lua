...
AddEventHandler('qb-trucker:server:DoBail', function(bool, vehInfo)
    ...
    if bool then
        if Player.PlayerData.money.cash >= Config.BailPrice then
            ...
            TriggerClientEvent('QBCore:Notify', src, 'Caution de 250$ payé en cash', 'success')
        ...
        elseif Player.PlayerData.money.bank >= Config.BailPrice then
            ...
            TriggerClientEvent('QBCore:Notify', src, 'Caution de 250$ payé depuis votre compte', 'success')
            TriggerClientEvent('qb-trucker:client:SpawnVehicle', src, vehInfo)
        else
            TriggerClientEvent('QBCore:Notify', src, 'Caution de 250$ requis', 'error')
        end
    else
        if Bail[Player.PlayerData.citizenid] ~= nil then
            Player.Functions.AddMoney('cash', Bail[Player.PlayerData.citizenid], "trucker-bail-paid")
            Bail[Player.PlayerData.citizenid] = nil
            TriggerClientEvent('QBCore:Notify', src, 'Caution de 250$ rendu', 'success')
        end
    end
end)
...
AddEventHandler('qb-trucker:server:01101110', function(drops)
    ...
    TriggerClientEvent('QBCore:Notify', src, 'Tu as gagné '..payment..'$', 'success')
end)
