...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
            ...
            if dist <= 1.0 then
				DrawText3D(MissionMarker.x, MissionMarker.y, MissionMarker.z, "~g~[E]~b~ Pour accepter la mission")
...
AddEventHandler('AttackTransport:InfoForLspd', function(x, y, z)
	if PlayerJob ~= nil and PlayerJob.name == 'police' then
		...
		if PoliceBlip == 0 then
			...
			AddTextComponentString("Attaque sur un transport d'argent")
		...
		if PoliceDist <= 4.5 then
			...
			if SilenceAlarm == 0 then
				hintToDisplay("Appuyer sur ~INPUT_DETONATE~ pour éteindre l'alarme")
				SilenceAlarm = 1
			end
    ...
    if PlayerJob.name == "police" then 
        local store = "Armored Truck"
            ...
            TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
                timeOut = 10000,
                alertTitle = "Tentative d'attaque sur un camion Blindé",
                coords = {
                    x = coords.x,
                    y = coords.y,
                    z = coords.z,
        ...
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentString("10-90: Camion blindé attaquer")
        EndTextCommandSetBlipName(blip)
        while transG ~= 0 do
            Wait(180 * 4)
            transG = transG - 1
...
function MissionNotification()
	...
	message = "Alors tu à envie de te faire de l'argent? bien... vas chercher une arme et met toi au travail... je t'envoie la localisation.",
	})
	Citizen.Wait(3000)
end
---
    ...
    while true do
		...
		if MissionStart == 1 then
			...
			if dist <= 55.0  then
				...
				if warning == 0 then
				warning = 1
				QBCore.Functions.Notify("Débarasser vous des gardes avant de mettres les explosifs", "error")
				end
			...
			if dist <= 7 and BlownUp == 0 and PlayerJob.name ~= 'police' then
				if BlowBackdoor == 0 then
					hintToDisplay("Appuyer sur [G] pour faire exploser la porte et prenez l'argent")
					BlowBackdoor = 1
				end
				if IsControlPressed(0, 47) and GuardsDead == 1 then 
...
if IsVehicleStopped(transport) then
	if IsVehicleSeatFree(transport, -1) and IsVehicleSeatFree(transport, 0) and IsVehicleSeatFree(transport, 1) and GuardsDead == 1 then
		if not IsEntityInWater(PlayerPedId()) then
			...
			QBCore.Functions.Notify('La charge explose dans '..TimeToBlow / 1000 ..' secondes.', "error")
			...
			BlownUp = 1
			lootable = 1
			QBCore.Functions.Notify("Commencer à récuperer l'argent", "success")
			RemoveBlip(TruckBlip)
		else
			QBCore.Functions.Notify("Sortez de l'eau", "error")
		end
	else
		QBCore.Functions.Notify('Le véhicule doit être vide pour mettre le chargement', "error")
	end
else
	QBCore.Functions.Notify('Vous ne pouvez pas braquer un véhicule en mouvement', "error")
end
end
		...
		if lootable == 1 then
			...
			if dist <= 4.5 then
				if PickupMoney == 0 then
					hintToDisplay("Appuyer sur [E] pour prendre l'argent")
					PickupMoney = 1
				end
				if IsControlJustPressed(0, 38) then 
				lootable = 0
...
function TakingMoney()
	...
	FreezeEntityPosition(PlayerPedId(), true)
	QBCore.Functions.Notify("Vous mettez l'argent dans le sac", "success")
	local _time = GetGameTimer()
	while GetGameTimer() - _time < 20000 do
		if IsControlPressed(0, 47) then
			break
		end
		hintToDisplay('Appuyer sur [G] pour partir')
		Citizen.Wait(1)
	end
