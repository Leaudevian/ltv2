...
if ActiveMission == 0 then
	...
	TriggerClientEvent('QBCore:Notify', _source, "Vous devez avoir "..ActivationCost.."$ sur votre compte bancaire pour accepter la mission")
	else
	...
	if copsOnDuty >= ActivePolice then
		...
		TriggerClientEvent('QBCore:Notify', _source, 'Besoin de'..ActivePolice.. ' SASP pour activer la mission.')
    end
	end
else
TriggerClientEvent('QBCore:Notify', _source, 'Quelqu\'un effectue déjà cette mission')
end
end)
...
AddEventHandler('qb-armoredtruckheist:server:callCops', function(streetLabel, coords)
    local place = "Camion blindé"
    local msg = "L'alarme a été activée d'un "..place.. " à " ..streetLabel
...
AddEventHandler('AttackTransport:graczZrobilnapad', function(moneyCalc)
	...
	TriggerClientEvent('QBCore:Notify', _source, 'Tu as pris '..bags..' sacs d\'argent de la camionnette')
