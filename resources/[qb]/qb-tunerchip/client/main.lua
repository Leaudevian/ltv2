...
RegisterNUICallback('save', function(data)
    QBCore.Functions.TriggerCallback('qb-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            ...
            setVehData(veh, data)
            QBCore.Functions.Notify('TunerChip v1.05: Véhicule réglé!', 'error')
...
AddEventHandler('qb-tunerchip:server:TuneStatus', function()
    ...
    QBCore.Functions.TriggerCallback('qb-tunerchip:server:GetStatus', function(status)
        if status then
            TriggerEvent("chatMessage", "État du véhicule", "warning", displayName..": Chiptuned: Oui")
        else
            TriggerEvent("chatMessage", "État du véhicule", "warning", displayName..": Chiptuned: Non")
        end
    end, plate)
end)
...
RegisterNUICallback('reset', function(data)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    QBCore.Functions.Notify('TunerChip v1.05: Le véhicule a été réinitialisé!', 'error')
end)
...
AddEventHandler('qb-tunerchip:client:openChip', function()
    ...
    if inVehicle then
        QBCore.Functions.Progressbar("connect_laptop", "Tunerchip v1.05: Le véhicule a été réinitialisé!", 2000, false, true, {
