...
AddEventHandler('smallresource:client:LoadNitrous', function()
    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            QBCore.Functions.Progressbar("use_nos", "NOS est connecté", 1000, false, true, {
            ...
            QBCore.Functions.Notify('Vous n\'êtes pas dans une voiture', 'error')
        end
    else
        QBCore.Functions.Notify('Vous avez déjà activé les NOS', 'error')
    end
end)
