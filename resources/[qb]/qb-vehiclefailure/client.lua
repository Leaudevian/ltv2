...
function CleanVehicle(vehicle)
	...
	QBCore.Functions.Progressbar("cleaning_vehicle", "Nettoyage de la voiture...", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	}, {}, {}, {}, function() -- Done
		QBCore.Functions.Notify("Véhicule nettoyé!")
...
function RepairVehicleFull(vehicle)
	...
	QBCore.Functions.Progressbar("repair_vehicle", "Réparation du véhicule..", math.random(20000, 30000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		QBCore.Functions.Notify("Véhicule réparé!")
...
function RepairVehicle(vehicle)
	...
	QBCore.Functions.Progressbar("repair_vehicle", "Réparation du véhicule..", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	...
	if isPedDrivingAVehicle() then
		...
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				notification("~g~" .. repairCfg.fixMessages[fixMessagePos] .. ", et maintenant aller dans un garage!")
				fixMessagePos = fixMessagePos + 1
				if fixMessagePos > repairCfg.fixMessageCount then fixMessagePos = 1 end
			else
				notification("~r~Votre véhicule est trop endommagé!")
			end
		else
		...
		notification("~y~Vous devez être dans un véhicule pour le réparer!")
	end
end)
...
AddEventHandler('iens:repaira', function()
	if isPedDrivingAVehicle() then
		...
		notification("Vous devez être dans un véhicule pour le réparer!")
	end
end)
...
notification("~r~Il y a une assistance sur route disponible appeler avec votre téléphone!")

end)

RegisterNetEvent('iens:notAllowed')
AddEventHandler('iens:notAllowed', function()
	notification("~r~Vous n'avez pas la permission de réparer ce véhicule")
end)
