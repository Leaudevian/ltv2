                ...
                if vehicle ~= nil and vehicle ~= 0 then
                    ...
                    if driver ~= 0 and not IsPedAPlayer(driver) then
                        if IsEntityDead(driver) then
                            IsRobbing = true
                            QBCore.Functions.Progressbar("rob_keys", "Prendre les clés..", 2500, false, true, {}, {}, {}, {}, function() -- Done
...
RegisterKeyMapping('togglelocks', 'Ouvrir/Fermer le véhicule', 'keyboard', 'L')
            ...
            if result then
                ...
                if vehLockStatus == 1 then
                    ...
                    if(GetVehicleDoorLockStatus(veh) == 2)then
                        ...
                        QBCore.Functions.Notify("Véhicule fermé!")
                    else
                        QBCore.Functions.Notify("Quelque chose s'est mal passé avec le système de verrouillage!")
                    end
                else
                    Citizen.Wait(750)
                    ClearPedTasks(ped)
                    ...
                    if(GetVehicleDoorLockStatus(veh) == 1)then
                        ...
                        Wait(200)
                        SetVehicleLights(veh,0)
                        QBCore.Functions.Notify("Véhicule ouvert!")
                    else
                        QBCore.Functions.Notify("Quelque chose s'est mal passé avec le système de verrouillage!")
                    end
                end
            else
                QBCore.Functions.Notify('Vous n`\'avez pas les clés du véhicule..', 'error')
            end
        end, plate)
    end
end
...
function Hotwire()
    if not HasKey then
        ...
        PoliceCall()
        QBCore.Functions.Progressbar("hotwire_vehicle", "Enclencher le contacteur d'allumage", hotwireTime, false, true, {
            ...
            if (math.random(0, 100) < 10) then
                ...
                TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
                TriggerEvent('vehiclekeys:client:SetOwner', GetVehicleNumberPlateText(vehicle))
                QBCore.Functions.Notify("Moteur démarré!")
            else
                SetVehicleEngineOn(veh, false, false, true)
                TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
                QBCore.Functions.Notify("Echec du démarrage!", "error")
            end
            IsHotwiring = false
        end, function() -- Cancel
            StopAnimTask(ped, "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
        ...
        if math.random(1, 100) <= chance then
            ...
            if closestPed ~= nil then
                ...
                if IsPedInAnyVehicle(ped) then
                    ...
                    local msg = "Tentative de vol de véhicule à " ..streetLabel.. ". Véhicule: " .. Name .. ", Immatriculation: " .. modelPlate
                    local alertTitle = "Tentative de vol de véhicule"
...
function RobVehicle(target)
    ...
    QBCore.Functions.Progressbar("rob_keys", "Tentative de vol..", 6000, false, true, {}, {}, {}, {}, function()
        local chance = math.random(1, 100)
        if chance > 85 then
            ...
            QBCore.Functions.Notify('Vous avez les clés!', 'success')
        else
            PoliceCall()
            TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
            TaskSmartFleePed(target, PlayerPedId(), 40.0, 20000)
            QBCore.Functions.Notify('Ils ont appelé les flics!', 'error')
            Wait(2000)
            IsRobbing = false
        end
    end)
