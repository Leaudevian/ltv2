        ...
        if QBCore ~= nil then
            ...
            if sellBackDist <= 13.0 and IsPedInAnyVehicle(ped) then 
                ...
                if sellBackDist <= 3.5 and IsPedInAnyVehicle(ped) then
                    ...
                    DrawText3Ds(Config.SellVehicleBack.x, Config.SellVehicleBack.y, Config.SellVehicleBack.z, '[~g~E~w~] - Vendre le véhicule  pour ~g~'..math.floor(sellVehData.price / 2)..'$')
                    if IsControlJustPressed(0, 38) then
                        QBCore.Functions.TriggerCallback('qb-garage:server:checkVehicleOwner', function(owned)
                            if owned then
                                ...
                                QBCore.Functions.Notify('Ce n\'est pas ton véhicule !', 'error', 3500)
                            end
            ...
            if inRange then
                    ...
                    if dstCheck <= 2 then
                        if not IsPedInAnyVehicle(ped) then
                            if not isConfirming then
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.45, '[~g~E~w~] - Voir le contrat du véhicule')
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.25, QBCore.Shared.Vehicles[Config.OccasionSlots[i]["model"]]["name"]..', Prix: ~g~'..Config.OccasionSlots[i]["price"]..'$')
                                if Config.OccasionSlots[i]["owner"] == QBCore.Functions.GetPlayerData().citizenid then
                                    DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.05, '[~r~G~w~] - Annulation du processus de vente')
                                ...
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.45, 'Êtes-vous sur de vouloir vendre ce véhicule ?')
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.25, '[~g~7~w~] - Oui | [~r~8~w~] - Non')
                ...
                if sellDist <= 13.0 and IsPedInAnyVehicle(ped) then 
                    ...
                    if sellDist <= 3.5 and IsPedInAnyVehicle(ped) then
                        DrawText3Ds(Config.SellVehicle.x, Config.SellVehicle.y, Config.SellVehicle.z, '[~g~E~w~] - Placer le véhicule à vendre')
                        if IsControlJustPressed(0, 38) then
                            ...
                            QBCore.Functions.TriggerCallback('qb-garage:server:checkVehicleOwner', function(owned)
                                if owned then
                                    ...
                                    QBCore.Functions.Notify('Ce n\'est pas votre véhicule ...', 'error', 3500)
                                end
                            end, VehiclePlate)
                        end
                    end
...
AddEventHandler('qb-occasions:client:BuyFinished', function(vehdata)
    ...
    QBCore.Functions.SpawnVehicle(vehdata.model, function(veh)
        ...
        QBCore.Functions.Notify("Véhicule acheté", "success", 2500)
...
AddEventHandler('qb-occasions:client:ReturnOwnedVehicle', function(vehdata)
    ...
    QBCore.Functions.SpawnVehicle(vehdata.model, function(veh)
        ...
        TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
        SetVehicleFuelLevel(veh, 100)
        QBCore.Functions.Notify("Votre véhicule est restitué")
        TriggerEvent("vehiclekeys:client:SetOwner", vehdata.plate)
        SetVehicleEngineOn(veh, true, true)
        Citizen.Wait(500)
        QBCore.Functions.SetVehicleProperties(veh, vehmods)
...
function sellVehicleWait(price)
    ...
    Citizen.Wait(1500)
    DoScreenFadeIn(250)
    QBCore.Functions.Notify('Votre voiture a bien été mise en vente ! Prix - $'..price, 'success')
