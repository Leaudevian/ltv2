...
AddEventHandler('qb-occasions:server:ReturnVehicle', function(vehicleData)
    ...
    if result[1] ~= nil then 
        if result[1].seller == Player.PlayerData.citizenid then
            ...
            TriggerClientEvent('QBCore:Notify', src, 'Ce n\'est pas votre véhicule', 'error', 3500)
        end
    else
        TriggerClientEvent('QBCore:Notify', src, 'Ce véhicule n\'existe pas', 'error', 3500)
    end
end)
...
AddEventHandler('qb-occasions:server:sellVehicle', function(vehiclePrice, vehicleData)
    ...
    TriggerEvent("qb-log:server:CreateLog", "vehicleshop", "Véhicule vendu", "red", "**"..GetPlayerName(src) .. "** a vendu un(e) " .. vehicleData.model .. " pour "..vehiclePrice.."$")
...
AddEventHandler('qb-occasions:server:sellVehicleBack', function(vData)
    ...
    TriggerClientEvent('QBCore:Notify', src, 'Tu as vendu ce véhicule pour $'..price, 'success', 5500)
...
AddEventHandler('qb-occasions:server:buyVehicle', function(vehicleData)
    ...
    if result[1] ~= nil and next(result[1]) ~= nil then
        if Player.PlayerData.money.bank >= result[1].price then
            ...
            TriggerEvent("qb-log:server:CreateLog", "vehicleshop", "bought", "green", "**"..GetPlayerName(src) .. "** à vendu pour "..result[1].price .."$ (" .. result[1].plate .. ") à **"..SellerCitizenId.."**")
            ...
            TriggerEvent('qb-phone:server:sendNewMailToOffline', SellerCitizenId, {
                ...
                subject = "Tu as vendu un véhicule !",
                message = "La voiture "..QBCore.Shared.Vehicles[result[1].model].name.." a été vendu pour $"..result[1].price.."!"
            })
        else
