				...
				if nearlocation <= 15 then
					...
					if nearlocation <= 3 then
						if not startVineyard then
							DrawText3Ds(-1928.81, 2059.53, 140.84, "[E] Commencer à cueillir des raisins") 
							
								if IsControlJustReleased(0,38) then
									if PlayerJob.name == "vineyard" then
										startVineyard = true
									else
										QBCore.Functions.Notify("Tu n'es pas vigneron !", "error")
									end
								end
						end
					end
...
function pickProcess()
    QBCore.Functions.Progressbar("pick_grape", "Cueillette des raisins ...", math.random(6000,8000), false, true, {
						...
						if not wineStarted then
							if not loadIngredients then
								if #(pos - vector3(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y, Config.Vineyard["wine"].coords.z)) < 1 then
									DrawText3Ds(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y,  Config.Vineyard["wine"].coords.z + 0.2, '[E] Mélange des ingrédients')
								...
								if not finishedWine then
									if #(pos - vector3(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y, Config.Vineyard["wine"].coords.z)) < 1 then
										DrawText3Ds(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y, Config.Vineyard["wine"].coords.z + 0.2, '[E] Démarrage du processus de vin')
										if IsControlJustPressed(0, 38) then
											if PlayerJob.name == "vineyard" then
												StartWineProcess()
											else
									...
									if #(pos - vector3(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y, Config.Vineyard["wine"].coords.z)) < 1 then
										DrawText3Ds(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y, Config.Vineyard["wine"].coords.z + 0.2, '[E] Prendre le vin')
										if IsControlJustPressed(0, 38) then
											if PlayerJob.name == "vineyard" then
							...
							DrawText3Ds(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y, Config.Vineyard["wine"].coords.z - 0.4, 'Terminé dans '..winetimer..'secondes')
						end
					end
				end
			...
			if nearlocation <= 15 then
				...
				if nearlocation <= 3 then	
					...
					if #(pos - vector3(Config.Vineyard["grapejuice"].coords.x, Config.Vineyard["grapejuice"].coords.y, Config.Vineyard["grapejuice"].coords.z)) < 1 then
						DrawText3Ds(Config.Vineyard["grapejuice"].coords.x, Config.Vineyard["grapejuice"].coords.y,  Config.Vineyard["grapejuice"].coords.z + 0.2, '[E] Faire du jus de raisin')
						if IsControlJustPressed(0, 38) then
							if PlayerJob.name == "vineyard" then
...
function grapeJuiceProcess()
    QBCore.Functions.Progressbar("grape_juice", "Jus de raisin en cours ...", math.random(15000,20000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
