                ...
                if IsPedShooting(ped) or IsControlJustPressed(0, 24) then
                    if CanShoot then
                        ...
                        if weapon ~= -1569615261 then
                            ...
                            QBCore.Functions.Notify("Cette arme est casser et ne peut plus être utiliser", "error")
                            MultiplierAmount = 0
                        end
                    end
                end
    ...
    if CurrentWeaponData ~= nil then
        if QBCore.Shared.Weapons[weapon]["name"] ~= "weapon_unarmed" and QBCore.Shared.Weapons[weapon]["ammotype"] == type:upper() then
            ...
            if total < maxAmmo then
                QBCore.Functions.Progressbar("taking_bullets", "Chargement des balles", math.random(4000, 6000), false, true, {
                ...
                QBCore.Functions.Notify("Capacité de balles max.", "error")
            end
        else
            QBCore.Functions.Notify("Vous n'avez pas d'arme.", "error")
        end
    else
        QBCore.Functions.Notify("Vous n'avez pas d'arme", "error")
    end
                    ...
                    if distance < 1 then
                        if data.IsRepairing then
                            if data.RepairingData.CitizenId ~= PlayerData.citizenid then
                                DrawText3Ds(data.coords.x, data.coords.y, data.coords.z, "La station de réparation n'est pas utilisable pour le moment")
                            else
                                if not data.RepairingData.Ready then
                                    DrawText3Ds(data.coords.x, data.coords.y, data.coords.z, 'Votre arme vas être réparer')
                                else
                                    DrawText3Ds(data.coords.x, data.coords.y, data.coords.z, "[E] pour prendre l'arme")
                                end
                            end
                        else
                            if CurrentWeaponData ~= nil and next(CurrentWeaponData) ~= nil then
                                if not data.RepairingData.Ready then
                                    ...
                                    DrawText3Ds(data.coords.x, data.coords.y, data.coords.z, "[E] Réparer l'arme, ~g~$"..Config.WeaponRepairCotsts[WeaponClass]..'~w~')
                                ...
                                if data.RepairingData.CitizenId == nil then
                                    DrawText3Ds(data.coords.x, data.coords.y, data.coords.z, "Vous n'avez pas d'arme en main...")
    ...
    if weapon ~= GetHashKey("WEAPON_UNARMED") then
        ...
        if WeaponAttachments[WeaponData.name] ~= nil then
            if WeaponAttachments[WeaponData.name][attachment] ~= nil then
                ...
                QBCore.Functions.Notify("L'arme ne supporte pas cette attachment..", "error")
            end
        end
    else
        QBCore.Functions.Notify("Vous n'avez pas d'arme en main...", "error")
    end
end)
