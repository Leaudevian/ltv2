            ...
            if WeaponSlot.info.quality ~= nil then
                for i = 1, RepeatAmount, 1 do
                    if WeaponSlot.info.quality - DecreaseAmount > 0 then
                        ...
                        TriggerClientEvent('QBCore:Notify', src, "Votre arme est endommagée, vous devez la faire réparer.", "error")
                        break
                    end
                end
            else
...
QBCore.Commands.Add("repairweapon", "Réparer une arme (God Uniquement)", {{name="hp", help="HP de votre arme"}}, true, function(source, args)
    ...
    if Player.PlayerData.items[data.slot] ~= nil then
        if Player.PlayerData.items[data.slot].info.quality ~= nil then
            if Player.PlayerData.items[data.slot].info.quality ~= 100 then
                if Player.Functions.RemoveMoney('cash', Config.WeaponRepairCotsts[WeaponClass]) then
                    ...
                    SetTimeout(Timeout, function()
                        ...
                        TriggerEvent('qb-phone:server:sendNewMailToOffline', Player.PlayerData.citizenid, {
                            sender = "Tyrone",
                            subject = "Réparation",
                            message = "Votre "..WeaponData.label.." est réparé que vous pouvez venir le récupérer. <br><br> Peace out madafaka"
                        })
                ...
                TriggerClientEvent("QBCore:Notify", src, "Cette arme n\'est pas endommagée..", "error")
                cb(false)
            end
        else
            TriggerClientEvent("QBCore:Notify", src, "Cette arme n\'est pas endommagée..", "error")
        ...
        TriggerClientEvent('QBCore:Notify', src, "Vous n\'avez pas une arme entre vos mains..", "error")
    ...
    if Inventory[CurrentWeaponData.slot] ~= nil then
        if Inventory[CurrentWeaponData.slot].info.attachments ~= nil and next(Inventory[CurrentWeaponData.slot].info.attachments) ~= nil then
            ...
            if not HasAttach then
                ...
                TriggerClientEvent("QBCore:Notify", src, "Vous avez déjà un "..AttachmentData.label:lower().." sur votre arme.", "error", 3500)
            end
        else
    ...
    if Inventory[ItemData.slot] ~= nil then
        if Inventory[ItemData.slot].info.attachments ~= nil and next(Inventory[ItemData.slot].info.attachments) ~= nil then
            ...
            if HasAttach then
                ...
                TriggerClientEvent("QBCore:Notify", src, "Vous avez retiré "..AttachmentComponent.label.." de votre arme!", "error")
