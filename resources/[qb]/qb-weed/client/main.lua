                    ...
                    if plyDistance < 0.8 then
                        ...
                        if plantData["plantStats"]["health"] > 0 then
                            if plantData["plantStage"] ~= plantData["plantStats"]["highestStage"] then
                                QBWeed.DrawText3Ds(plantData["plantCoords"]["x"], plantData["plantCoords"]["y"], plantData["plantCoords"]["z"], 'Sort: '..plantData["plantSort"]["label"]..'~w~ ['..plantData["plantStats"]["gender"]..'] | Nutrition: ~b~'..plantData["plantStats"]["food"]..'% ~w~ | Health: ~b~'..plantData["plantStats"]["health"]..'%')
                            else
                                ...
                                if IsControlJustPressed(0, 38) then
                                    QBCore.Functions.Progressbar("remove_weed_plant", "Récolter la weed", 8000, false, true, {
                                        ...
                                        ClearPedTasks(ped)
                                        QBCore.Functions.Notify("Progres Annuler", "error")
                                    end)
                                end
                            end
                        elseif plantData["plantStats"]["health"] == 0 then
                            QBWeed.DrawText3Ds(plantData["plantCoords"]["x"], plantData["plantCoords"]["y"], plantData["plantCoords"]["z"], 'Le plan de weed est mort. Appuyer sur ~r~ E ~w~ pour enlever le plan de weed.')
                            if IsControlJustPressed(0, 38) then
                                QBCore.Functions.Progressbar("remove_weed_plant", "Enlever le plan de weed", 8000, false, true, {
                                    disableMovement = true,
                                    disableCarMovement = true,
                                    disableMouse = false,
                                    disableCombat = true,
...
AddEventHandler('qb-weed:client:placePlant', function(type, item)
    ...
    if currentHouse ~= nil then
        if ClosestPlant == 0 then
            ...
            QBCore.Functions.Notify("Vous ne pouvez pas planter ici", 'error', 3500)
        end
    else
        QBCore.Functions.Notify("Cette endroit n'est pas sur, essayer chez vous", 'error', 3500)
    end
end)
        ...
        if ClosestTarget ~= 0 then
            ...
            if plyDistance < 1.0 then
                if plantData["plantStats"]["food"] == 100 then
                    QBCore.Functions.Notify("Le plan de weed n'a pas besoin de nutrition", 'error', 3500)
                else
                    QBCore.Functions.Progressbar("plant_weed_plant", "Vous nourrissez la plante", math.random(4000, 8000), false, true, {
                        disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                ...
                QBCore.Functions.Notify("Vous ne pouvez pas le placer ici", "error")
            end
        else
            QBCore.Functions.Notify("Vous ne pouvez pas le placer ici", "error")
        end
