...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    for pos, data in ipairs(queueList) do
        if tmp.priority then
            ...
            if _pos then
                Queue:DebugPrint(string_format("%s[%s] est prioritaire et placé %d/%d dans la file d'attente", tmp.name, ids[1], _pos, queueCount))
                break
            end
        end
    end
    ...
    if not _pos then
        ...
        Queue:DebugPrint(string_format("%s[%s] est placé %d/%d dans la file d'attente", tmp.name, ids[1], _pos, queueCount))
    end
...
function Queue:AddToConnecting(ids, ignorePos, autoRemove, done)
    local function remove()
        ...
        Queue:DebugPrint("Le joueur n'a pas pu être ajouté dans la file d'attente")
    end
    ...
    if type(id) == "table" then
        for _id, power in pairs(id) do
            if _id and type(_id) == "string" and power and type(power) == "number" then
                ...
                Queue:DebugPrint("Erreur lors de l'ajout d'un identifiant de priorité, des données non valides ont été transmises")
                return false
            end
        end
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    Queue:CanJoin(src, function(reason)
        ...
        if reason then
            -- prevent joining
            allow = false
            done(reason and tostring(reason) or "Vous avez êtes bloquer")
            Queue:RemoveFromQueue(ids)
            Queue:RemoveFromConnecting(ids)
            Queue:DebugPrint(string_format("%s[%s] est bloqué ; Raison: %s", name, ids[1], reason))
            CancelEvent()
            return
        end
    ...
    if Queue:IsInQueue(ids) then
        ...
        Queue:DebugPrint(string_format("%s[%s] a rejoint la file d'attente avant l'annulation", name, ids[1]))
    else
        Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    while true do
        ...
        if not data or not data.deferrals or not data.source or not pos then
            remove("[Queue] Retiré de la file d'attente, données invalides :(")
        ...
        if data.timeout >= Config.QueueTimeOut and os_time() - connectTime > 5 then
            remove("[Queue] TimeOut")
            Queue:DebugPrint(name .. "[" .. ids[1] .. "] a été retiré de la file d'attente suite à un TimeOut")
            return
        end
    ...
    while true do
        ...
        while i <= Queue:ConnectingSize() do
            ...
            if ((data.timeout >= 300 and not endPoint) or data.timeout >= Config.ConnectTimeOut) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                remove(data)
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] a été retiré de la file d'attente suite à un TimeOut")
            else
                i = i + 1
            end
        end
...
commands.removec = function(args)
    ...
    Queue:DebugPrint("REMOVED " .. tostring(name) .. " RETIRER DE LA FILE D'ATTENTE")
end
...
commands.printp = function()
    Queue:DebugPrint("FILE D'ATTENTE PRIORITAIRE")
...
commands.printcount = function()
    Queue:DebugPrint("Nombre de joueur : " .. Queue:GetPlayerCount())
end
