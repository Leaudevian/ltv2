	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Navigue avec ~y~W,A,S,D~s~ est confirme avec ~y~SPACE~s~ pour le bloc de gauche.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Utilise ~y~Arrow Keys~s~ et ~y~ENTER~s~ pour le bloc de droite")
			else
